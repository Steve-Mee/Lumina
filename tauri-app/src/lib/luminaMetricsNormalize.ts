/** Lumina metrics normalize helpers (store/HUD residual). */
import type { LuminaMetrics } from "@/lib/luminaMetricsTypes";
import { EMPTY_METRICS } from "@/lib/luminaMetricsTypes";
...
export function extractPhaseFromSnapshot(snapshot: Record<string, unknown>): string {
  for (const [key, payload] of Object.entries(snapshot)) {
    if (
...
export function shallowUiFromRoot(snapshot: Record<string, unknown>): Partial<LuminaMetrics> | null {
