# AWAKENING_MARK_EYES_POLISH_AUDIT

## Gate 0 live-check + inspect_polish_protocol

```json
{
  "origin_main": "b6e835ca74e70b36274826efd506a76d9d5bfeb2",
  "POLICY_EDGE_MIN_TRADES": 150,
  "OBSERVATION_DIM": 43,
  "pct_synthetic": 0.0,
  "pct_real_historical": 0.0,
  "pct_real": 100.0,
  "inspect_complete": true
}
```

```json
{
  "init_sha_a9ffa852": "lumina_core/birth/awakening_mark_eyes_polish.py:1",
  "forbidden_init_8cc435c6": "lumina_core/birth/awakening_mark_eyes_polish.py:40",
  "forbidden_init_d313b107": "lumina_core/birth/awakening_mark_eyes_polish.py:40",
  "forbidden_init_53df2d78": "lumina_core/birth/awakening_mark_eyes_polish.py:40",
  "forbidden_init_scratch": "lumina_core/birth/awakening_mark_eyes_polish.py:97",
  "timesteps_10000": "lumina_core/birth/awakening_mark_eyes_polish.py:29",
  "seed_20260906": "lumina_core/birth/awakening_mark_eyes_polish.py:25",
  "start_et_2026_08_03": "lumina_core/birth/awakening_mark_eyes_polish.py:173",
  "holdout_pct_0_40": "lumina_core/birth/awakening_mark_eyes_polish.py:28",
  "floor_150": "lumina_core/birth/foundation_metrics.py:39",
  "license_requires_both_legs": "lumina_core/birth/awakening_mark_eyes_polish_flags.py:140",
  "genesis_eyes_ok_forced_false": "lumina_core/birth/awakening_mark_eyes_polish_flags.py:157",
  "hooks_default_false": "lumina_core/birth/awakening_path_exit_k3.py:22",
  "hooks_shape_default_false": "lumina_core/birth/awakening_path_shape_k3_dead.py:57",
  "synthetic_pct_zero": "lumina_core/birth/data_source_honesty.py:22",
  "reset_num_timesteps_false": "lumina_core/birth/awakening_mark_eyes_polish_train.py:197",
  "missing_sites": [],
  "gate0_complete": true
}
```

## T0 identity

```json
{
  "origin_main": "b6e835ca74e70b36274826efd506a76d9d5bfeb2",
  "fixture_seed": 20260906,
  "fixture_train_hash": "afcea4fa72734337",
  "init_sha256": "a9ffa8529e02f2d8f8a535be4dcce205a43abe20bdec492add78126a8181188b",
  "child_sha256": "cebe1804e7d0e20d0120c4e7b61823a220640e1b1a7d9d73108eba373af32ee3",
  "init_policy": "continue_a9ffa852",
  "OBSERVATION_DIM": 43,
  "POLICY_EDGE_MIN_TRADES": 150,
  "timesteps": 10000,
  "train_seed": 20260906
}
```

## T1 honesty / G1 fixture

```json
{
  "pct_synthetic_cloud_fixture": 0.0,
  "pct_real_historical": 0.0,
  "pct_real": 100.0,
  "synthetic_source_reasons": [
    "synthetic_source:synthetic_cloud_fixture"
  ],
  "min_real_data_pct": 95.0,
  "G6_tag": "REAL_DOOR_LOCKED"
}
```

## T2 G2 frozen-child vs G4 polish child (THIS tape)

### Leg A

```json
{
  "leg": "A",
  "n_policy_base": 150,
  "n_policy_child": 150,
  "wr_base": 0.4666666666666667,
  "wr_child": 0.46,
  "mean_r_base": -0.0628327555400125,
  "mean_r_child": -0.037262189664975486,
  "n_H_base": 15,
  "n_H_child": 21,
  "bars_held_p50_base": 100.0,
  "bars_held_p50_child": 94.5,
  "delta_mean_r": 0.02557056587503701,
  "delta_n_H": -6,
  "HOLE_OK": false,
  "MOVED": false,
  "S_THIN": false,
  "S_HARM": false,
  "S_MISSING": false
}
```

### Leg B

```json
{
  "leg": "B",
  "n_policy_base": 150,
  "n_policy_child": 150,
  "wr_base": 0.4666666666666667,
  "wr_child": 0.54,
  "mean_r_base": -0.06361365410095753,
  "mean_r_child": 0.0680918275697903,
  "n_H_base": 26,
  "n_H_child": 20,
  "bars_held_p50_base": 90.0,
  "bars_held_p50_child": 90.0,
  "delta_mean_r": 0.13170548167074783,
  "delta_n_H": 6,
  "HOLE_OK": true,
  "MOVED": true,
  "S_THIN": false,
  "S_HARM": false,
  "S_MISSING": false
}
```

## T3 license vs G2 books

```json
{
  "tag": "POLISH_FAIL",
  "law": "NONE",
  "licensed_next_family": "H_NONE",
  "MOVED_A": false,
  "MOVED_B": true,
  "GENESIS_EYES_OK": false,
  "Proof": false,
  "REAL": "no",
  "honesty": "Continue a9ffa852 one 10k on a NEW synthetic tape. Not a second Birth. Floor POLICY_EDGE_MIN_TRADES=150 stays. GENESIS_EYES_OK stays false. POLISH_OK is not Evolution Proof. REAL=no. Source synthetic_cloud_fixture. License vs the frozen child on THIS tape, not vs newborn or path_early."
}
```

## G6 REAL door

```json
{
  "G6_tag": "REAL_DOOR_LOCKED",
  "REAL": "no",
  "rows": [
    {
      "function": "config.yaml mode",
      "result": "PASS",
      "why": "mode='sim' (must stay sim)"
    },
    {
      "function": "BirthCertificateThresholds.min_real_data_pct lumina_core/birth/birth_certificate.py:14",
      "result": "FAIL",
      "why": "real_data_pct=0.0 < min_real_data_pct=95.0"
    },
    {
      "function": "tick source",
      "result": "FAIL-CLOSED",
      "why": "source=synthetic_cloud_fixture (synthetic_cloud_fixture is not a REAL certificate)"
    },
    {
      "function": "ApplicationContainer.start lumina_core/container/container_lifecycle.py:164",
      "result": "PASS",
      "why": "container.start not called (broker connect forbidden)"
    },
    {
      "function": "NinjaTrader / Fabric / gRPC",
      "result": "PASS",
      "why": "NT/Fabric host not contacted"
    },
    {
      "function": "PromotionGate.evaluate lumina_core/evolution/promotion_gate.py:136",
      "result": "FAIL",
      "why": "no proving certificate / no promotion evidence on synthetic first life"
    },
    {
      "function": "evolution_proof_passed lumina_core/birth/evolution_proof_gate.py:137",
      "result": "FAIL",
      "why": "stamped=False (must stay false)"
    },
    {
      "function": "maturation_eligible_for_real lumina_core/maturity/maturation_progress.py:190",
      "result": "FAIL",
      "why": "eligible=False blockers=['Birth Certificate v2 issued', 'Evolution Proof passed', 'SIM stability READY_FOR_REAL (5-day green streak)', 'Promotion gate passed (shadow validation)', 'Perfect Birth Phase complete (twin vs Steve accuracy + never-stop recovery + auto-approval + shadow alignment)']"
    },
    {
      "function": "certificate OOS WR 0.48",
      "result": "FAIL",
      "why": "min_oos_winrate=0.48 not claimed"
    },
    {
      "function": "kill-switch / Dead Man lumina_core/risk/risk_gates.py:169",
      "result": "N/A",
      "why": "not armed against a live broker (no broker connect)"
    },
    {
      "function": "HardRiskController lumina_core/risk/risk_controller.py:36",
      "result": "PRESENT",
      "why": "risk engine independent of strategy (bounded context lumina_core/risk)"
    },
    {
      "function": "first live/SIM broker order",
      "result": "NONE",
      "why": "no container.start, no order path"
    },
    {
      "function": "lumina_core/rl/observation_builder.py:36 OBSERVATION_DIM",
      "result": "PASS",
      "why": "OBSERVATION_DIM=43 (must stay 43)"
    },
    {
      "function": "PATH_EXIT_K3_SHADOW / PATH_SHAPE_K3_SHADOW",
      "result": "PASS",
      "why": "exit=False shape=False default False"
    }
  ],
  "broken": [],
  "real_data_pct": 0.0,
  "source": "synthetic_cloud_fixture",
  "mode": "sim",
  "evolution_proof_stamped": false,
  "promotion_passed": false,
  "nt_called": false,
  "container_start_called": false,
  "observation_dim": 43
}
```

## Honesty

Continue a9ffa852 one 10k on a NEW synthetic tape. Not a second Birth. Floor POLICY_EDGE_MIN_TRADES=150 stays. GENESIS_EYES_OK stays false. POLISH_OK is not Evolution Proof. REAL=no. Source synthetic_cloud_fixture. License vs the frozen child on THIS tape, not vs newborn or path_early.

Origin genesis/budget artifacts were not overwritten.
GENESIS_EYES_OK is false. REAL=no. Floor 150. One continue 10k.

## flags

```json
{
  "source": "awakening_mark_eyes_polish",
  "fixture_seed": 20260906,
  "fixture_train_hash": "afcea4fa72734337",
  "init_sha256": "a9ffa8529e02f2d8f8a535be4dcce205a43abe20bdec492add78126a8181188b",
  "child_sha256": "cebe1804e7d0e20d0120c4e7b61823a220640e1b1a7d9d73108eba373af32ee3",
  "init_policy": "continue_a9ffa852",
  "learn_called": true,
  "actual_timesteps": 10000,
  "optimizer_steps": 180,
  "A": {
    "n_policy_base": 150,
    "n_policy_child": 150,
    "mean_r_base": -0.0628327555400125,
    "mean_r_child": -0.037262189664975486,
    "n_H_base": 15,
    "n_H_child": 21,
    "wr_base": 0.4666666666666667,
    "wr_child": 0.46,
    "n_W_base": 50,
    "n_W_child": 52,
    "bars_held_p50_base": 100.0,
    "bars_held_p50_child": 94.5,
    "delta_mean_r": 0.02557056587503701,
    "delta_n_H": -6,
    "HOLE_OK": false,
    "S_THIN": false,
    "S_HARM": false,
    "MOVED": false,
    "S_MISSING": false
  },
  "B": {
    "n_policy_base": 150,
    "n_policy_child": 150,
    "mean_r_base": -0.06361365410095753,
    "mean_r_child": 0.0680918275697903,
    "n_H_base": 26,
    "n_H_child": 20,
    "wr_base": 0.4666666666666667,
    "wr_child": 0.54,
    "n_W_base": 49,
    "n_W_child": 63,
    "bars_held_p50_base": 90.0,
    "bars_held_p50_child": 90.0,
    "delta_mean_r": 0.13170548167074783,
    "delta_n_H": 6,
    "HOLE_OK": true,
    "S_THIN": false,
    "S_HARM": false,
    "MOVED": true,
    "S_MISSING": false
  },
  "tag": "POLISH_FAIL",
  "GENESIS_EYES_OK": false,
  "law": "NONE",
  "licensed_next_family": "H_NONE",
  "evolution_proof_stamped": false,
  "REAL": "no",
  "playground": false,
  "hook_default": false,
  "used_old_path_early": false,
  "used_budget_ledgers_as_baseline": false,
  "real_data_pct": 0.0,
  "G6_tag": "REAL_DOOR_LOCKED",
  "overall": "AWAKENING_MARK_EYES_POLISH SHADOW_MEASURE",
  "MOVED_A": false,
  "MOVED_B": true,
  "missing_reason": ""
}
```
