...
overall=`GENESIS_CLOUD_LADDER SHADOW_MEASURE` birth_exited=`True` G5_tag=`GENESIS_EYES_FAIL` G6_tag=`REAL_DOOR_LOCKED` REAL=`no` playground=`false` evolution_proof_stamped=`false`. First life. Door locked.
