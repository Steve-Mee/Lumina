...
| Field | PR #10 holdout (live 532 / ckpt 533) | PR #11 S5 (stall 950) | This shadow S5 (996, after cap) |
|---|---|---|---|
| n | 532 (live `oos_dd=5757.72`) / 533 (checkpoint) | 950 | **996** |
| sum | −2,878,857.77 (532) / −2,880,869.53 (533) | −2,120,334.68 | **−131,009.46** |
| min | **−1,053,820.7964710598** | −631,258.80 | **−501.25** |
| max | +339,416.20 | +778,641.20 | **+501.25** |
| mean | −5,411.39 (532) / −5,405.01 (533) | −2,231.93 | **−131.54** |
| median | ~−$526 typical increment | median −211.80; median \|pnl\| 348.71 | 731 closes at exactly ±501.25 |
| `stage_val_r` of min | −157.36 | −81.93 | −0.97 (clipped; ledger `trade_r`) |

SSOT: `git show 7dd4a69:reports/birth_cloud_run/artifacts/lumina_birth_checkpoint.json` (PR #10),
PR #11 n=950 (pre-this-PR artifacts), this-shadow
`reports/birth_cloud_run/workspace/state/lumina_birth_checkpoint.json`.

---

## Counts \|pnl\| > 1k / 2k / 10k / 100k

| Threshold | PR #10 (533) | PR #11 (950) | This shadow S5 (996) |
|---|---|---|---|
| \|pnl\| > 1,000 | 179 | 297 | **0** |
| \|pnl\| > 2,000 | 162 | 268 | **0** |
| \|pnl\| > 10,000 | 84 | 146 | **0** |
| \|pnl\| > 100,000 | 33 | 56 | **0** |
| \|pnl\| > 501.25 | — | — | **0** (cap + 1 tick holds) |
...
### This shadow S5 (persisted `close_ledger`, qty/reason live)

All 996 closes: `qty=1`, `cap_usd=500`, `point_value=20` (NQ fill), `plant=False`.
109 gap flags; booked PnL still ≤ $501.25. Equity column is the exam yardstick
(`S5_DD_EQUITY_USD=50000`), not gym running equity. `stop_pct` = geometry
`risk_usd / (entry × NQ $20)` ≈ 0.00268.

| pnl | qty | pv | entry | exit | stop_pct | risk_usd | trade_r | reason | gap | plant/policy | equity |
|---|---|---|---|---|---|---|---|---|---|---|---|
| −501.25 | 1 | 20 | 21132.07 | booked at cap (not raw mark) | ~0.00268 | 1132.83 | −0.442 | stop | False | policy | 50000 |
| −501.25 | 1 | 20 | 21397.50 | booked at cap | ~0.00268 | 1147.06 | −0.437 | stop | False | policy | 50000 |
| −501.25 | 1 | 20 | 21418.25 | booked at cap | ~0.00268 | 1148.17 | −0.437 | stop | False | policy | 50000 |
| −501.25 | 1 | 20 | 21419.00 | booked at cap | ~0.00268 | 1148.21 | −0.437 | time_stop | False | policy | 50000 |
| −501.25 | 1 | 20 | 21421.25 | booked at cap | ~0.00268 | 1148.33 | −0.437 | time_stop | False | policy | 50000 |

Ledger reasons: stop 439 / time_stop 430 / target 127 / flatten 0.
A close of \|pnl\| > $500 + 1 tick at qty=1 is **not** in this book.

---

## force_qty_one live on shadow config

**True** — `lumina_core/rl/gym_stop_fill.py:32` `birth_force_qty_one` returns True
for every birth regime (including `stage5_probe_handoff`).

Live assignment: `lumina_core/birth/sim_runner.py:257`
`force_qty_one=bool(birth_force_qty_one(...))`.

Shadow HUD: `last_close_qty=1`, `last_cap_usd=500.0`.
...
`lumina_core/birth/notional_cap.py:24` `birth_close_cap_usd`,
`:53` `clip_birth_exam_pnl`, `:69` `birth_stop_pct_dollar_cap` (qty in the denominator).
...
Live shadow after Gate 0+1: plant/policy = **0/996**, FORCE_OPEN = **0**,
occupancy seed = S4 **0.4746** (`s4_receipt`), occ ≈ **0.280** in [0.25, 0.75].
`oos_edge` clears −0.03 (`edge_vs_first_touch=+0.048`). `oos_dd` last live
**219.42** percent-of-50k > 25; `oos_sharpe` **−4.52** ≤ −2.0. That is B2 —
a real process/OOS fail. Floors not raised.
