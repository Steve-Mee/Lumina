...
@pytest.fixture(autouse=True)
def _disable_birth_meta_controller_for_root_birth_tests(
    monkeypatch: pytest.MonkeyPatch, request: pytest.FixtureRequest
) -> None:
    """Birth tests under tests/ (not tests/birth/) need the same meta-controller shim."""
    module_name = getattr(request.module, "__name__", "")
    if not (
        module_name.startswith("tests.test_birth_")
        or module_name.startswith("tests.test_lumina_birth_")
    ):
        return
    if module_name.endswith(("test_meta_controller", "test_meta_self_eval")):
        return
