...
def test_coverage_license_and_honesty() -> None:
    ...
    assert (
        isolated_workspace(Path("reports") / "birth_cloud_run")
        .as_posix()
        .endswith("awakening_path_shape_k3_dead/workspace")
    )
...
def test_coverage_measure_tiny_fixture() -> None:
    ...
    extra = [
        _row(close_reason="time_stop", trade_r=-0.1, pnl=-5.0, entry_bar_index=200 + i, bars_held=8) for i in range(30)
    ]
...
def test_coverage_write_reports_and_tables(tmp_path: Path) -> None:
    ...
    from lumina_core.birth.awakening_path_shape_k3_dead_tables import (
        table_t0,
        table_t1,
        table_t2,
        table_t3,
    ...
    exits = [
        _row(
            ...
            pnl=-12.0,
            path_exit_k3=True,
            path_exit_k3_shape="DEAD",
            path_exit_k3_mae_r=-0.30,
            path_exit_k3_mfe_r=0.0,
            path_exit_k3_family=FAMILY,
    ...
    write_path_shape_k3_dead_reports(
        ...
        skip_replay=False,
        replay_ran=True,
        gate0_sha="eb3184db8a7931991752e0e3eef3f1149269d20f",
    )
...
def test_coverage_run_measure_only(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        run_mod, "inspect_path_shape_k3_dead_protocol", lambda: {"gate0_complete": True, "missing_sites": []}
    )
...
def test_coverage_run_evaluate_only_mocked(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    holes = [
        _row(entry_bar_index=i, path_k3_unreal_r=-0.30, path_k3_mae_r=-0.30, path_k3_mfe_r=0.0, bars_held=8)
        for i in range(50)
    ]
    winners = [
        _row(
    ...
    extra = [
        _row(close_reason="time_stop", trade_r=-0.1, pnl=-5.0, entry_bar_index=200 + i, bars_held=8) for i in range(75)
    ]
    early_rows = holes + winners + extra
    import json as _json

    early_a.write_text("".join(_json.dumps(r) + "\n" for r in early_rows), encoding="utf-8")
    ...
    def _eval_leg(**kwargs: Any) -> SimpleNamespace:
        ...
        name = (
            "path_shape_k3_dead_A_close_ledger.jsonl" if seed == 20260902 else "path_shape_k3_dead_B_close_ledger.jsonl"
        )
        path = reports / "artifacts" / name
        path.parent.mkdir(parents=True, exist_ok=True)
        holes_s = [_row() for _ in range(50)]
        winners_s = [_row(close_reason="target", trade_r=1.21, pnl=60.0) for _ in range(30)]
        exits = [
            _row(
                close_reason="force_exit",
                trade_r=-0.20,
        ...
        extra_s = [_row(close_reason="time_stop", trade_r=-0.1, pnl=-5.0) for _ in range(50)]
        path.write_text("".join(_json.dumps(r) + "\n" for r in holes_s + winners_s + exits + extra_s), encoding="utf-8")
        return SimpleNamespace(optimizer_steps=0, trajectories=[], rollout_steps=0)

    import lumina_core.birth.awakening_path_shape_k3_dead_eval as eval_mod
    ...
    monkeypatch.setattr(eval_mod, "run_path_shape_k3_dead_eval_leg", _eval_leg)
    live = run_path_shape_k3_dead(reports=tmp_path, workspace_a=tmp_path, workspace_b=tmp_path)
    assert live["parent_loaded"] is True
    assert live["replay_ran"] is True
    assert live["A"]["t0"]["n_policy"] == 150
    assert live["A"]["mean_stamped_shape"] == "DEAD"
    src = Path("lumina_core/birth/awakening_path_shape_k3_dead_eval.py").read_text(encoding="utf-8")
    assert "PATH_SHAPE_K3_SHADOW.set" in src
    assert SOURCE in src
    monkeypatch.setattr(eval_mod, "run_path_shape_k3_dead_eval_leg", lambda **_k: SimpleNamespace(optimizer_steps=1))
    with pytest.raises(PathShapeK3DeadProtocolError, match="optimizer_steps"):
        run_path_shape_k3_dead(reports=tmp_path, workspace_a=tmp_path, workspace_b=tmp_path)


def test_coverage_gate0_incomplete_skips_gate2(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    import lumina_core.birth.awakening_path_shape_k3_dead_run as run_mod
    from lumina_core.birth.awakening_path_shape_k3_dead_run import run_path_shape_k3_dead

    monkeypatch.setattr(
        run_mod,
        "inspect_path_shape_k3_dead_protocol",
        lambda: {"gate0_complete": False, "missing_sites": ["eps_sit"]},
    )
    out = run_path_shape_k3_dead(reports=tmp_path, skip_replay=True)
    assert out["replay_ran"] is False
    assert (tmp_path / "AWAKENING_PATH_SHAPE_K3_DEAD_AUDIT.md").is_file()
...
def test_coverage_overall_and_main(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert (
        overall_path_shape_k3_dead_string(replay_ran=True, optimizer_steps=1, gate2_attempted=True)
        == OVERALL_INCONCLUSIVE
    )
    assert (
        overall_path_shape_k3_dead_string(replay_ran=True, s_missing_hook=True, gate2_attempted=True)
        == OVERALL_INCONCLUSIVE
    )
    assert (
        overall_path_shape_k3_dead_string(replay_ran=True, tfamily_clone=True, gate2_attempted=True)
        == OVERALL_INCONCLUSIVE
    )
...
def test_coverage_assert_n_exit_clone() -> None:
    ...
    with pytest.raises(PathShapeK3DeadProtocolError, match="DEAD"):
        assert_n_exit_not_tfamily_clone(
            n_exit_a=20,
            exits_a=[{"path_exit_k3": True}],
            mean_stamped_threshold_a=None,
        )
