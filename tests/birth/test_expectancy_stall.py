...
def test_stage2_edgescore_uses_rolling_wr_for_expectancy() -> None:
    cfg = BirthCurriculumConfig(stage2_expectancy_floor=-0.15, stage2_edgescore_enabled=True)
    # Lifetime 29% WR → exp -0.21 fails; rolling 40% → exp -0.10 passes.
    ...
    assert edge_roll.expectancy_ok is True
    assert edge_roll.passed is True
