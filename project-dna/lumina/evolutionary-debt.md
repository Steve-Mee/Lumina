...
| **H5** | Secure self-code evolution beyond evaluate-only | **Champion/challenger ladder (ADR-0045)**: K1–K16 are the Done gate (waves are build order only). OverlayPort + internal paper venue + council/cutover. Never live repo / REAL auto-apply. Defaults remain fail-closed OFF. |
...
| **H7** | Birth exit vs maturation ADR | **Done (2026-08)**: ADR-0036 refined by **ADR-0046** — Birth exit = five `foundation_v2` receipts + fitness vector (`is_birth_exit_sufficient`); artifacts/cert/WR 20/35/40 cannot exit; API `/api/maturity/birth-exit`; floors in `docs/birth-curriculum-stage-floors.md` |
