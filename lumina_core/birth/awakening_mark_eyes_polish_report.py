"""AUDIT / VERDICT markdown for AWAKENING_MARK_EYES_POLISH."""
...
from lumina_core.birth.awakening_mark_eyes_polish_tables import HONESTY_PARAGRAPH
...
def render_verdict(*, flags: dict[str, Any], t2_a: dict[str, Any], t2_b: dict[str, Any]) -> str:
    return "\n".join(
        [
            "# AWAKENING_MARK_EYES_POLISH_VERDICT",
            "",
            f"**tag:** `{flags.get('tag')}`",
            f"**law:** `{flags.get('law')}`",
            ...
            f"**init_sha256:** `{str(flags.get('init_sha256') or '')[:16]}`",
