"""G4/G6 evaluate-only: frozen a9ffa852 then scratch V1 child on THIS coupling tape A/B."""
...
from lumina_core.birth.awakening_coupling_diagnose import BASELINE_ZIP_NAME, CHILD_ZIP_NAME, SOURCE, CouplingProtocolError
...
from lumina_core.birth.awakening_physics_tape import PHYSICS_HOLD_PCT
...
LEDGER_NAMES = {
    ("base", "A"): "coupling_base_A_close_ledger.jsonl",
    ("base", "B"): "coupling_base_B_close_ledger.jsonl",
    ("child", "A"): "coupling_child_A_close_ledger.jsonl",
    ("child", "B"): "coupling_child_B_close_ledger.jsonl",
}
...
def _assert_eval_ready(leg: str, zip_path: Path, kind: str) -> None:
    if TRAIN:
        raise CouplingProtocolError("TRAIN must stay False")
    if str(leg) not in {"A", "B"}:
        raise CouplingProtocolError("seeds recorded as labels A/B only")
    if zip_path.name == "awakening_mark_eyes_v2_pi_star.zip":
        raise CouplingProtocolError("used_v2_child is forbidden")
    allowed = {BASELINE_ZIP_NAME} if kind == "base" else {CHILD_ZIP_NAME}
    if zip_path.name not in allowed:
        raise CouplingProtocolError(f"refused PPO.load of non-coupling zip {zip_path.name}")
    if bool(PATH_EXIT_K3_SHADOW.get()) or bool(PATH_SHAPE_K3_SHADOW.get()):
        raise CouplingProtocolError("path_exit / path_shape hooks must stay False")


def eval_coupling_leg(
    *,
...
def run_coupling_eval(
    *,
    ...
    if TRAIN:
        raise CouplingProtocolError("TRAIN flag False")
    if kind not in {"base", "child"}:
        raise CouplingProtocolError("kind must be base or child")
    ...
    book_a = eval_coupling_leg(holdout=leg_a, work=work, art=art, zip_path=zip_path, kind=kind, leg="A")
    book_b = eval_coupling_leg(holdout=leg_b, work=work, art=art, zip_path=zip_path, kind=kind, leg="B")
...
__all__ = ["eval_coupling_leg", "organism_stats", "policy_obs_dim", "run_coupling_eval"]
