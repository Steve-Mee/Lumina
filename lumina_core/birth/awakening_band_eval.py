"""G4/G6 evaluate-only: frozen a9ffa852 then scratch V1 child. FORCE_OPEN off."""
...
from lumina_core.birth.awakening_band_flags import SOURCE
from lumina_core.birth.awakening_band_tape import BASELINE_ZIP_NAME, CHILD_ZIP_NAME, BandProtocolError
...
LEDGER_NAMES = {
    ("base", "A"): "band_base_A_close_ledger.jsonl",
    ("base", "B"): "band_base_B_close_ledger.jsonl",
    ("child", "A"): "band_child_A_close_ledger.jsonl",
    ("child", "B"): "band_child_B_close_ledger.jsonl",
}
...
def _assert_eval_ready(leg: str, zip_path: Path, kind: str) -> None:
    if TRAIN:
        raise BandProtocolError("TRAIN must stay False")
    if str(leg) not in {"A", "B"}:
        raise BandProtocolError("seeds recorded as labels A/B only")
    if zip_path.name == "awakening_mark_eyes_v2_pi_star.zip":
        raise BandProtocolError("used_v2_child is forbidden")
    allowed = {BASELINE_ZIP_NAME} if kind == "base" else {CHILD_ZIP_NAME}
    if zip_path.name not in allowed:
        raise BandProtocolError(f"refused PPO.load of non-band zip {zip_path.name}")
    if bool(PATH_EXIT_K3_SHADOW.get()) or bool(PATH_SHAPE_K3_SHADOW.get()):
        raise BandProtocolError("path_exit / path_shape hooks must stay False")
...
def eval_band_leg(
    *,
    ...
    stats.update({"S_MISSING": False, "ledger": str(ledger), "n_rows": len(rows), "obs_dim": dim, "train": bool(TRAIN), "eval_force_open": False})
    return stats
...
def run_band_eval(
    *,
    ...
    if TRAIN:
        raise BandProtocolError("TRAIN flag False")
    if kind not in {"base", "child"}:
        raise BandProtocolError("kind must be base or child")
    ...
    book_a = eval_band_leg(holdout=leg_a, work=work, art=art, zip_path=zip_path, kind=kind, leg="A")
    book_b = eval_band_leg(holdout=leg_b, work=work, art=art, zip_path=zip_path, kind=kind, leg="B")
...
__all__ = ["eval_band_leg", "run_band_eval"]
