"""G3: scratch V1 PPO.learn() of 10_000 env steps. Geometry close reward. FORCE_OPEN train-only."""
...
logger = get_logger("lumina.birth.awakening_geom_train")


def should_learn(*, unhittable: bool, n_policy_a: int, n_policy_b: int) -> bool:
    if bool(unhittable):  # learn skipped when unhittable
        return False
    floor = int(POLICY_EDGE_MIN_TRADES)
    if int(n_policy_a) < floor or int(n_policy_b) < floor:
        return False
    return True


def pin_train_seed(seed: int) -> None:
    if int(seed) not in GEOM_SEEDS:
        raise GeomProtocolError(f"train seed {seed} not in {GEOM_SEEDS}")
...
def _fail(**kwargs: Any) -> dict[str, Any]:
    out = {
        ...
        "use_geom_close_reward": True,
    }
    out.update(kwargs)
    return out


def run_geom_v1_train(
    *,
    work: Path,
    art: Path,
    train_seed: int,
    ...
    if init_zip is not None:
        ...
        raise GeomProtocolError("init_policy must be scratch")
    if int(timesteps) != int(GEOM_TIMESTEPS):
        raise GeomProtocolError(f"timesteps {timesteps} != {GEOM_TIMESTEPS}")
    if bool(PATH_EXIT_K3_SHADOW.get()) or bool(PATH_SHAPE_K3_SHADOW.get()):
        raise GeomProtocolError("hooks must stay False")
    pin_train_seed(int(train_seed))
    tape = load_geom_train_split(work)
    train = list(tape["train"])
    if not train:
        raise GeomProtocolError("TRAIN split empty — refuse holdout ticks")
    ...
    if not bool(env.env.envelope.get("participation_envelope_enabled")):
        raise GeomProtocolError("FORCE_OPEN train envelope failed to arm")
    if _obs_shape(env) != (int(MARK_EYES_OBS_DIM),):
        raise GeomProtocolError(f"env observation space {_obs_shape(env)} != (46,)")
    try:
        cls = ppo_cls
        if cls is None:
    ...
    except GeomProtocolError:
        raise
    except Exception as exc:
        logger.error("awakening.geom.sb3_missing: %s", exc)
    ...
    try:
        ...
        logger.error("awakening.geom.learn_failed: %s", exc)
        return _fail(error=f"S_MISSING: learn() {exc}")
    actual = int(getattr(model, "num_timesteps", 0) or 0)
    if actual <= 0:
        return _fail(learn_called=True, error="S_MISSING: actual_timesteps == 0 — do not relabel a9ffa852 as geom child")
...
__all__ = ["pin_train_seed", "run_geom_v1_train", "should_learn"]
