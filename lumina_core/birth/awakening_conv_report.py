"""AUDIT / VERDICT markdown for AWAKENING_ENRICHER_CONVERSION."""
...
from lumina_core.birth.awakening_conv_tables import HONESTY_PARAGRAPH
...
def render_verdict(*, flags: dict[str, Any], t2_a: dict[str, Any], t2_b: dict[str, Any]) -> str:
    return "\n".join(
        [
            "# AWAKENING_ENRICHER_CONVERSION_VERDICT",
            "",
            f"**tag:** `{flags.get('tag')}`",
            f"**law:** `{flags.get('law')}`",
            ...
            f"**prod_enricher_default_changed:** `{flags.get('prod_enricher_default_changed')}`",
