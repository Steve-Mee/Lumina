...
from lumina_core.birth.awakening_drift_flags import SOURCE
from lumina_core.birth.awakening_drift_tape import BASELINE_ZIP_NAME, CHILD_ZIP_NAME, DriftProtocolError
...
LEDGER_NAMES = {
    ("base", "A"): "drift_base_A_close_ledger.jsonl",
    ("base", "B"): "drift_base_B_close_ledger.jsonl",
    ("child", "A"): "drift_child_A_close_ledger.jsonl",
    ("child", "B"): "drift_child_B_close_ledger.jsonl",
}
...
def _assert_eval_ready(leg: str, zip_path: Path, kind: str) -> None:
    if TRAIN:
        raise DriftProtocolError("TRAIN must stay False")
    if str(leg) not in {"A", "B"}:
        raise DriftProtocolError("seeds recorded as labels A/B only")
    if zip_path.name == "awakening_mark_eyes_v2_pi_star.zip":
        raise DriftProtocolError("used_v2_child is forbidden")
    allowed = {BASELINE_ZIP_NAME} if kind == "base" else {CHILD_ZIP_NAME}
    if zip_path.name not in allowed:
        raise DriftProtocolError(f"refused PPO.load of non-drift zip {zip_path.name}")
    if bool(PATH_EXIT_K3_SHADOW.get()) or bool(PATH_SHAPE_K3_SHADOW.get()):
        raise DriftProtocolError("path_exit / path_shape hooks must stay False")
...
def run_drift_eval(
    *,
    ...
    if TRAIN:
        raise DriftProtocolError("TRAIN flag False")
    if kind not in {"base", "child"}:
        raise DriftProtocolError("kind must be base or child")
    ...
    book_a = eval_drift_leg(holdout=leg_a, work=work, art=art, zip_path=zip_path, kind=kind, leg="A")
    book_b = eval_drift_leg(holdout=leg_b, work=work, art=art, zip_path=zip_path, kind=kind, leg="B")
...
__all__ = ["eval_drift_leg", "run_drift_eval"]
