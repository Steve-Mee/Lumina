"""CurriculumStageHandler (single responsibility).

The procedural orchestration logic previously in stage_training_loop.py
has been moved here. This handler owns stage execution and emits/receives
events on the central bus for coordination with CurriculumOrchestrator,
plateau_handler, phoenix_handler, etc.

All cross context signaling uses the EventBus. Constitution violations are fail-closed.
"""
...
logger = get_logger("lumina.birth.curriculum_stage_handler")
...
class CurriculumStageHandler:
    """Dedicated single-responsibility handler for stage execution.

    It subscribes to stage requests, runs the (moved) stage logic using
    the focused submodules (curriculum, plateau, phoenix, remediation, intra),
    and emits strict typed events on completion or abort.
    """
    ...
    def _on_stage_requested(self, event: DomainEvent) -> None:
        """Light handler: announce start, rely on shim + dedicated modules for execution.

        The heavy procedural logic (moved from stage_training_loop) is invoked
        via the re-exported run_stage_research_loop called from the compat layer
        or directly. This method ensures events are emitted for the orchestrator.
        """
        try:
        ...
        # Do not re-execute heavy logic here (would double-run with shim).
        # The executor (run_stage_research_loop) is responsible for the work
        # and should publish COMPLETED or ABORTED (or the caller can do it).
        # For now we emit a best-effort completed after the fact via other paths.

    def _resolve_host(self) -> Any | None:
        if callable(self.host_factory):
            try:
...
def create_and_attach_stage_handler(bus: EventBus, host: Any) -> CurriculumStageHandler:
    h = CurriculumStageHandler(bus, host_factory=host)
    h.attach()
    return h


# The run_stage_research_loop symbol is provided by the appended source below.
# It is the moved logic; orchestration boundaries now also emit via the handler.
...
# Sync patchable names from the thin shim for test compatibility
try:
    import lumina_core.birth.stage_training_loop as _shim_mod
    if hasattr(_shim_mod, 'run_policy_rollout'):
        run_policy_rollout = _shim_mod.run_policy_rollout
    if hasattr(_shim_mod, 'mine_winning_patterns'):
        mine_winning_patterns = _shim_mod.mine_winning_patterns
    if hasattr(_shim_mod, 'expand_birth_data'):
        expand_birth_data = _shim_mod.expand_birth_data
except Exception:
    pass

# Patch support (no late import statement to satisfy E402)
__import__('sys').modules[__name__ + '.time'] = __import__('time')
