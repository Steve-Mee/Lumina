"""G2: NEW 2/2/2 occupancy tape. Seed 20260912. Isolated enrich only. Split imported."""
...
from lumina_core.birth.awakening_conv_flags import BASELINE_SHA256, CONV_PHASE_BLOCKS, CONV_SEED
...
CONV_START_ET = datetime(2025, 12, 1, 18, 0, tzinfo=ET)  # seed 20260912
CONV_START_ET_ISO = "2025-12-01T18:00:00-05:00"
CONV_DAYS = 90
CONV_RTH_SEC = 10
CONV_ETH_SEC = 60
CONV_DRIFT_RTH = 0.00024
CONV_RANGE_KAPPA = 0.01
CONV_HOLD_INSIDE_BLOCK = 0.40
CONV_TIMESTEPS = 10_000
TRAIN_SEED = 20260912
...
CHILD_ZIP_NAME = "awakening_conv_v1_pi_star.zip"
CHILD_META_NAME = "awakening_conv_v1_pi_star.json"
CHILD_SCHEMA = "awakening_conv_v1_pi_star_v1"
FLAGS_NAME = "awakening_conv_flags.json"
CONV_ROOT = REPO_ROOT / "reports" / "awakening_conv_run"
CONV_WORK, CONV_ART = CONV_ROOT / "workspace", CONV_ROOT / "artifacts"
...
assert CONV_DRIFT_RTH == 0.00024 == OCCUPANCY_DRIFT_RTH
assert CONV_RANGE_KAPPA == 0.01 == OCCUPANCY_RANGE_KAPPA
assert CONV_HOLD_INSIDE_BLOCK == 0.40 == STRAT_HOLD_PCT
assert CONV_PHASE_BLOCKS == 6 and CONV_SEED == 20260912
...
def refuse_this_tape_hash(sha: str) -> str:
    ...
    if text.startswith(FORBIDDEN_TAPE_PREFIXES):
        raise ConvProtocolError(f"refused old tape hash {text[:16]} as THIS exam tape")
    return text
...
def assert_forbidden_init(path: Path | str, sha: str = "") -> Path:
    ...
    if text[:8] in FORBIDDEN_INIT_SHA16:
        raise ConvProtocolError(f"refused forbidden init sha {text[:8]}")
    if target.name in FORBIDDEN_INIT_NAMES or is_gitignored_ppo_zip(target) or "/lumina_agents/ppo/" in f"/{posix}":
        raise ConvProtocolError(f"refused forbidden init {target.name}")
    return target


def generate_conv_tape_ticks() -> tuple[list[dict[str, Any]], list[str], dict[str, int]]:
    stamps = _iter_session_times(
        start_et=CONV_START_ET,
        calendar_days=CONV_DAYS,
        rth_bar_seconds=CONV_RTH_SEC,
        eth_bar_seconds=CONV_ETH_SEC,
    )
    if len(stamps) < 1_000:
        raise ConvProtocolError(f"conv fixture too thin: {len(stamps)}")
    rng = np.random.default_rng(CONV_SEED)
    labels = generator_labels(len(stamps), CONV_PHASE_BLOCKS)
    ...
    # no oracle regime — price must earn TREND_* via isolated wrapper
    ...
    if any("regime" in row or "gen_phase" in row for row in ticks):
        raise ConvProtocolError("no oracle regime")
...
def persist_conv_fixture(work: Path, art: Path) -> dict[str, Any]:
    raw, phases, gen_counts = generate_conv_tape_ticks()
    ...
    if any("gen_phase" in row for row in enriched):
        raise ConvProtocolError("generator phase side-channel must be stripped before persist")
    ...
    paths = save_birth_data_cache(
        ...
        train_hash=t_hash,
        enrich_version=ENRICH_VERSION,
        requested_days=max(FOUNDATION_HISTORY_START_DAYS, CONV_DAYS),
    ...
    payload = {
        ...
        "fixture_seed": CONV_SEED,
        "start_et": CONV_START_ET_ISO,
...
def inspect_conv_protocol() -> dict[str, Any]:
    sites = {
        "physics_slope_abs_012": ("lumina_core/birth/awakening_conv_enrich.py", "PHYSICS_SLOPE_ABS = 0.12"),
        "prod_default_015": ("lumina_core/birth/awakening_conv_enrich.py", "production default still 0.15"),
        "exam_seed_20260912": ("lumina_core/birth/awakening_conv_tape.py", "seed 20260912"),
        "per_phase_60_40_import": ("lumina_core/birth/awakening_conv_tape.py", "per-phase 60/40 import"),
        "fracs_25_25": ("lumina_core/birth/awakening_conv_tape.py", "25/25 train AND holdout"),
        "no_oracle_regime": ("lumina_core/birth/awakening_conv_tape.py", "no oracle regime"),
        "body_skipped": ("lumina_core/birth/awakening_conv_run.py", "body skipped when not world_ok"),
        "floor_150": ("lumina_core/birth/foundation_metrics.py", "POLICY_EDGE_MIN_TRADES = 150"),
        "both_leg_license": ("lumina_core/birth/awakening_conv_flags.py", "both-leg license"),
        "genesis_eyes_ok_false": ("lumina_core/birth/awakening_conv_flags.py", "GENESIS_EYES_OK false"),
    }
