"""PATH_EXIT K3 T025 evaluate-only replay. Hook ON + T_FP here only. No learn()."""
...
from lumina_core.birth.awakening_path_exit_k3_t025_tables import table_t2, table_t4
...
logger = get_logger("lumina.birth.awakening_path_exit_k3_t025_eval")
...
def run_path_exit_k3_t025_eval_leg(
    *,
    seed: int,
    ...
    ledger = assert_isolated_write(
        path_exit_k3_t025_ledger_path(reports, leg="A" if seed == EVAL_A_SEED else "B")
    )
    loaded = load_frozen_policy(policy_path)
    if loaded is None:
        raise PathExitK3T025ProtocolError(f"parent policy unloadable: {policy_path}")
...
def _replay_path_exit_k3_t025_armed(
    *,
    reports_path: Path,
    proto: dict[str, Any],
    workspace_a: Path,
    ...
    base_a, _rows_base_a = _load_baseline(artifacts, leg="A")
    base_b, _rows_base_b = _load_baseline(artifacts, leg="B")
    ...
    metrics_a = run_path_exit_k3_t025_eval_leg(
        seed=EVAL_A_SEED,
    ...
    if int(getattr(metrics_a, "optimizer_steps", 0) or 0) != 0:
        raise PathExitK3T025ProtocolError("optimizer_steps must stay 0")
    path_a = assert_isolated_write(path_exit_k3_t025_ledger_path(reports_path, leg="A"))
    ...
    metrics_b = run_path_exit_k3_t025_eval_leg(
        seed=EVAL_B_SEED,
    ...
    if int(getattr(metrics_b, "optimizer_steps", 0) or 0) != 0:
        raise PathExitK3T025ProtocolError("optimizer_steps must stay 0")
    path_b = assert_isolated_write(path_exit_k3_t025_ledger_path(reports_path, leg="B"))
    ...
    logger.info(
        "awakening.path_exit_k3_t025.replay overall=%s tag=%s n_exit_a=%s n_exit_b=%s",
        overall,
        licensed.get("tag"),
        payload_a["t0"]["n_exit"],
        payload_b["t0"]["n_exit"],
...
def replay_path_exit_k3_t025(
    *,
    ...
    try:
        return _replay_path_exit_k3_t025_armed(
            reports_path=reports_path,
            proto=proto,
...
__all__ = [
    ...
    "run_path_exit_k3_t025_eval_leg",
    "write_inconclusive",
    "write_jsonl_sha256",
]
