"""G5: one scratch V1 PPO.learn() of 10_000 env steps. Train seed 20260911. TRAIN only."""
...
from lumina_core.birth.awakening_strat_split import StratProtocolError
from lumina_core.birth.awakening_strat_tape import (
    ...
    load_strat_train_split,
)
from lumina_core.logging_utils import get_logger
from lumina_core.rl.ppo_trainer import PPOTrainer

logger = get_logger("lumina.birth.awakening_strat_train")


def pin_train_seed(seed: int) -> None:
    if int(seed) != int(TRAIN_SEED):
        raise StratProtocolError(f"train seed {seed} != {TRAIN_SEED}")
...
def run_strat_v1_train(
    *,
    work: Path,
    art: Path,
    init_zip: Path | str | None = None,
    timesteps: int = STRAT_TIMESTEPS,
    learn_fn: Any | None = None,
    ppo_cls: Any | None = None,
) -> dict[str, Any]:
    if init_zip is not None:
        assert_forbidden_init(init_zip)
        raise StratProtocolError("init_policy must be scratch")
    if int(timesteps) != int(STRAT_TIMESTEPS):
        raise StratProtocolError(f"timesteps {timesteps} != {STRAT_TIMESTEPS}")
    if bool(PATH_EXIT_K3_SHADOW.get()) or bool(PATH_SHAPE_K3_SHADOW.get()):
        raise StratProtocolError("hooks must stay False")
    pin_train_seed(TRAIN_SEED)
    tape = load_strat_train_split(work)
    train = list(tape["train"])
    if not train:
        raise StratProtocolError("TRAIN split empty — refuse holdout ticks")
    ...
    if _obs_shape(env) != (int(MARK_EYES_OBS_DIM),):
        raise StratProtocolError(f"env observation space {_obs_shape(env)} != (46,)")
    try:
        cls = ppo_cls
        if cls is None:
    ...
    except StratProtocolError:
        raise
    except Exception as exc:
        logger.error("awakening.strat.sb3_missing: %s", exc)
        return {
            "status": "S_MISSING",
            "learn_called": False,
            "actual_timesteps": 0,
    ...
    try:
        ...
        logger.error("awakening.strat.learn_failed: %s", exc)
        return {
            "status": "S_MISSING",
            "learn_called": False,
            "actual_timesteps": 0,
    ...
    if actual <= 0:
        return {
            ...
            "child_sha256": "",
            "init_policy": "scratch",
            "error": "S_MISSING: actual_timesteps == 0 — do not relabel a9ffa852 as strat child",
        }
...
__all__ = ["pin_train_seed", "run_strat_v1_train"]
