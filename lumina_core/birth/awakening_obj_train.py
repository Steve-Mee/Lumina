"""G3: one scratch V1 PPO.learn() of 10_000 env steps. FORCE_OPEN train-only."""
...
logger = get_logger("lumina.birth.awakening_obj_train")


def pin_train_seed(seed: int) -> None:
    if int(seed) != int(TRAIN_SEED):
        raise ObjProtocolError(f"train seed {seed} != {TRAIN_SEED}")
...
def run_obj_v1_train(
    *,
    work: Path,
    art: Path,
    init_zip: Path | str | None = None,
    timesteps: int = OBJ_TIMESTEPS,
    learn_fn: Any | None = None,
    ppo_cls: Any | None = None,
) -> dict[str, Any]:
    if init_zip is not None:
        assert_forbidden_init(init_zip)
        raise ObjProtocolError("init_policy must be scratch")
    if int(timesteps) != int(OBJ_TIMESTEPS):
        raise ObjProtocolError(f"timesteps {timesteps} != {OBJ_TIMESTEPS}")
    if bool(PATH_EXIT_K3_SHADOW.get()) or bool(PATH_SHAPE_K3_SHADOW.get()):
        raise ObjProtocolError("hooks must stay False")
    pin_train_seed(TRAIN_SEED)
    tape = load_obj_train_split(work)
    train = list(tape["train"])
    if not train:
        raise ObjProtocolError("TRAIN split empty — refuse holdout ticks")
    ...
    if not bool(env.env.envelope.get("participation_envelope_enabled")):
        raise ObjProtocolError("FORCE_OPEN train envelope failed to arm")
    if _obs_shape(env) != (int(MARK_EYES_OBS_DIM),):
        raise ObjProtocolError(f"env observation space {_obs_shape(env)} != (46,)")
    try:
        cls = ppo_cls
        if cls is None:
    ...
    except ObjProtocolError:
        raise
    except Exception as exc:
        logger.error("awakening.obj.sb3_missing: %s", exc)
        return {
            "status": "S_MISSING",
            "learn_called": False,
            "actual_timesteps": 0,
    ...
    try:
        ...
        logger.error("awakening.obj.learn_failed: %s", exc)
        return {
            "status": "S_MISSING",
            "learn_called": False,
            "actual_timesteps": 0,
    ...
    if actual <= 0:
        return {
            ...
            "train_force_open": True,
            "eval_force_open": False,
            "error": "S_MISSING: actual_timesteps == 0 — do not relabel a9ffa852 as obj child",
        }
...
__all__ = ["pin_train_seed", "run_obj_v1_train"]
