"""G4/G6 evaluate-only: frozen a9ffa852 then scratch V1 child on THIS stratified tape A/B."""
...
from lumina_core.birth.awakening_strat_split import STRAT_HOLD_PCT, StratProtocolError
from lumina_core.birth.awakening_strat_tape import BASELINE_ZIP_NAME, CHILD_ZIP_NAME, SOURCE
...
LEDGER_NAMES = {
    ("base", "A"): "strat_base_A_close_ledger.jsonl",
    ("base", "B"): "strat_base_B_close_ledger.jsonl",
    ("child", "A"): "strat_child_A_close_ledger.jsonl",
    ("child", "B"): "strat_child_B_close_ledger.jsonl",
}
...
def _assert_eval_ready(leg: str, zip_path: Path, kind: str) -> None:
    if TRAIN:
        raise StratProtocolError("TRAIN must stay False")
    if str(leg) not in {"A", "B"}:
        raise StratProtocolError("seeds recorded as labels A/B only")
    if zip_path.name == "awakening_mark_eyes_v2_pi_star.zip":
        raise StratProtocolError("used_v2_child is forbidden")
    allowed = {BASELINE_ZIP_NAME} if kind == "base" else {CHILD_ZIP_NAME}
    if zip_path.name not in allowed:
        raise StratProtocolError(f"refused PPO.load of non-strat zip {zip_path.name}")
    if bool(PATH_EXIT_K3_SHADOW.get()) or bool(PATH_SHAPE_K3_SHADOW.get()):
        raise StratProtocolError("path_exit / path_shape hooks must stay False")
...
def run_strat_eval(
    *,
    ...
    if TRAIN:
        raise StratProtocolError("TRAIN flag False")
    if kind not in {"base", "child"}:
        raise StratProtocolError("kind must be base or child")
    ...
    book_a = eval_strat_leg(holdout=leg_a, work=work, art=art, zip_path=zip_path, kind=kind, leg="A")
    book_b = eval_strat_leg(holdout=leg_b, work=work, art=art, zip_path=zip_path, kind=kind, leg="B")
...
__all__ = ["eval_strat_leg", "organism_stats", "policy_obs_dim", "run_strat_eval"]
