"""Evaluate-only Awakening PATH_EARLY autopsy: parent π* on A then B.
...
logger = get_logger("lumina.birth.awakening_path_early_run")
...
def run_path_early_eval_leg(
    *,
    ...
    if TRAIN:
        raise RuntimeError("path early TRAIN must stay False")
    assert_eval_seed(int(seed))
    ledger = assert_isolated_write(
        path_early_ledger_path(reports, leg="A" if seed == EVAL_A_SEED else "B")
    )
    loaded = load_frozen_policy(policy_path)
    if loaded is None:
        raise PathEarlyProtocolError(f"parent policy unloadable: {policy_path}")
...
def _empty_leg(zip_sha: str, *, skip_replay: bool) -> dict[str, Any]:
    return {
        "t0": table_t0(
            [],
...
def run_path_early(
    *,
    ...
    proto = inspect_path_early_protocol()
    if not proto.get("gate0_complete"):
        raise PathEarlyProtocolError(f"Gate 0 incomplete: {proto.get('missing_sites')}")
    ...
    try:
        ...
        parent_loaded = True
    except PathEarlyProtocolError:
        zip_sha = ""
        parent_loaded = False
    overall = overall_path_early_string(
    ...
    empty_leg = _empty_leg(zip_sha, skip_replay=bool(skip_replay))
    ...
    metrics_a = run_path_early_eval_leg(
        seed=EVAL_A_SEED,
    ...
    if int(getattr(metrics_a, "optimizer_steps", 0) or 0) != 0:
        raise PathEarlyProtocolError("optimizer_steps must stay 0")
    path_a = assert_isolated_write(path_early_ledger_path(reports_path, leg="A"))
    ...
    metrics_b = run_path_early_eval_leg(
        seed=EVAL_B_SEED,
    ...
    if int(getattr(metrics_b, "optimizer_steps", 0) or 0) != 0:
        raise PathEarlyProtocolError("optimizer_steps must stay 0")
    path_b = assert_isolated_write(path_early_ledger_path(reports_path, leg="B"))
    ...
    logger.info(
        "awakening.path_early.done overall=%s tag=%s n_a=%s n_b=%s",
        overall,
