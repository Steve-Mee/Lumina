"""G6 license for AWAKENING_PRICE_BAND. Floor 150. Both-leg. Guard intact."""

from __future__ import annotations

from typing import Any

from lumina_core.birth.awakening_band_tape import BASELINE_SHA256, NQ_MAX, NQ_MIN
...
TAG_BAND_OK = "BAND_OK"
TAG_BAND_BODY = "BAND_BODY"
TAG_BAND_THIN = "BAND_THIN"
TAG_BAND_HARM = "BAND_HARM"
TAG_BAND_WORLD_FAIL = "BAND_WORLD_FAIL"
...
def empty_band_flags() -> dict[str, Any]:
    return {
        "source": SOURCE,
        "nq_min": float(NQ_MIN),
        "nq_max": float(NQ_MAX),
...
def license_band(
    ...
    *,
    ...
    if world_fail:
        tag, law, family = TAG_BAND_WORLD_FAIL, LAW_NONE, FAMILY_H_NONE
    elif miss:
        tag, law, family = TAG_S_MISSING, LAW_NONE, FAMILY_H_NONE
    elif harm:
        tag, law, family = TAG_BAND_HARM, LAW_NONE, FAMILY_H_NONE
    elif child_thin:
        tag, law, family = TAG_BAND_THIN, LAW_NONE, FAMILY_H_NONE
    elif base_thin and child_thick and not (moved_a and moved_b):
        tag, law, family = TAG_BAND_BODY, LAW_NONE, FAMILY_H_NONE
    elif moved_a and moved_b:
        tag, law, family = TAG_BAND_OK, LAW_SHADOW, FAMILY  # both-leg license
    else:
        tag, law, family = TAG_BAND_BODY, LAW_NONE, FAMILY_H_NONE
    return {
        "tag": tag,
...
def compose_band_flags(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if str(flags.get("tag")) == TAG_BAND_OK and not (bool(flags.get("MOVED_A")) and bool(flags.get("MOVED_B"))):
        flags["tag"] = TAG_BAND_BODY
        flags["law"] = LAW_NONE
