"""OPEN_POLICY_SIGNAL audit / verdict / flags writers. Measure-only, no train law."""
...
def _forbidden_grep() -> dict[str, Any]:
    ...
    signal_src = (birth / "awakening_open_policy_signal.py").read_text(encoding="utf-8")
    run_src = (birth / "awakening_open_policy_signal_run.py").read_text(encoding="utf-8")
    envelope_fn = "decide_stage2" + "_participation("
    controller = envelope_fn in signal_src or envelope_fn in run_src
    return {
...
def write_open_policy_signal_reports(
    *,
    reports: Path,
    overall: str,
    zip_sha: str,
    ...
    thresholds_a = flags_a.get("thresholds") or {}
    thresholds_b = flags_b.get("thresholds") or {}
    flags_payload = {
        "A": flags_a,
        "B": flags_b,
        "tag": tag,
    ...
    for key, path in POLICY_SIGNAL_STASH_ATTR_PATHS.items():
        a_present = any(
            key in r and r.get(key) is not None
            for r in (payload_a.get("_raw_rows") or [])
        ) if payload_a.get("_raw_rows") else "n/a"
        b_present = any(
            key in r and r.get(key) is not None
            for r in (payload_b.get("_raw_rows") or [])
        ) if payload_b.get("_raw_rows") else "n/a"
        stash_lines.append(f"| `{key}` | `{path}` | {a_present} | {b_present} |")
    fix_a = fixture_a or {}
    fix_b = fixture_b or {}
    audit = [
        ...
        "## Mission",
        "",
        "Among policy trades that OPEN in NEUTRAL, does the frozen π* value-head / entropy / "
        "action-margin knowable at the open bar separate `stop × close NEUTRAL` (hole H) from "
        "+R closes (winners W)?",
        "Measure-only. Gate 1 law NONE. No open-mask. No learn().",
        f"**Date:** {now}",
        f"**Gate 0 (PR #23 land):** `{gate0_sha}`",
        ...
        *stash_lines,
        ...
        "",
        f"- A thresholds: `{json.dumps(thresholds_a, default=str)}`",
        f"- B thresholds: `{json.dumps(thresholds_b, default=str)}`",
        "",
        ...
        "runtime=`select_runtime()`, ledger_source=`awakening_open_policy_signal`, "
        ...
        "- **Autonomy:** measurement compounds; the organism learns whether its own value-head "
        "distinguishes hole from winners at NEUTRAL-open.",
        "- **Experiment:** one variable (policy-internal signal split inside NEUTRAL-open). "
        "External open features (PR #23) stay closed. Close-tax family stays closed. "
        "Blanket NEUTRAL-refuse stays forbidden.",
        "",
    ]
    (reports / "AWAKENING_OPEN_POLICY_SIGNAL_AUDIT.md").write_text(
        "\n".join(audit) + "\n", encoding="utf-8"
    )
    verdict = [
    ...
    (reports / "AWAKENING_OPEN_POLICY_SIGNAL_VERDICT.md").write_text(
        "\n".join(verdict) + "\n", encoding="utf-8"
    )
    block = f"""
---
...
**Landed:** PR #23 on main before replay (record merge SHA or green HEAD). Gate 0 SHA `{gate0_sha}`.
**Leg A** seed 20260902 n_U={t1_a.get("n_U")} n_H={t1_a.get("n_H")} n_W={t1_a.get("n_W")} wr_policy={t0_a.get("wr_policy")} tag={flags_a.get("tag")} winning_P={flags_a.get("winning_P")}.
...
__all__ = ["leg_payload", "write_open_policy_signal_reports"]
