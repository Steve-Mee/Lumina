"""AUDIT / VERDICT markdown for AWAKENING_OBJECTIVE_TRADE."""
...
from lumina_core.birth.awakening_obj_tables import HONESTY_PARAGRAPH
...
def render_verdict(*, flags: dict[str, Any], t2_a: dict[str, Any], t2_b: dict[str, Any]) -> str:
    return "\n".join(
        [
            "# AWAKENING_OBJECTIVE_TRADE_VERDICT",
            "",
            f"**tag:** `{flags.get('tag')}`",
            f"**law:** `{flags.get('law')}`",
