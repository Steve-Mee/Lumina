"""CLI: equal-occupancy tape, then scratch 46-dim V1 10k iff world_ok. License vs G3 a9ffa852."""
...
from lumina_core.birth.awakening_occupancy_eval import run_occupancy_eval
...
from lumina_core.birth.awakening_occupancy_train import run_occupancy_v1_train
...
POINTER = (
    ...
    "Equal UP/DOWN/RANGE generator occupancy + scratch 46-dim V1 10k iff world_ok, "
    "license vs frozen a9ffa852 lives under `reports/awakening_occupancy_run`. "
...
def assert_origin_untouched(before: dict[str, str], *, repo: Path | None = None) -> None:
    ...
    for name, sha in before.items():
        if sha and after.get(name) != sha:
            raise OccupancyProtocolError(f"origin artifact overwritten: {name}")


def copy_baseline_zip(art: Path) -> str:
    art.mkdir(parents=True, exist_ok=True)
    if not ORIGIN_EYES_ZIP.is_file():
        raise OccupancyProtocolError("frozen living MARK_EYES zip missing")
    dest = art / BASELINE_ZIP_NAME
    shutil.copy2(ORIGIN_EYES_ZIP, dest)
    digest = write_bytes_sha(dest)
    if digest != BASELINE_SHA256:
        raise OccupancyProtocolError(f"baseline sha must be a9ffa852 pin, got {digest[:16]}")
    return digest


def prepare_occupancy_trees(*, repo: Path | None = None) -> tuple[Path, Path, Path]:
    root = (repo or REPO_ROOT) / "reports" / "awakening_occupancy_run"
...
def run_awakening_occupancy(*, repo: Path | None = None) -> dict[str, Any]:
    ...
    try:
        ...
        fixture = persist_occupancy_fixture(work, art)
        world_ok = bool(fixture.get("world_ok"))
        if float(fixture.get("real_data_pct") or 0.0) == 100.0:
            raise OccupancyProtocolError("Gate 1 regression: real_data_pct printed 100")
    except OccupancyProtocolError as exc:
        missing = True
        reason = str(exc)
    if body_exam_enabled(world_ok) and not missing:
        try:
            base_eval = run_occupancy_eval(work=work, art=art, zip_path=art / BASELINE_ZIP_NAME, kind="base")
    ...
    if body_exam_enabled(world_ok) and not missing:
        try:
            train_out = run_occupancy_v1_train(work=work, art=art, init_zip=None)
    ...
    if body_exam_enabled(world_ok) and not missing:
        try:
            child_eval = run_occupancy_eval(work=work, art=art, zip_path=art / CHILD_ZIP_NAME, kind="child")
    ...
    if (not world_ok) or (not have_books):
        ...
        leg_a = compute_occupancy_leg(base_a, child_a, missing=missing)
        leg_b = compute_occupancy_leg(base_b, child_b, missing=missing)
    licensed = license_occupancy(leg_a, leg_b, missing=missing, world_ok=world_ok)
    flags = compose_occupancy_flags(
        {
            "phase_blocks": int(fixture.get("phase_blocks_used") or 6),
    ...
    (reports / "AWAKENING_OCCUPANCY_AUDIT.md").write_text(
        render_audit(gate0=gate0, proto=proto, t0=t0, t1=t1, t2_a=t2_a, t2_b=t2_b, t3=t3, flags=flags, g6=g6),
        encoding="utf-8",
    )
    (reports / "AWAKENING_OCCUPANCY_VERDICT.md").write_text(
...
def _append_logs(flags: dict[str, Any], fixture: dict[str, Any]) -> None:
    ...
    gen = fixture.get("gen_counts") or {}
    block = [
        f"## {now} — AWAKENING_OCCUPANCY_BALANCE",
        "",
        "LAW: #41 OTHER = 3/2/1 occupancy. This window forces %3 equal blocks.",
        f"gen counts UP={gen.get('UP', flags.get('gen_up'))} "
        f"DOWN={gen.get('DOWN', flags.get('gen_down'))} "
        f"RANGE={gen.get('RANGE', flags.get('gen_range'))}",
        ...
        f"- hash=`{flags.get('fixture_train_hash')}` seed=`{OCCUPANCY_SEED}` "
    ...
    log_path = OCCUPANCY_ROOT / "LUMINA_OCCUPANCY_EXPERIMENT_LOG.md"
    existing = (
        log_path.read_text(encoding="utf-8")
        if log_path.is_file()
        else "# LUMINA Awakening OCCUPANCY_BALANCE experiment log\n\n"
    )
    ...
    if birth_log.is_file():
        ...
        if "Pointer — AWAKENING_OCCUPANCY_BALANCE" not in text:
            birth_log.write_text(text.rstrip() + POINTER, encoding="utf-8")


def main() -> int:
    flags = run_awakening_occupancy()
