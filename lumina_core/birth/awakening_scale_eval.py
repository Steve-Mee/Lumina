"""G3/G5 evaluate-only: frozen a9ffa852 then scratch V1 child. FORCE_OPEN off."""
...
from lumina_core.birth.awakening_scale_enrich import ScaleProtocolError
from lumina_core.birth.awakening_scale_flags import SOURCE
from lumina_core.birth.awakening_scale_tape import BASELINE_ZIP_NAME, CHILD_ZIP_NAME
...
LEDGER_NAMES = {
    ("base", "A"): "scale_base_A_close_ledger.jsonl",
    ("base", "B"): "scale_base_B_close_ledger.jsonl",
    ("child", "A"): "scale_child_A_close_ledger.jsonl",
    ("child", "B"): "scale_child_B_close_ledger.jsonl",
}
...
def _assert_eval_ready(leg: str, zip_path: Path, kind: str) -> None:
    if TRAIN:
        raise ScaleProtocolError("TRAIN must stay False")
    if str(leg) not in {"A", "B"}:
        raise ScaleProtocolError("seeds recorded as labels A/B only")
    if zip_path.name == "awakening_mark_eyes_v2_pi_star.zip":
        raise ScaleProtocolError("used_v2_child is forbidden")
    allowed = {BASELINE_ZIP_NAME} if kind == "base" else {CHILD_ZIP_NAME}
    if zip_path.name not in allowed:
        raise ScaleProtocolError(f"refused PPO.load of non-scale zip {zip_path.name}")
    if bool(PATH_EXIT_K3_SHADOW.get()) or bool(PATH_SHAPE_K3_SHADOW.get()):
        raise ScaleProtocolError("path_exit / path_shape hooks must stay False")
...
def run_scale_eval(
    *,
    ...
    if TRAIN:
        raise ScaleProtocolError("TRAIN flag False")
    if kind not in {"base", "child"}:
        raise ScaleProtocolError("kind must be base or child")
    ...
    book_a = eval_scale_leg(holdout=leg_a, work=work, art=art, zip_path=zip_path, kind=kind, leg="A")
    book_b = eval_scale_leg(holdout=leg_b, work=work, art=art, zip_path=zip_path, kind=kind, leg="B")
...
__all__ = ["eval_scale_leg", "run_scale_eval"]
