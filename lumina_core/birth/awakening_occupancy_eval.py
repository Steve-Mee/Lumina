"""G3/G5 evaluate-only: frozen a9ffa852 then scratch V1 child on THIS occupancy tape A/B."""
...
LEDGER_NAMES = {
    ("base", "A"): "occ_base_A_close_ledger.jsonl",
    ("base", "B"): "occ_base_B_close_ledger.jsonl",
    ("child", "A"): "occ_child_A_close_ledger.jsonl",
    ("child", "B"): "occ_child_B_close_ledger.jsonl",
}
...
def _assert_eval_ready(leg: str, zip_path: Path, kind: str) -> None:
    if TRAIN:
        raise OccupancyProtocolError("TRAIN must stay False")
    if str(leg) not in {"A", "B"}:
        raise OccupancyProtocolError("seeds recorded as labels A/B only")
    if zip_path.name == "awakening_mark_eyes_v2_pi_star.zip":
        raise OccupancyProtocolError("used_v2_child is forbidden")
    allowed = {BASELINE_ZIP_NAME} if kind == "base" else {CHILD_ZIP_NAME}
    if zip_path.name not in allowed:
        raise OccupancyProtocolError(f"refused PPO.load of non-occupancy zip {zip_path.name}")
    if bool(PATH_EXIT_K3_SHADOW.get()) or bool(PATH_SHAPE_K3_SHADOW.get()):
        raise OccupancyProtocolError("path_exit / path_shape hooks must stay False")


def eval_occupancy_leg(
    *,
...
def run_occupancy_eval(
    *,
    ...
    if TRAIN:
        raise OccupancyProtocolError("TRAIN flag False")
    if kind not in {"base", "child"}:
        raise OccupancyProtocolError("kind must be base or child")
    ...
    book_a = eval_occupancy_leg(holdout=leg_a, work=work, art=art, zip_path=zip_path, kind=kind, leg="A")
    book_b = eval_occupancy_leg(holdout=leg_b, work=work, art=art, zip_path=zip_path, kind=kind, leg="B")
...
__all__ = ["eval_occupancy_leg", "organism_stats", "policy_obs_dim", "run_occupancy_eval"]
