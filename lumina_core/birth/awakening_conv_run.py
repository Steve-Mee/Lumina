"""CLI: isolated |0.12| tape, then scratch 46-dim V1 10k iff world_ok. License vs G4 a9ffa852."""
...
from lumina_core.birth.awakening_conv_enrich import PHYSICS_SLOPE_ABS, PROD_SLOPE_ABS, ConvProtocolError
from lumina_core.birth.awakening_conv_eval import run_conv_eval
...
from lumina_core.birth.awakening_conv_tables import (
    HONESTY_PARAGRAPH,
    table_t0_identity,
    table_t1_honesty,
    table_t2_leg,
...
from lumina_core.birth.awakening_conv_tape import (
    ...
    persist_conv_fixture,
)
from lumina_core.birth.awakening_conv_train import run_conv_v1_train
...
POINTER = (
    ...
    "Isolated |0.12| slope knob + scratch 46-dim V1 10k iff world_ok, "
    "license vs frozen a9ffa852 lives under `reports/awakening_conv_run`. "
...
def assert_origin_untouched(before: dict[str, str], *, repo: Path | None = None) -> None:
    ...
    for name, sha in before.items():
        if sha and after.get(name) != sha:
            raise ConvProtocolError(f"origin artifact overwritten: {name}")


def copy_baseline_zip(art: Path) -> str:
    art.mkdir(parents=True, exist_ok=True)
    if not ORIGIN_EYES_ZIP.is_file():
        raise ConvProtocolError("frozen living MARK_EYES zip missing")
    dest = art / BASELINE_ZIP_NAME
    shutil.copy2(ORIGIN_EYES_ZIP, dest)
    digest = write_bytes_sha(dest)
    if digest != BASELINE_SHA256:
        raise ConvProtocolError(f"baseline sha must be a9ffa852 pin, got {digest[:16]}")
    return digest


def prepare_conv_trees(*, repo: Path | None = None) -> tuple[Path, Path, Path]:
    root = (repo or REPO_ROOT) / "reports" / "awakening_conv_run"
...
def run_awakening_conv(*, repo: Path | None = None) -> dict[str, Any]:
    ...
    try:
        ...
        fixture = persist_conv_fixture(work, art)
        world_ok = bool(fixture.get("world_ok"))
        if float(fixture.get("real_data_pct") or 0.0) == 100.0:
            raise ConvProtocolError("Gate 1 regression: real_data_pct printed 100")
    except ConvProtocolError as exc:
        missing = True
        reason = str(exc)
    if body_exam_enabled(world_ok) and not missing:
        try:
            base_eval = run_conv_eval(work=work, art=art, zip_path=art / BASELINE_ZIP_NAME, kind="base")
    ...
    if body_exam_enabled(world_ok) and not missing:
        try:
            train_out = run_conv_v1_train(work=work, art=art, init_zip=None)
    ...
    if body_exam_enabled(world_ok) and not missing:
        try:
            child_eval = run_conv_eval(work=work, art=art, zip_path=art / CHILD_ZIP_NAME, kind="child")
    ...
    if (not world_ok) or (not have_books):
        ...
        leg_a = compute_conv_leg(base_a, child_a, missing=missing)
        leg_b = compute_conv_leg(base_b, child_b, missing=missing)
    licensed = license_conv(leg_a, leg_b, missing=missing, world_ok=world_ok)
    ...
    (reports / "AWAKENING_CONV_AUDIT.md").write_text(
        render_audit(gate0=gate0, proto=proto, t0=t0, t1=t1, t2_a=t2_a, t2_b=t2_b, t3=t3, flags=flags, g6=g6),
        encoding="utf-8",
    )
    (reports / "AWAKENING_CONV_VERDICT.md").write_text(render_verdict(flags=flags, t2_a=t2_a, t2_b=t2_b), encoding="utf-8")
    _append_logs(flags)
...
def _append_logs(flags: dict[str, Any]) -> None:
    ...
    block = [
        f"## {now} — AWAKENING_ENRICHER_CONVERSION",
        "",
        "LAW: #43 missed 25/25 by 1–3 points at |0.15|. This window tries |0.12| once.",
        "prod default unchanged",
        ...
        f"- hash=`{flags.get('fixture_train_hash')}` seed=`{CONV_SEED}` "
        ...
        f"floor=150 GENESIS_EYES_OK=false oracle_regime=false "
        f"prod_enricher_default_changed=false REAL=no G6=`{flags.get('G6_tag')}`",
        "",
        HONESTY_PARAGRAPH,
        "",
    ]
    log_path = CONV_ROOT / "LUMINA_CONV_EXPERIMENT_LOG.md"
    existing = (
        log_path.read_text(encoding="utf-8")
        if log_path.is_file()
        else "# LUMINA Awakening ENRICHER_CONVERSION experiment log\n\n"
    )
    ...
    if birth_log.is_file():
        ...
        if "Pointer — AWAKENING_ENRICHER_CONVERSION" not in text:
            birth_log.write_text(text.rstrip() + POINTER, encoding="utf-8")


def main() -> int:
    flags = run_awakening_conv()
