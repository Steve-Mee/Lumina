"""G5 license for AWAKENING_PHYSICS_TAPE. Vs G2 a9ffa852 books. Floor 150."""
...
TAG_PHYSICS_OK = "PHYSICS_OK"
TAG_PHYSICS_FAIL = "PHYSICS_FAIL"
TAG_PHYSICS_WORLD_FAIL = "PHYSICS_WORLD_FAIL"
TAG_PHYSICS_THIN = "PHYSICS_THIN"
TAG_S_HARM = "S_HARM"
...
def empty_physics_flags() -> dict[str, Any]:
    return {
        "source": SOURCE,
        "fixture_seed": PHYSICS_SEED,
        "fixture_train_hash": "",
        "trend_up_frac_holdout": 0.0,
        "trend_down_frac_holdout": 0.0,
        "world_ok": False,
        "attempts": 0,
...
def license_physics(
    ...
    *,
    ...
    if miss:
        ...
        tag, law, family = TAG_PHYSICS_THIN, LAW_NONE, FAMILY_H_NONE
    elif moved_a and moved_b:
        tag, law, family = TAG_PHYSICS_OK, LAW_SHADOW, FAMILY  # license both legs
    else:
        tag, law, family = TAG_PHYSICS_FAIL, LAW_NONE, FAMILY_H_NONE
    return {
        "tag": tag,
        "law": law,
...
def compose_physics_flags(payload: dict[str, Any]) -> dict[str, Any]:
    flags = empty_physics_flags()
    ...
    if not world_ok:
        ...
        return flags
    if str(flags.get("tag")) == TAG_PHYSICS_OK and not (
        bool(flags.get("MOVED_A")) and bool(flags.get("MOVED_B"))
    ):
        flags["tag"] = TAG_PHYSICS_FAIL
        flags["law"] = LAW_NONE
        flags["licensed_next_family"] = FAMILY_H_NONE
    return flags
