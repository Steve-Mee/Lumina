"""CLI: isolated slope 0.004 on NEW 8e-6 tape, FORCE_OPEN train-only 10k, license vs a9ffa852."""
...
from lumina_core.birth.awakening_drift_run import origin_guard_paths as drift_origin_guard_paths
from lumina_core.birth.awakening_occupancy_tape import write_bytes_sha
from lumina_core.birth.awakening_scale_enrich import PHYSICS_SLOPE_ABS, PROD_SLOPE_ABS, ScaleProtocolError
from lumina_core.birth.awakening_scale_eval import run_scale_eval
...
from lumina_core.birth.awakening_scale_report import render_audit, render_verdict
from lumina_core.birth.awakening_scale_tables import (
    HONESTY_PARAGRAPH,
    table_t0_identity,
    table_t1_honesty,
    table_t2_leg,
...
from lumina_core.birth.awakening_scale_tape import (
    ...
    persist_scale_fixture,
)
from lumina_core.birth.awakening_scale_train import run_scale_v1_train
...
POINTER = (
    ...
    "Isolated PHYSICS_SLOPE_ABS=0.004 on a NEW 8e-6 tape + scratch 46-dim V1 10k "
    "lives under `reports/awakening_scale_run`. Floor 150 stays. 1% guard intact. "
    "GENESIS_EYES_OK is false. REAL=no. Production ±0.15 unchanged. PHASE_BLOCKS=6. "
    "Last synthetic-world knob. world_engineering_closed=true.\n"
)
DRIFT_FLAGS = REPO_ROOT / "reports" / "awakening_drift_run" / "artifacts" / "awakening_drift_flags.json"


def origin_guard_paths(*, repo: Path | None = None) -> dict[str, Path]:
    paths = dict(drift_origin_guard_paths(repo=repo))
    root = repo or REPO_ROOT
    dj = root / "reports" / "awakening_drift_run" / "artifacts"
    dl = root / "reports" / "awakening_drift_run"
    paths.update(
        {
            "awakening_drift_flags.json": dj / "awakening_drift_flags.json",
            "01_drift_fixture_manifest.json": dj / "01_drift_fixture_manifest.json",
            "AWAKENING_DRIFT_VERDICT.md": dl / "AWAKENING_DRIFT_VERDICT.md",
            "LUMINA_DRIFT_EXPERIMENT_LOG.md": dl / "LUMINA_DRIFT_EXPERIMENT_LOG.md",
        }
    )
    return paths
...
def assert_origin_untouched(before: dict[str, str], *, repo: Path | None = None) -> None:
    ...
    for name, sha in before.items():
        if sha and after.get(name) != sha:
            raise ScaleProtocolError(f"origin artifact overwritten: {name}")


def copy_baseline_zip(art: Path) -> str:
    art.mkdir(parents=True, exist_ok=True)
    if not ORIGIN_EYES_ZIP.is_file():
        raise ScaleProtocolError("frozen living MARK_EYES zip missing")
    dest = art / BASELINE_ZIP_NAME
    shutil.copy2(ORIGIN_EYES_ZIP, dest)
    digest = write_bytes_sha(dest)
    if digest != BASELINE_SHA256:
        raise ScaleProtocolError(f"baseline sha must be a9ffa852 pin, got {digest[:16]}")
    return digest


def prepare_scale_trees(*, repo: Path | None = None) -> tuple[Path, Path, Path]:
    root = (repo or REPO_ROOT) / "reports" / "awakening_scale_run"
...
def live_check_g0() -> dict[str, Any]:
    if int(POLICY_EDGE_MIN_TRADES) != 150:
        raise ScaleProtocolError("G0: POLICY_EDGE_MIN_TRADES must be 150")
    if not DRIFT_FLAGS.is_file():
        raise ScaleProtocolError("G0: origin drift flags missing")
    booked = json.loads(DRIFT_FLAGS.read_text(encoding="utf-8"))
    if str(booked.get("tag") or "") != "DRIFT_ENRICH_FAIL":
        raise ScaleProtocolError(f"G0: drift tag must be DRIFT_ENRICH_FAIL, got {booked.get('tag')}")
    if abs(float(booked.get("drift_rth") or 0.0) - 8.0e-6) > 1e-15:
        raise ScaleProtocolError(f"G0: drift_rth must be 8e-6, got {booked.get('drift_rth')}")
    if booked.get("in_band") is not True:
        raise ScaleProtocolError("G0: drift in_band must be true")
    return {"tag": "DRIFT_ENRICH_FAIL", "drift_rth": 8.0e-6, "in_band": True, "floor": 150}


def run_awakening_scale(*, repo: Path | None = None) -> dict[str, Any]:
    os.environ["LUMINA_FABRIC_SUPERVISOR"] = "0"
    os.environ["VOICE_ENABLED"] = "false"
    origin_before = snapshot_origin_artifacts(repo=repo)
    reports, work, art = prepare_scale_trees(repo=repo)
    ...
    try:
        ...
        fixture = persist_scale_fixture(work, art)
        ...
        if float(fixture.get("real_data_pct") or 0.0) == 100.0:
            raise ScaleProtocolError("Gate 1 regression: real_data_pct printed 100")
    except ScaleProtocolError as exc:
        missing = True
        reason = str(exc)
    if body_exam_enabled(world_ok) and not missing:
        try:
            base_eval = run_scale_eval(work=work, art=art, zip_path=art / BASELINE_ZIP_NAME, kind="base")
    ...
    if body_exam_enabled(world_ok) and not missing:
        try:
            child_eval = run_scale_eval(work=work, art=art, zip_path=art / CHILD_ZIP_NAME, kind="child")
    ...
    if world_fail:
        ...
        licensed = license_scale(leg_a, leg_b, world_fail=True)
    elif enrich_fail:
        leg_a, leg_b = empty_leg(), empty_leg()
        licensed = license_scale(leg_a, leg_b, enrich_fail=True)
    elif (not world_ok) or (not have_books):
        leg_a = {**empty_leg(), "S_MISSING": True}
        leg_b = {**empty_leg(), "S_MISSING": True}
        licensed = license_scale(leg_a, leg_b, missing=True)
    else:
        leg_a = compute_scale_leg(base_a, child_a, missing=missing)
        leg_b = compute_scale_leg(base_b, child_b, missing=missing)
        licensed = license_scale(leg_a, leg_b, missing=missing)
    flags = compose_scale_flags(
        {
...
def _append_logs(flags: dict[str, Any], fixture: dict[str, Any]) -> None:
    ...
    block = [
        f"## {now} — AWAKENING_SLOPE_SCALE",
        "",
        "LAW: detector scaled with drift. 0.12*(8e-6/2.4e-4)=0.004.",
        "This is the last synthetic-world knob.",
        f"attempts={json.dumps(flags.get('attempts') or [], default=str)}",
        f"fracs train_up={fixture.get('train_up_frac')} train_down={fixture.get('train_down_frac')} "
        f"hold_up={fixture.get('hold_up_frac')} hold_down={fixture.get('hold_down_frac')}",
        f"tag=`{flags.get('tag')}`",
        "",
    ...
    log_path = SCALE_ROOT / "LUMINA_SCALE_EXPERIMENT_LOG.md"
    existing = (
        log_path.read_text(encoding="utf-8")
        if log_path.is_file()
        else "# LUMINA Awakening SLOPE_SCALE experiment log\n\n"
    )
    ...
    if birth_log.is_file():
        ...
        if "Pointer — AWAKENING_SLOPE_SCALE" not in text:
            birth_log.write_text(text.rstrip() + POINTER, encoding="utf-8")


def main() -> int:
    flags = run_awakening_scale()
