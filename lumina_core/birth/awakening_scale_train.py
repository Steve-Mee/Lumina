...
logger = get_logger("lumina.birth.awakening_scale_train")


def pin_train_seed(seed: int) -> None:
    if int(seed) not in SCALE_SEEDS:
        raise ScaleProtocolError(f"train seed {seed} not in {SCALE_SEEDS}")
...
def run_scale_v1_train(
    *,
    work: Path,
    art: Path,
    train_seed: int,
    init_zip: Path | str | None = None,
    timesteps: int = SCALE_TIMESTEPS,
    learn_fn: Any | None = None,
    ppo_cls: Any | None = None,
) -> dict[str, Any]:
    if init_zip is not None:
        assert_forbidden_init(init_zip)
        raise ScaleProtocolError("init_policy must be scratch")
    if int(timesteps) != int(SCALE_TIMESTEPS):
        raise ScaleProtocolError(f"timesteps {timesteps} != {SCALE_TIMESTEPS}")
    if bool(PATH_EXIT_K3_SHADOW.get()) or bool(PATH_SHAPE_K3_SHADOW.get()):
        raise ScaleProtocolError("hooks must stay False")
    pin_train_seed(int(train_seed))
    tape = load_scale_train_split(work)
    train = list(tape["train"])
    if not train:
        raise ScaleProtocolError("TRAIN split empty — refuse holdout ticks")
    ...
    if not bool(env.env.envelope.get("participation_envelope_enabled")):
        raise ScaleProtocolError("FORCE_OPEN train envelope failed to arm")
    if _obs_shape(env) != (int(MARK_EYES_OBS_DIM),):
        raise ScaleProtocolError(f"env observation space {_obs_shape(env)} != (46,)")
    try:
        cls = ppo_cls
        if cls is None:
        ...
        logger.error("awakening.scale.sb3_missing: %s", exc)
    ...
    try:
        ...
        logger.error("awakening.scale.learn_failed: %s", exc)
        return _fail(error=f"S_MISSING: learn() {exc}")
    actual = int(getattr(model, "num_timesteps", 0) or 0)
    if actual <= 0:
        return _fail(
            learn_called=True, error="S_MISSING: actual_timesteps == 0 — do not relabel a9ffa852 as scale child"
        )
...
__all__ = ["pin_train_seed", "run_scale_v1_train"]
