"""Stage loop rollout — training loop I/O + EventBus choreography + main iteration."""
...
# TOPIC_COMPLETED = "birth.curriculum.stage.completed"
...
def run_stage_research_loop(
    ...
    *,
    ...
    bus.restore_states(
        stage=stage,
        stage_metrics=stage_metrics if isinstance(stage_metrics, dict) else None,
    )
    plateau_state = bus.plateau_state
    remediation_state = bus.remediation_state
    organism_autonomy_state = bus.autonomy_state
    wa_state = bus.wall_adaptation_state
    retries_this_stage = int(wa_state.retries_this_stage)
    adaptation_tier = int(wa_state.adaptation_tier)
    escalation_level = int(wa_state.escalation_level)
    adaptation_history = list(wa_state.adaptation_history)
    last_adaptation_stage_trades = int(wa_state.last_adaptation_stage_trades)
    adaptation_stuck_escapes = int(wa_state.adaptation_stuck_escapes)
    ...
    def _stage_metrics_payload() -> dict[str, Any]:
        ...
        if cur_cfg.meta_controller_enabled:
            payload.update(bus.meta_metrics_payload(stage))
        ...
        payload.update(bus.adaptation_recovery_metrics(stage))
    ...
    def _write_progress(
        *,
        ...
        wa_metrics = bus.adaptation_recovery_metrics(stage)
        ...
        if cur_cfg.meta_controller_enabled:
            scorecard.update(bus.meta_scorecard_fields(stage, meta_last_plan))
    ...
    def _apply_meta_plan(plan: MetaActionPlan, *, trigger: str = "") -> None:
        ...
        if plan.exit_strong_recovery:
            ...
            bus.meta_patch_state(stage, explore_multiplier=1.0)
        ...
        if plan.reward_tweak is not None:
            bus.meta_patch_state(stage, active_reward=reward_config_to_dict(plan.reward_tweak))
        if plan.primary != RecoveryStrategy.HOLD:
            meta_message_suffix = (
                f" · meta: {plan.primary.value} ({plan.rationale})"
            )
        self_eval_suffix = bus.meta_format_self_eval_suffix(stage)
        if self_eval_suffix:
    ...
    def _evaluate_wall_trigger(
        *,
        elapsed_stage_sec: float,
        failure_key: str,
        force: bool = False,
    ) -> dict[str, Any] | None:
        return bus.wall_evaluate_trigger(
            stage,
            **_wall_eval_context(
                elapsed_stage_sec=elapsed_stage_sec,
                failure_key=failure_key,
    ...
    def _would_certified_stage_stall(
        *,
        ...
        trigger = _evaluate_wall_trigger(
            elapsed_stage_sec=elapsed_stage_sec,
            failure_key=failure_key,
            force=force,
        )
    ...
    def _finalize_certified_stage_stall(
        ...
        *,
        ...
        autonomy_decision = bus.autonomy_evaluate_terminal_stall(stage, 
            cfg=cur_cfg,
    ...
    def _apply_phoenix_in_loop(*, stall_reason: str) -> bool:
        ...
        if not cur_cfg.autonomous_recovery_enabled or not cur_cfg.phoenix_loop_enabled:
            return False
        patch = bus.phoenix_begin_cycle(
            stage,
            stall_reason=stall_reason,
            novelty=str(organism_autonomy_state.phoenix.last_action or "expand_data"),
        )
        if patch is None:
            return False
        ...
        _cid = bus.emit(
            "plateau_reset_cycle",
            stage,
            {"stage_trades": stage_trades, "stage_wins": stage_wins},
        )
        bus.registry.pop_response(_cid)
        novelty_value = str(organism_autonomy_state.phoenix.last_action or "expand_data")
    ...
    def _try_stall_remediation_on_terminal(pending: dict[str, Any]) -> bool:
        ...
        if stall_reason != TERMINAL_STALL_REASON:
            return False
        if not bus.remediation_should_run(stage, plateau_exhausted=True):
            return False
        if bus.remediation_can_start(stage):
            bus.remediation_begin_cycle(
                stage,
                stage_trades=stage_trades,
                stage_wins=stage_wins,
            )
        ...
        if bus.remediation_is_exhausted(stage):
            if _trade_budget_remaining() > 0 and bus.remediation_can_start(stage):
                reset_plateau_for_new_cycle(
                    plateau_state,
                    stage_trades=stage_trades,
                    stage_wins=stage_wins,
            ...
            if cur_cfg.autonomous_recovery_enabled and _apply_phoenix_in_loop(
                ...
                return True
            return False
        action_raw = bus.remediation_begin_step(
            stage,
            stage_trades=stage_trades,
            stage_wins=stage_wins,
        )
        ...
        bus.remediation_record_outcome(
            stage,
    ...
    def _maybe_advance_stall_remediation_in_loop() -> bool:
        ...
        if not bus.remediation_should_advance(stage, current_winrate=current_winrate):
            return False
    ...
    def _meta_self_eval_phase_str() -> str:
        if cur_cfg.meta_controller_enabled and cur_cfg.meta_self_eval_enabled:
            return str(bus.meta_self_eval_state(stage).get('phase', ''))
        return ""
    ...
    def _start_policy_swarm() -> None:
        ...
        if (
            ...
            return
        baseline = (
            bus.meta_controller.active_reward
            if cur_cfg.meta_controller_enabled
    ...
    def _maybe_record_and_advance_swarm(*, trades: int, wins: int, total_pnl: float) -> None:
        ...
        if swarm_state.variant_index >= len(swarm_state.variants):
            ...
            if winner is not None:
                ...
                if cur_cfg.meta_controller_enabled:
                    bus.meta_patch_state(stage, active_reward=reward_config_to_dict(winner.reward))
    ...
    def _maybe_detect_plateau(*, stage_trades: int, stage_wins: int) -> None:
        ...
        if bus.plateau_check_enter(
            ...
            / float(max(1, stage_range_total_signals)),
            range_round_trips=stage_range_round_trips,
            velocity_stall=low_velocity_attempts > 0,
        ):
    ...
    def _sync_adaptation_from_bus() -> None:
        nonlocal retries_this_stage, adaptation_tier, escalation_level
        nonlocal adaptation_history, last_adaptation_stage_trades, adaptation_stuck_escapes
        wa = bus.wall_adaptation_state
        retries_this_stage = int(wa.retries_this_stage)
        adaptation_tier = int(wa.adaptation_tier)
        escalation_level = int(wa.escalation_level)
        adaptation_history = list(wa.adaptation_history)
        last_adaptation_stage_trades = int(wa.last_adaptation_stage_trades)
        adaptation_stuck_escapes = int(wa.adaptation_stuck_escapes)

    def _apply_bus_adaptation_result(result: dict[str, Any]) -> bool:
        nonlocal attempt, winrate_stagnation_count, hold_stagnation_count
        nonlocal wall_budget_exhausted, stage_started_at, ppo_steps_at_plateau_evolution_step
        if not result.get("applied"):
            return False
        _sync_adaptation_from_bus()
        decision = result.get("decision") if isinstance(result.get("decision"), dict) else {}
        chunk = decision.get("new_chunk_target")
        if chunk is not None:
            cur_cfg.rollout_chunk_trades = int(chunk)
        if result.get("mine"):
            _mine_and_inject(aggressive=bool(result.get("mine_aggressive", False)))
        if result.get("expand_data"):
            _maybe_expand_data()
        if result.get("spawn_plateau") and not plateau_state.active:
            bus.plateau_enter(stage, stage_trades=stage_trades, stage_wins=stage_wins)
            ppo_steps_at_plateau_evolution_step = int(host.ppo_steps)
    ...
    def _adaptation_recovery_context(
        *,
        failure_key: str,
    ...
    def _try_adaptive_stall_recovery(
        *,
        failure_key: str,
        trigger_type: str = "certified_stall",
        constitution_blocked: bool = False,
        ...
        if _should_terminal_stall_in_adaptive():
            return False
        result = bus.adaptation_try_recovery(
            stage,
            **_adaptation_recovery_context(
                failure_key=failure_key,
                trigger_type=trigger_type,
                constitution_blocked=constitution_blocked,
            ),
        )
        return _apply_bus_adaptation_result(result)
    ...
    def _force_never_stop_recovery(*, failure_key: str) -> bool:
        ...
        result = bus.adaptation_never_stop(
            stage,
            failure_key=failure_key,
            rollout_chunk_trades=cur_cfg.rollout_chunk_trades,
            terminal_blocked=_should_terminal_stall_in_adaptive(),
        ...
        applied = _apply_bus_adaptation_result(result)
        if applied and adaptation_tier >= 1:
            _mine_and_inject()
        if applied and adaptation_tier >= 2 and cur_cfg.auto_expand_on_adaptation:
            _maybe_expand_data()
        return applied
    ...
    while True:
        ...
        wall_trigger = _evaluate_wall_trigger(
            elapsed_stage_sec=elapsed_stage_sec,
            failure_key=failure_key,
        ...
        if stall_pending is not None:
            ...
            if adaptation_stuck:
                ...
                if _try_adaptation_stuck_escape(failure_key=failure_key):
                    continue
            elif _try_adaptive_stall_recovery(
                failure_key=failure_key,
                trigger_type=trigger_type,
                constitution_blocked=constitution_blocked,
            ):
        ...
        if cur_cfg.meta_controller_enabled:
            ...
            if cur_cfg.meta_self_eval_enabled:
                bus.meta_maybe_start_self_eval(stage, 
                    pre_snap,
                    strong_recovery_attempts=strong_recovery_attempts,
                    attempt=attempt + 1,
                )
            if (
                ...
                and bool(bus.meta_self_eval_state(stage).get('active', False))
            ):
                if bus.meta_self_eval_state(stage).get('phase') == SelfEvalPhase.PROBING.value:
                    pre_plan = bus.meta_decide_probe_rollout(stage, pre_snap)
                elif bus.meta_self_eval_state(stage).get('phase') == SelfEvalPhase.COMMITTED.value:
                    pre_plan = bus.meta_decide_committed_rollout(stage, pre_snap)
                else:
                ...
                pre_plan = bus.meta_decide_pre_rollout(
                    stage,
                    pre_snap,
            ...
            explore_steps = bus.meta_apply_explore_multiplier(stage, 
            ...
            if bool(bus.meta_self_eval_state(stage).get('reward_tweak_active', False)):
                reward_override = bus.meta_controller.active_reward
        else:
        ...
        if cur_cfg.meta_controller_enabled:
            bus.meta_patch_state(stage, increment_rollouts=True)
            ...
            self_eval_skip_review = (
                cur_cfg.meta_self_eval_enabled
                and bool(bus.meta_self_eval_state(stage).get('active', False))
                and SelfEvalPhase(bus.meta_self_eval_state(stage).get('phase', 'idle'))
                in (SelfEvalPhase.PROBING, SelfEvalPhase.COMMITTED)
            )
            if self_eval_skip_review:
                complete_plan = bus.meta_on_probe_complete(stage, 
                    snap,
                    attempt=attempt + 1,
                )
                if complete_plan is not None:
                    ...
                    if complete_plan.suggest_provisional_pass:
                        prov = bus.meta_evaluate_provisional_fallback(stage, 
                            snap,
                            allow_provisional=allow_provisional,
                            strong_recovery_attempts=strong_recovery_attempts,
                            stage_trades=stage_trades,
                    ...
                    committed_plan = bus.meta_decide_committed_rollout(stage, snap)
                    _apply_meta_plan(committed_plan, trigger="self_eval_committed")
                meta_message_suffix = bus.meta_format_self_eval_suffix(stage)
            else:
                ...
                exhausted_self_eval = (
                    cur_cfg.meta_self_eval_enabled
                    and bus.meta_self_eval_state(stage).get('phase') == SelfEvalPhase.EXHAUSTED.value
                )
                ...
                if should_review:
                    if exhausted_self_eval:
                        ...
                        review_plan = bus.meta_decide(stage, snap, trigger=review_trigger)
        ...
        if plateau_state.active:
            bus.plateau_increment_rollout(stage)
            failure_key_rollout = {
                CurriculumStage.STAGE1_TREND: "stage1_winrate",
                CurriculumStage.STAGE2_RANGE: "stage2_metric",
                CurriculumStage.STAGE3_MIXED: "stage3_constitution",
        ...
        if remediation_state.active:
            bus.remediation_increment_rollout(stage)
