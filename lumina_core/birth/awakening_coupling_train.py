"""G5: one scratch V1 PPO.learn() of 10_000 env steps. Train seed 20260909. TRAIN only."""
...
from lumina_core.birth.awakening_physics_tape import PHYSICS_HOLD_PCT, PHYSICS_TIMESTEPS, assert_forbidden_init, write_bytes_sha
...
logger = get_logger("lumina.birth.awakening_coupling_train")
CHILD_SCHEMA = "awakening_coupling_v1_pi_star_v1"
TRAIN_SEED = int(EXAM_SEED)


def pin_train_seed(seed: int) -> None:
    if int(seed) != int(TRAIN_SEED):
        raise CouplingProtocolError(f"train seed {seed} != {TRAIN_SEED}")
...
def load_coupling_train_split(work: Path) -> dict[str, Any]:
    split = load_split_cache(work, holdout_pct=PHYSICS_HOLD_PCT)
    if split is None or not split.train:
        raise CouplingProtocolError("coupling train split missing")
...
def run_coupling_v1_train(
    *,
    ...
    if init_zip is not None:
        ...
        raise CouplingProtocolError("init_policy must be scratch")
    if int(timesteps) != int(PHYSICS_TIMESTEPS):
        raise CouplingProtocolError(f"timesteps {timesteps} != {PHYSICS_TIMESTEPS}")
    if bool(PATH_EXIT_K3_SHADOW.get()) or bool(PATH_SHAPE_K3_SHADOW.get()):
        raise CouplingProtocolError("hooks must stay False")
    pin_train_seed(TRAIN_SEED)
    tape = load_coupling_train_split(work)
    train = list(tape["train"])
    if not train:
        raise CouplingProtocolError("TRAIN split empty — refuse holdout ticks")
    ...
    if _obs_shape(env) != (int(MARK_EYES_OBS_DIM),):
        raise CouplingProtocolError(f"env observation space {_obs_shape(env)} != (46,)")
    try:
        cls = ppo_cls
        if cls is None:
    ...
    except CouplingProtocolError:
        raise
    except Exception as exc:
        logger.error("awakening.coupling.sb3_missing: %s", exc)
        return {
            "status": "S_MISSING",
            "learn_called": False,
            "actual_timesteps": 0,
    ...
    try:
        ...
        logger.error("awakening.coupling.learn_failed: %s", exc)
        return {
            "status": "S_MISSING",
            "learn_called": False,
            "actual_timesteps": 0,
    ...
    if actual <= 0:
        return {
            ...
            "child_sha256": "",
            "init_policy": "scratch",
            "error": "S_MISSING: actual_timesteps == 0 — do not relabel a9ffa852 as coupling child",
        }
...
__all__ = ["pin_train_seed", "run_coupling_v1_train"]
