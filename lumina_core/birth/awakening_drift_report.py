"""AUDIT / VERDICT markdown for AWAKENING_PHYSICAL_DRIFT."""
...
from lumina_core.birth.awakening_drift_tables import HONESTY_PARAGRAPH
...
def render_verdict(*, flags: dict[str, Any], t2_a: dict[str, Any], t2_b: dict[str, Any]) -> str:
    return "\n".join(
        [
            "# AWAKENING_PHYSICAL_DRIFT_VERDICT",
            "",
            f"**tag:** `{flags.get('tag')}`",
            f"**law:** `{flags.get('law')}`",
            ...
            f"**used_old_drift_00024:** `{flags.get('used_old_drift_00024')}`",
