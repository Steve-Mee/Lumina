"""G4: one scratch V1 PPO.learn() of 10_000 env steps. Train seed 20260910. TRAIN only."""
...
logger = get_logger("lumina.birth.awakening_occupancy_train")


def pin_train_seed(seed: int) -> None:
    if int(seed) != int(TRAIN_SEED):
        raise OccupancyProtocolError(f"train seed {seed} != {TRAIN_SEED}")
...
def run_occupancy_v1_train(
    *,
    work: Path,
    art: Path,
    init_zip: Path | str | None = None,
    timesteps: int = OCCUPANCY_TIMESTEPS,
    learn_fn: Any | None = None,
    ppo_cls: Any | None = None,
) -> dict[str, Any]:
    if init_zip is not None:
        assert_forbidden_init(init_zip)
        raise OccupancyProtocolError("init_policy must be scratch")
    if int(timesteps) != int(OCCUPANCY_TIMESTEPS):
        raise OccupancyProtocolError(f"timesteps {timesteps} != {OCCUPANCY_TIMESTEPS}")
    if bool(PATH_EXIT_K3_SHADOW.get()) or bool(PATH_SHAPE_K3_SHADOW.get()):
        raise OccupancyProtocolError("hooks must stay False")
    pin_train_seed(TRAIN_SEED)
    tape = load_occupancy_train_split(work)
    train = list(tape["train"])
    if not train:
        raise OccupancyProtocolError("TRAIN split empty — refuse holdout ticks")
    ...
    if _obs_shape(env) != (int(MARK_EYES_OBS_DIM),):
        raise OccupancyProtocolError(f"env observation space {_obs_shape(env)} != (46,)")
    try:
        cls = ppo_cls
        if cls is None:
    ...
    except OccupancyProtocolError:
        raise
    except Exception as exc:
        logger.error("awakening.occupancy.sb3_missing: %s", exc)
        return {
            "status": "S_MISSING",
            "learn_called": False,
            "actual_timesteps": 0,
    ...
    try:
        ...
        logger.error("awakening.occupancy.learn_failed: %s", exc)
        return {
            "status": "S_MISSING",
            "learn_called": False,
            "actual_timesteps": 0,
    ...
    if actual <= 0:
        return {
            ...
            "child_sha256": "",
            "init_policy": "scratch",
            "error": "S_MISSING: actual_timesteps == 0 — do not relabel a9ffa852 as occupancy child",
        }
...
__all__ = ["pin_train_seed", "run_occupancy_v1_train"]
