"""G1: tradeable NQ band [12000, 28000]. At most three seeds. No clip-as-success."""
...
NQ_MAX = 28000.0
BAND_START_PRICE = 21150.0
BAND_SEEDS = (20260914, 20260915, 20260916)  # at most three seeds
BAND_START_ET = datetime(2025, 11, 3, 18, 0, tzinfo=ET)
BAND_START_ET_ISO = BAND_START_ET.isoformat()
BAND_DAYS = 90
BAND_RTH_SEC = 10
BAND_ETH_SEC = 60
BAND_DRIFT_RTH = 0.00024
BAND_RANGE_KAPPA = 0.01
BAND_PHASE_BLOCKS = 6
BAND_TIMESTEPS = 10_000
...
CHILD_ZIP_NAME = "awakening_band_v1_pi_star.zip"
CHILD_META_NAME = "awakening_band_v1_pi_star.json"
CHILD_SCHEMA = "awakening_band_v1_pi_star_v1"
FLAGS_NAME = "awakening_band_flags.json"
BAND_ROOT = REPO_ROOT / "reports" / "awakening_band_run"
BAND_WORK, BAND_ART = BAND_ROOT / "workspace", BAND_ROOT / "artifacts"
FORBIDDEN_TAPE_PREFIXES = (
    "5726ae7e", "e963d1ce", "afcea4fa", "5e7eae98", "8d1aa6f8",
    "9b66a162", "7923fa61", "b1f16c99", "39304755", "7e86c2bb",
)
...
FORBIDDEN_INIT_NAMES = frozenset(
    {
        "genesis_mark_eyes_pi_star.zip", "baseline_a9ffa852_pi_star.zip", "baseline_mark_eyes_v1_pi_star.zip",
        "awakening_mark_eyes_polish_pi_star.zip", "awakening_mark_eyes_v2_pi_star.zip", "init_mark_eyes_pi_star.zip",
        "birth_exit_pi_star.zip", "genesis_birth_exit_pi_star.zip", "awakening_mark_eyes_pi_star.zip",
        "awakening_occupancy_v1_pi_star.zip", "awakening_strat_v1_pi_star.zip", "awakening_conv_v1_pi_star.zip",
        "awakening_obj_v1_pi_star.zip",
    }
)
assert POLICY_EDGE_MIN_TRADES == 150 and MARK_EYES_OBS_DIM == 46
assert BAND_DRIFT_RTH == 0.00024 == OCCUPANCY_DRIFT_RTH and BAND_RANGE_KAPPA == 0.01 == OCCUPANCY_RANGE_KAPPA
assert BAND_PHASE_BLOCKS == 6 and len(BAND_SEEDS) == 3 and BAND_START_PRICE == 21150.0 == float(GENESIS_START_PRICE)
assert MIN_TREND_UP_FRAC == 0.25 and MIN_TREND_DOWN_FRAC == 0.25
assert PHYSICS_SLOPE_ABS == 0.12 and PROD_SLOPE_ABS == 0.15 and BASELINE_SHA256.startswith("a9ffa852")
assert NQ_MIN == 12000.0 and NQ_MAX == 28000.0


class BandProtocolError(RuntimeError):
    """AWAKENING_PRICE_BAND protocol crime (fail-closed)."""


def tape_in_band(ticks: list[dict[str, Any]]) -> tuple[bool, float, float]:
    lasts = [float(t["last"]) for t in ticks]
    return (min(lasts) >= NQ_MIN and max(lasts) <= NQ_MAX, min(lasts), max(lasts))


def decide_world_ok(*, in_band: bool, fracs_ok: bool, clipped: bool = False) -> bool:
    if clipped:  # no clip-as-success
        return False
    return bool(in_band) and bool(fracs_ok)


def next_band_seed(attempts: list[dict[str, Any]]) -> int | None:
    if len(attempts) >= 3:  # at most three seeds
        return None
    return int(BAND_SEEDS[len(attempts)])


def refuse_this_tape_hash(sha: str) -> str:
    text = str(sha or "").strip().lower()
    if text.startswith(FORBIDDEN_TAPE_PREFIXES):
        raise BandProtocolError(f"refused old tape hash {text[:16]} as THIS exam tape")
    return text
...
def assert_forbidden_init(path: Path | str, sha: str = "") -> Path:
    ...
    if text[:8] in FORBIDDEN_INIT_SHA16:
        raise BandProtocolError(f"refused forbidden init sha {text[:8]}")
    if target.name in FORBIDDEN_INIT_NAMES or is_gitignored_ppo_zip(target) or "/lumina_agents/ppo/" in f"/{posix}":
        raise BandProtocolError(f"refused forbidden init {target.name}")
    return target


def generate_band_tape_ticks(*, seed: int, start_price: float = BAND_START_PRICE) -> tuple[list[dict[str, Any]], list[str], dict[str, int]]:
    stamps = _iter_session_times(
        start_et=BAND_START_ET,
        calendar_days=BAND_DAYS,
        rth_bar_seconds=BAND_RTH_SEC,
        eth_bar_seconds=BAND_ETH_SEC,
    )
    if len(stamps) < 1_000:
        raise BandProtocolError(f"band fixture too thin: {len(stamps)}")
    rng = np.random.default_rng(int(seed))
    labels = generator_labels(len(stamps), BAND_PHASE_BLOCKS)
    ...
    if any("regime" in row or "gen_phase" in row for row in ticks):
        raise BandProtocolError("no oracle")
    return ticks, labels, gen_counts


def try_band_seeds() -> dict[str, Any]:
    attempts: list[dict[str, Any]] = []
    for seed in BAND_SEEDS:  # at most three seeds
        ticks, labels, gen_counts = generate_band_tape_ticks(seed=int(seed))
    ...
    return {
        ...
        "seed_used": 0,
        "in_band": False,
        "price_min": float(last["min"]),
        "price_max": float(last["max"]),
        "world_fail": True,
    }


def _write_manifest(art: Path, payload: dict[str, Any]) -> dict[str, Any]:
    sidecar = art / "01_band_fixture_manifest.json"
    write_fixture_sidecar(sidecar, payload)
    sidecar.write_text(json.dumps(payload, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    return payload


def persist_band_fixture(work: Path, art: Path) -> dict[str, Any]:
    probe = try_band_seeds()
    art.mkdir(parents=True, exist_ok=True)
    base = {
        "seed_used": int(probe.get("seed_used") or 0),
        "attempts": list(probe.get("attempts") or []),
        "price_min": float(probe.get("price_min") or 0.0),
        "price_max": float(probe.get("price_max") or 0.0),
        "in_band": bool(probe.get("in_band")),
        "world_ok": False,
        "world_fail": bool(probe.get("world_fail")),
        "train_up_frac": 0.0,
        "train_down_frac": 0.0,
        "hold_up_frac": 0.0,
        "hold_down_frac": 0.0,
        ...
        "phase_blocks": int(BAND_PHASE_BLOCKS),
        "splitter": SPLITTER_NAME,
        "start_et": BAND_START_ET_ISO,
        "start_price": float(BAND_START_PRICE),
        "nq_min": float(NQ_MIN),
        "nq_max": float(NQ_MAX),
        "clipped": False,
    }
    if bool(probe.get("world_fail")):
        return _write_manifest(art, base)
    ...
    if any("gen_phase" in row for row in enriched):
        raise BandProtocolError("generator phase side-channel must be stripped before persist")
    ...
    paths = save_birth_data_cache(
        ...
        requested_days=max(FOUNDATION_HISTORY_START_DAYS, BAND_DAYS),
    ...
    if float(payload["real_data_pct"]) != 0.0:
        raise BandProtocolError("real_data_percentage must be 0.0")
    if int(payload["holdout_tick_count"]) < MIN_HOLDOUT_TICKS or len(legs) != 2 or min(legs) < MIN_TICKS_PER_LEG:
        raise BandProtocolError("holdout/leg density failed")
    _write_manifest(art, payload)
    if not world_ok:
        raise BandProtocolError("S_MISSING: post-enrich 25/25 both splits failed")
    return payload


def load_band_train_split(work: Path) -> dict[str, Any]:
    split = load_split_cache(work, holdout_pct=STRAT_HOLD_PCT)
    if split is None or not split.train:
        raise BandProtocolError("band train split missing")
    manifest = load_cache_manifest(work) or {}
    return {"train": list(split.train), "holdout": list(split.holdout), "train_hash": str(manifest.get("hash") or manifest.get("train_hash") or "")}
...
def inspect_band_protocol() -> dict[str, Any]:
    sites = {
        "nq_min_max": ("lumina_core/birth/awakening_band_tape.py", "NQ_MIN 12000 NQ_MAX 28000"),
        "no_clip_as_success": ("lumina_core/birth/awakening_band_tape.py", "no clip-as-success"),
        "at_most_three_seeds": ("lumina_core/birth/awakening_band_tape.py", "at most three seeds"),
        ...
        "both_leg_license": ("lumina_core/birth/awakening_band_flags.py", "both-leg license"),
        "genesis_eyes_ok_false": ("lumina_core/birth/awakening_band_flags.py", "GENESIS_EYES_OK false"),
    }
