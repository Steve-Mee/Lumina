"""AUDIT / VERDICT markdown for AWAKENING_OCCUPANCY_BALANCE."""
...
from lumina_core.birth.awakening_occupancy_tables import HONESTY_PARAGRAPH
...
def render_verdict(*, flags: dict[str, Any], t2_a: dict[str, Any], t2_b: dict[str, Any]) -> str:
    return "\n".join(
        [
            "# AWAKENING_OCCUPANCY_BALANCE_VERDICT",
            "",
            f"**tag:** `{flags.get('tag')}`",
            f"**law:** `{flags.get('law')}`",
