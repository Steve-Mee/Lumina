...
def evaluate_stage3_edgescore(
    *,
    ...
    """Stage-3 EdgeScore: WR hygiene + hold cap + expectancy + entropy + constitution."""
    ...
    hold_cap = float(getattr(cfg, "stage3_hold_ratio_max", 0.70))
    ...
    hygiene_ok = winrate >= wr_floor or roll >= wr_floor
    activity_ok = hold_v <= hold_cap
    ...
    score = max(
        ...
        min(
            1.0,
            0.35 * max(0.0, min(1.0, (effective_wr - wr_floor) / max(1e-6, 0.50 - wr_floor)))
    ...
    if not hygiene_ok:
        blockers.append(f"hygiene wr {winrate:.1%}/{roll:.1%} < {wr_floor:.0%}")
    if not activity_ok:
        blockers.append(f"hold {hold_v:.1%} > {hold_cap:.0%}")
    if not entropy_ok:
