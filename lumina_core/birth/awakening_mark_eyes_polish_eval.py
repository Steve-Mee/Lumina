"""G2/G4 evaluate-only: frozen a9ffa852 then polish child on THIS tape A/B."""
...
LEDGER_NAMES = {
    ("base", "A"): "polish_base_A_close_ledger.jsonl",
    ("base", "B"): "polish_base_B_close_ledger.jsonl",
    ("child", "A"): "polish_child_A_close_ledger.jsonl",
    ("child", "B"): "polish_child_B_close_ledger.jsonl",
}
...
def _assert_eval_ready(leg: str, zip_path: Path) -> None:
    if TRAIN:
        raise PolishProtocolError("TRAIN must stay False")
    if str(leg) not in {"A", "B"}:
        raise PolishProtocolError("seeds recorded as labels A/B only")
    refuse_old_baseline(zip_path)
    if zip_path.name not in {INIT_ZIP_NAME, CHILD_ZIP_NAME}:
        raise PolishProtocolError(f"refused PPO.load of non-polish zip {zip_path.name}")
    if bool(PATH_EXIT_K3_SHADOW.get()) or bool(PATH_SHAPE_K3_SHADOW.get()):
        raise PolishProtocolError("path_exit / path_shape hooks must stay False")
    ...
    *,
    ...
    try:
        run_evaluate_only(
            ...
            ledger_source=f"awakening_mark_eyes_polish_{kind}_{leg}",
...
def run_polish_eval(
    *,
    ...
    if TRAIN:
        raise PolishProtocolError("TRAIN flag False")
    if kind not in {"base", "child"}:
        raise PolishProtocolError("kind must be base or child")
    ...
    book_a = eval_polish_leg(holdout=leg_a, work=work, art=art, zip_path=zip_path, kind=kind, leg="A")
    book_b = eval_polish_leg(holdout=leg_b, work=work, art=art, zip_path=zip_path, kind=kind, leg="B")
...
__all__ = ["eval_polish_leg", "organism_stats", "policy_obs_dim", "run_polish_eval"]
