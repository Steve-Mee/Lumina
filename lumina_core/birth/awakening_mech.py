...
def inspect_grind_live_path() -> dict[str, Any]:
    ...
    envelope_enabled = (
        "participation_envelope_enabled" in grind_src and "foundation_occupancy_envelope_enabled" in grind_src
    )
    ...
    dump = {
        ...
        "rolling_occupancy_window_in_grind_kwargs": "occupancy_control_window=" in grind_src
        and "occupancy_control_window_bars" in grind_src,
    ...
    dump["W_WIRE"] = compute_w_wire(
        envelope_enabled=bool(envelope_enabled),
        chatter_bound_live=bool(chatter_live),
        refractory_live=bool(refractory_live),
        min_dwell_in_kwargs=bool(min_dwell),
        plant_tag_present=bool(plant_tag),
    )
    return dump
