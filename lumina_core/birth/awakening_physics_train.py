"""G3: one scratch V1 PPO.learn() of 10_000 env steps. Train seed 20260908. TRAIN only."""
...
logger = get_logger("lumina.birth.awakening_physics_train")


def pin_train_seed(seed: int) -> None:
    if int(seed) != int(TRAIN_SEED):
        raise PhysicsProtocolError(f"train seed {seed} != {TRAIN_SEED}")
...
def run_physics_v1_train(
    *,
    ...
    if init_zip is not None:
        ...
        raise PhysicsProtocolError("init_policy must be scratch")
    if int(timesteps) != int(PHYSICS_TIMESTEPS):
        raise PhysicsProtocolError(f"timesteps {timesteps} != {PHYSICS_TIMESTEPS}")
    if bool(PATH_EXIT_K3_SHADOW.get()) or bool(PATH_SHAPE_K3_SHADOW.get()):
        raise PhysicsProtocolError("hooks must stay False")
    pin_train_seed(TRAIN_SEED)
    tape = load_physics_train_split(work)
    train = list(tape["train"])
    if not train:
        raise PhysicsProtocolError("TRAIN split empty — refuse holdout ticks")
    ...
    if _obs_shape(env) != (int(MARK_EYES_OBS_DIM),):
        raise PhysicsProtocolError(f"env observation space {_obs_shape(env)} != (46,)")
    try:
        cls = ppo_cls
        if cls is None:
    ...
    except PhysicsProtocolError:
        raise
    except Exception as exc:
        logger.error("awakening.physics.sb3_missing: %s", exc)
        return {
            "status": "S_MISSING",
            "learn_called": False,
            "actual_timesteps": 0,
    ...
    try:
        ...
        logger.error("awakening.physics.learn_failed: %s", exc)
        return {
            "status": "S_MISSING",
            "learn_called": False,
            "actual_timesteps": 0,
    ...
    if actual <= 0:
        return {
            ...
            "child_sha256": "",
            "init_policy": "scratch",
            "error": "S_MISSING: actual_timesteps == 0 — do not relabel a9ffa852 as physics child",
        }
...
__all__ = ["pin_train_seed", "run_physics_v1_train"]
