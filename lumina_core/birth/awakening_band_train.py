"""G5: scratch V1 PPO.learn() of 10_000 env steps. Reuses FORCE_OPEN train factory."""
...
logger = get_logger("lumina.birth.awakening_band_train")


def pin_train_seed(seed: int) -> None:
    if int(seed) not in BAND_SEEDS:
        raise BandProtocolError(f"train seed {seed} not in {BAND_SEEDS}")
...
def run_band_v1_train(
    *,
    work: Path,
    art: Path,
    train_seed: int,
    init_zip: Path | str | None = None,
    timesteps: int = BAND_TIMESTEPS,
    learn_fn: Any | None = None,
    ppo_cls: Any | None = None,
) -> dict[str, Any]:
    if init_zip is not None:
        assert_forbidden_init(init_zip)
        raise BandProtocolError("init_policy must be scratch")
    if int(timesteps) != int(BAND_TIMESTEPS):
        raise BandProtocolError(f"timesteps {timesteps} != {BAND_TIMESTEPS}")
    if bool(PATH_EXIT_K3_SHADOW.get()) or bool(PATH_SHAPE_K3_SHADOW.get()):
        raise BandProtocolError("hooks must stay False")
    pin_train_seed(int(train_seed))
    tape = load_band_train_split(work)
    train = list(tape["train"])
    if not train:
        raise BandProtocolError("TRAIN split empty — refuse holdout ticks")
    ...
    if not bool(env.env.envelope.get("participation_envelope_enabled")):
        raise BandProtocolError("FORCE_OPEN train envelope failed to arm")
    if _obs_shape(env) != (int(MARK_EYES_OBS_DIM),):
        raise BandProtocolError(f"env observation space {_obs_shape(env)} != (46,)")
    try:
        cls = ppo_cls
        if cls is None:
    ...
    except BandProtocolError:
        raise
    except Exception as exc:
        logger.error("awakening.band.sb3_missing: %s", exc)
    ...
    try:
        ...
        logger.error("awakening.band.learn_failed: %s", exc)
        return _fail(error=f"S_MISSING: learn() {exc}")
    actual = int(getattr(model, "num_timesteps", 0) or 0)
    if actual <= 0:
        return _fail(learn_called=True, error="S_MISSING: actual_timesteps == 0 — do not relabel a9ffa852 as band child")
    ...
    if child_sha == BASELINE_SHA256:
        return _fail(learn_called=True, actual_timesteps=actual, child_sha256=child_sha, error="S_MISSING: child sha identical to a9ffa852")
    payload = {
        "schema": CHILD_SCHEMA,
        "sha256": child_sha,
        "init_policy": "scratch",
...
__all__ = ["pin_train_seed", "run_band_v1_train"]
