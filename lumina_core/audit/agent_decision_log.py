...
class AgentDecisionLog:
    ...
    def _last_hash_unlocked(self, *, is_real_mode: bool) -> str:
        ...
        try:
            ...
            return str(parsed.get("entry_hash", "GENESIS"))
            ...
            FaultPolicy.handle(
                ...
                fault_cls=AgentDecisionLogChainError,
                message=f"AgentDecisionLog failed to load previous hash from {self.path}",
    ...
    def _default_config_snapshot_hash(self) -> str:
        ...
        try:
            ...
            FaultPolicy.handle(
                domain=FaultDomain.AGENT_DECISION_LOG,
                operation="parse_config_snapshot",
                exc=exc,
                is_real_mode=False,
                message=f"AgentDecisionLog failed to parse config snapshot at {config_path}",
            ...
            try:
                ...
                FaultPolicy.handle(
                    domain=FaultDomain.AGENT_DECISION_LOG,
                    operation="read_config_snapshot_fallback",
                    exc=fallback_exc,
                    is_real_mode=False,
                    message=f"AgentDecisionLog failed to read config snapshot fallback at {config_path}",
                    context={"path": str(config_path)},
                    logger_obj=logger,
                )
