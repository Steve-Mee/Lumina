...
class MemoryService:
    ...
    def update_world_model(self, df: pd.DataFrame, regime: str, pa_summary: str) -> dict[str, Any]:
        world_model = self.engine.world_model or {
            ...
            "micro": {
                "regime": "NEUTRAL",
                "orderflow_bias": "balanced",
                "volume_profile": "fair_value",
                "last_update": None,
            },
        }

        world_model["micro"]["regime"] = regime
        world_model["micro"]["orderflow_bias"] = (
            "bullish" if df["close"].iloc[-1] > df["close"].iloc[-20:].mean() else "bearish"
        )
        world_model["micro"]["volume_profile"] = (
            "high_volume_node" if df["volume"].iloc[-1] > df["volume"].iloc[-20:].mean() * 1.8 else "fair_value"
        )
