...
class SupervisorPhaseStateMachine:
    ...
    def tick(
        ...
        **kwargs: Any,
        ...
        last_validation = getattr(engine, "last_validation", None) if engine is not None else self._last_validation
        if validator is not None and hasattr(validator, "run_3year_validation"):
            ...
            if should_run_validation:
                try:
                    ...
                    log_structured(err)
                    self._logger.error(f"Periodic validator run failed: {exc}")

        if time.time() - self._last_balance_fetch > 10:
        ...
        eod_force_hold = bool(eod_closer.enforce_eod_force_close(float(price)))
        ...
        should_run_swarm = (
            ...
            and self._swarm_last_cycle_minute != current_swarm_minute
        )
        if should_run_swarm:
            try:
                ...
                if hasattr(app, "get_current_dream_snapshot"):
                    dream_snapshot = app.get_current_dream_snapshot()
                self._swarm_last_cycle = time.time()
                self._swarm_last_cycle_minute = current_swarm_minute
            except Exception as exc:
                err = LuminaError(
                ...
                log_structured(err)
                self._logger.error(f"Swarm cycle error: {exc}")
                ...
                if not close_handled:
                    ...
                    if callable(publish_fn):
                        try:
                            ...
                            log_structured(err)
                            self._logger.error(f"TraderLeague publish error: {exc}")
                    ...
                    if swarm_manager is not None and hasattr(swarm_manager, "register_trade_result"):
                        try:
                            ...
                            log_structured(err)
                            self._logger.error(f"Swarm trade register error: {exc}")
                    ...
                    try:
                        ...
                        log_structured(err)
                        self._logger.error(f"Post-trade backtest error: {exc}")

        if swarm_manager is not None and time.time() - self._swarm_last_dashboard >= 60:
            try:
                ...
                self._swarm_last_dashboard = time.time()
            except Exception as exc:
                err = LuminaError(
                    severity=ErrorSeverity.RECOVERABLE_LEARNING,
                ...
                log_structured(err)
                self._logger.error(f"Swarm dashboard error: {exc}")
        ...
        if time.time() - self._last_status_print >= status_interval:
            rl_bias_str = ""
        ...
        if time.time() - self._last_infinite_sim_status >= 1800:
            inf_sim = getattr(engine, "infinite_simulator", None) if engine is not None else None
            if inf_sim is not None:
                self._logger.info("INFINITE_SIM_MONITOR,status=ready")
            self._last_infinite_sim_status = time.time()

        if time.time() - self._last_oracle > 60 and len(getattr(app, "pnl_history", []) or []) > 5:
        ...
        if time.time() - self._last_save > 30:
            try:
                app.save_state()
                self._logger.info("STATE_SAVED,status=ok")
            except Exception as _save_exc:
                self._logger.error(f"STATE_SAVE_FAILED: {_save_exc}\n{traceback.format_exc()}")
            self._last_save = time.time()

        if time.time() - self._last_monitoring_snapshot > 15:
            try:
                _publish_runtime_monitoring_snapshot(app)
            except Exception as exc:
                self._logger.warning(f"MONITORING_SNAPSHOT_WRITE_FAILED: {exc}")
            self._last_monitoring_snapshot = time.time()
