...
class MarketDataHistoryFetchMixin:
    """CrossTrade historical bar POST/fetch with daysBack-first + day-aligned pagination."""
    ...
    def _utc_iso_z(dt: datetime) -> str:
        if dt.tzinfo is None:
            dt = dt.replace(tzinfo=_mds().timezone.utc)
        else:
            dt = dt.astimezone(_mds().timezone.utc)
    ...
    def _sanitize_historical_payload_dates(payload: dict[str, Any]) -> dict[str, Any]:
        ...
        for key in ("from", "to"):
            ...
            try:
                ...
                out[key] = MarketDataHistoryFetchMixin._utc_iso_z(parsed.to_pydatetime())
            except Exception:
                out[key] = text
        return out
