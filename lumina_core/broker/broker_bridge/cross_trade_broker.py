...
class CrossTradeBroker(BrokerBridge):
    ...
    def get_account_info(self) -> AccountInfo:
        ...
        try:
            ...
            for layer in layers:
                if balance == 0.0:
                    balance = self._pick_float(layer, ACCOUNT_BALANCE_KEYS)
                if equity == 0.0:
                    equity = self._pick_float(layer, ACCOUNT_EQUITY_KEYS)
                if pnl == 0.0:
                    pnl = self._pick_float(layer, ACCOUNT_PNL_KEYS)
                if available_margin is None:
                    parsed_margin = self._pick_float(layer, ACCOUNT_AVAILABLE_MARGIN_KEYS)
