"""Community trades, leaderboard, and wisdom endpoints."""
...
from backend.app_auth import check_rate_limit, verify_admin_role, verify_api_key
...
def configure_community_security(security: dict[str, Any]) -> None:
    global _SECURITY
    _SECURITY = security
...
def _sec() -> dict[str, Any]:
    if _SECURITY is None:
        raise RuntimeError("configure_community_security not called")
    return _SECURITY
...
def _ingest_trade(trade: TradeSubmit) -> dict[str, int | str]:
    ...
    try:
        ...
        logging.exception("Unhandled broad exception fallback in lumina_os/backend/community_endpoints.py")
        db.rollback()
