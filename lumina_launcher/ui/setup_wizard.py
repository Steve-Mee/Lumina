    ...
    *,
    ...
    st.session_state[_WIZARD_STATE_KEY] = {
        "credentials": {
            ...
            "TAURI_SIGNING_PRIVATE_KEY_PATH": str(env_values.get(ENV_SIGNING_KEY_PATH, "")),
        },
...
def render_setup_wizard(
    *,
    ...
    if step_name == "welcome":
        ...
        st.markdown("#### Desktop build (Tauri updater signing)")
        signing_service = TauriSigningService(workspace_root)
        signing_configured = signing_service.is_configured(config_manager.parse_env_file())
        if signing_configured:
            st.success("Tauri signing key is geconfigureerd.")
        else:
            st.info("Genereer een signing key voor `npm run tauri build` en auto-updates.")
        signing_col_a, signing_col_b = st.columns([3, 2])
        with signing_col_a:
            creds["TAURI_SIGNING_PRIVATE_KEY_PATH"] = st.text_input(
                "TAURI signing key path (.env)",
                value=creds.get("TAURI_SIGNING_PRIVATE_KEY_PATH", ""),
                disabled=True,
            )
        with signing_col_b:
            if st.button("Genereer Tauri signing key", use_container_width=True, disabled=signing_configured):
                result = signing_service.generate_keypair(config_manager=config_manager, force=False)
                if result.success:
                    creds["TAURI_SIGNING_PRIVATE_KEY_PATH"] = config_manager.parse_env_file().get(
                        ENV_SIGNING_KEY_PATH, ""
                    )
        ...
        with st.expander("Tauri signing — geavanceerd"):
            st.warning(
                "Regenereren maakt een nieuw keypair. Bestaande installs kunnen updates "
                "niet meer verifiëren tenzij je opnieuw installeert."
            )
            if st.button("Regenereer Tauri signing key", use_container_width=True):
                result = signing_service.generate_keypair(config_manager=config_manager, force=True)
                if result.success:
                    creds["TAURI_SIGNING_PRIVATE_KEY_PATH"] = config_manager.parse_env_file().get(
                        ENV_SIGNING_KEY_PATH, ""
                    )
        ...
        st.write(
            f"  - TAURI_SIGNING_PRIVATE_KEY_PATH: `{creds.get('TAURI_SIGNING_PRIVATE_KEY_PATH') or '(empty)'}`"
        )
