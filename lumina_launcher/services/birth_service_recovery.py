"""Recovery/start helpers for BirthService (M5)."""
...
from lumina_launcher.services.birth_runner_wipe import wipe_all_birth_data
...
class BirthServiceRecoveryMixin:
    ...
    def _should_auto_resume_stalled_birth(self, progress: Dict[str, Any]) -> bool:
        ...
        if progress.get("retryable") is False:
            return False
        # Sacred surface: swarm no-lift freeze — accept_champion or wipe only.
        from lumina_launcher.services.birth_runner_recovery import champion_freeze_active_for_svc
    ...
    def phoenix_recovery_stalled_stage(self, target_trades: int | None = None) -> Dict[str, Any]:
        ...
        blocked = reject_if_champion_freeze(self)
        if blocked is not None:
            return blocked
