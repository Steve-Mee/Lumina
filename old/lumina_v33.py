...
def pre_dream_daemon():
    ...
    while True:
        ...
        try:
            ...
            # Vision call krijgt de consensus als extra context
            vision_content = [
                {"type": "text", "text": f"""Multi-Agent Consensus: {consensus['signal']} (conf {consensus['confidence']:.2f})
Agent votes: {consensus['agent_votes']}
Gebruik dit als basis. Geef ALLEEN JSON met: signal, confidence, stop, target, reason, why_no_trade, confluence_score, chosen_strategy, fib_levels_drawn, narrative_reasoning"""},
            ...
            payload = {
                "model": VISION_MODEL,
                "messages": [
                    {"role": "system", "content": "Je bent visueel getraind. Gebruik consensus als basis en geef een uitvoerbaar trade-besluit."},
            ...
            if r.status_code == 200:
                ...
                print(f"[{datetime.now().strftime('%H:%M:%S')}] 👁️ v33 MULTI-AGENT DREAM: {current_dream.get('chosen_strategy')} → {current_dream['signal']} (conf={current_dream['confluence_score']:.2f})")
            else:
