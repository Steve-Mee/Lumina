...
print("✅ Multi-Agent Swarm v20 – 24/7 Robuuste Data Pipeline (MESH26.CME + multi yf) (v3.2 compliant)")
...
def fetch_quote():
    # 1. CrossTrade (primary - alleen live als markt open)
    formats = [INSTRUMENT, "MES1!", "MES JUN 26", "MESM6", "MESH26.CME", "MES JUN26", "MESM26.CME"]
    for fmt in formats:
        try:
    ...
    # 2. yfinance multi-fallback (werkt 24/7, zelfs nu op zondag)
    yf_tickers = ["MES=F", "MESH26.CME", "MESM26.CME", "ES=F", "ESM26.CME", "MES JUN 26"]
    for tick in yf_tickers:
        print(f"   🔄 Trying yfinance {tick}...")
        try:
            ticker = yf.Ticker(tick)
            data = ticker.history(period="5d", interval="5m")
            if not data.empty:
                price = float(data['Close'].iloc[-1])
                volume = int(data.get('Volume', pd.Series([140000])).iloc[-1])
                print(f"   ✅ yfinance SUCCESS met {tick} | Price: {price:.2f} | Vol: {volume}")
                return price, volume
        except Exception as e:
            print(f"   ⚠️ yfinance {tick} failed: {str(e)[:100]}")

    # 3. Laatste noodfallback
    print("   ❌ Alle bronnen gefaald → random mock (tijdelijk tot markt open)")
...
def calculate_atr(df):
    ...
    return changes.rolling(14).mean().iloc[-1]
...
if __name__ == "__main__":
    print("🚀 v20 Swarm LIVE – 24/7 Data + yfinance fixes (Ctrl+C om stoppen)\n")
    try:
        while True:
