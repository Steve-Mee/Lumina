...
print("✅ Swarm Core v4.4 – FIX: RecurrentPPO env + Forced Critic debug")
...
# === MODEL + GLOBALE ENV (FIX voor AssertionError) ===
...
model.set_env(dummy_env)  # <<< CRASH-FIX: env koppelen na load()
print("   ✅ DummyEnv gekoppeld aan model (AssertionError opgelost)")
    ...
    try:
        ...
        if r.status_code == 200:
            ...
            if isinstance(executions, list) and len(executions) > 0:
                ...
                print(f"   📈 Real PnL fetched: {pnl:.2f}")
                return pnl
    except:
        pass
...
def get_real_x_sentiment():
    try:
        payload = {"model": "grok-4.20-0309-reasoning", "messages": [
            {"role": "system", "content": "Geef alleen één woord: BULLISH, BEARISH of NEUTRAL."},
            {"role": "user", "content": "Wat is het huidige sentiment op X voor MES JUN 26 futures?"}
        ]}
        ...
        if r.status_code == 200:
            ...
            print(f"   📡 REAL X-SENTIMENT: {sentiment}")
            return sentiment
    except Exception as e:
...
def actor_node(state: TradingState) -> TradingState:
    ...
    try:
        ...
        print(f"   ⚠️ PPO predict failed ({e}) → fallback HOLD")
        action = 0
...
def critic_node(state: TradingState) -> TradingState:
    ...
    print(f"   🔥 CRITIC START - Markt open: {state['is_market_open']} (forcing call voor debug)")
    for attempt in range(3):
        print(f"   🔥 CRITIC ATTEMPT {attempt+1}/3 - Payload voorbereiden...")
        payload = {"model": "grok-4.20-0309-reasoning", "messages": [
        ...
        print(f"   📏 Payload length: {len(str(payload))} chars")
        try:
            r = requests.post("https://api.x.ai/v1/chat/completions", headers={"Authorization": f"Bearer {XAI_KEY}"}, json=payload, timeout=15)
            print(f"   📡 Critic status code: {r.status_code}")
            if r.status_code == 200:
                advice = r.json()["choices"][0]["message"]["content"]
                print(f"   🔥 GROK CRITIC SUCCESS")
                break
            else:
                print(f"   ❌ Critic error response: {r.text[:500]}")
        except Exception as e:
            print(f"   ❌ Critic exception: {type(e).__name__}: {e}")
    veto = []
...
def supervisor_node(state: TradingState) -> TradingState:
    print(f"\n📊 Regime Oracle: {state['regime']} (ATR {state['atr']:.2f}) | X-Sentiment: {state.get('x_sentiment', 'N/A')}")
    print(f"🧠 Actor (PPO LSTM): {state['actor_signal']}")
    ...
    if state["is_market_open"] and state["final_signal"] != "HOLD":
        ...
        if qty == 0:
            ...
            quantity = 1
            print(f"   💰 PAPER-TRADE: {side} {quantity} contract(s)")
            if not DRY_RUN:
                try:
    ...
    if len(equity_curve) > 100:
        ...
        if drawdown > 15:
            print("   🚨 DRAW DOWN >15% → VOLLEDIGE PAUZE (guardrail)")
...
def replay_buffer_node(state: TradingState) -> TradingState:
    ...
    if log_len % 40 == 0:
        print(f"   🧠 REPLAY BUFFER AGENT ACTIVE: Real PnL = {pnl:.2f}")
        dummy_env = DummyTradingEnv()
        model.set_env(dummy_env)  # extra safety
        model.learn(total_timesteps=64, env=dummy_env, reset_num_timesteps=False, progress_bar=False)
        model.save(model_path)

    if log_len % 200 == 0:
        print("   🔥 CONTINUAL LEARNING ACTIVE: full retrain + exploration boost (bot blijft leren op achtergrond)")
        dummy_env = DummyTradingEnv()
    ...
    try:
        ...
        buy_ratio = 0
    print(f"   📋 STRATEGY FINGERPRINT: Bot leert momenteel BUY in LOW_VOL ({buy_ratio:.0f}% BUY-ratio) + exploreert nog")
...
if __name__ == "__main__":
    print("🚀 Swarm Core v4.4 LIVE – RecurrentPPO FIX + Forced Critic debug (Ctrl+C om stoppen)\n")
    try:
