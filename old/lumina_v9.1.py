...
VECTOR_MEMORY = "lumina_memory.npy"
MUTATIONS_DIR = "lumina_mutations"
os.makedirs(MUTATIONS_DIR, exist_ok=True)
print("✅ lumina_v9.1.py – LUMINA v5.0 + SELF-CODE MUTATION ENGINE (Grok-powered) + SAFE PROPOSALS + DYNAMIC LOADING")
print(f"🔍 DEBUG: Instrument={INSTRUMENT} | Account={CROSSTRADE_ACCOUNT} | DRY_RUN={DRY_RUN} | MutationsDir={MUTATIONS_DIR}")
if os.path.exists(SIGNAL_LOG):
    try:
        ...
        os.remove(SIGNAL_LOG)
model_path = "ppo_trading_model_v21_lumina"
...
def background_world_knowledge_update():
    ...
    while True:
        ...
        try:
            payload = {"model": "grok-beta", "messages": [{"role": "system", "content": "Geef alleen één woord: BULLISH, BEARISH of NEUTRAL."}, {"role": "user", "content": "Wat is het huidige sentiment op X voor MES JUN 26 futures?"}]}
            ...
            if r.status_code == 200:
                ...
                print(f"[{datetime.now().strftime('%H:%M:%S')}] 🌍 World Knowledge background update: {new_k}")
        except:
            pass
...
def save_to_memory(obs, reward, outcome):
    global memory_store
    memory_store.append((np.array(obs), reward, outcome))
    if len(memory_store) > 2000: memory_store.pop(0)
    np.save(VECTOR_MEMORY, np.array([o[0] for o in memory_store]))
...
def meta_consciousness_node(state: TradingState) -> TradingState:
    global evo_params, cycle_counter
    if cycle_counter % 75 != 0 or not XAI_KEY: return state
    logger.info("🌌 META-CONSCIOUSNESS: xAI self-reflection gestart...")
    print(f"[{datetime.now().strftime('%H:%M:%S')}] 🌌 META-CONSCIOUSNESS: xAI self-reflection gestart...")
    try:
        log_df = pd.read_csv(SIGNAL_LOG).tail(200)
        summary = f"Sharpe: {state.get('sharpe',0):.2f} | Expectancy: {state.get('expectancy',0):.2f} | Winrate: {state.get('winrate',0):.1f}% | Cycles: {cycle_counter}"
        payload = {"model": "grok-beta", "messages": [{"role": "system", "content": "Je bent LUMINA's Meta-Consciousness. Analyseer performance en stel 1-2 concrete verbeteringen voor (evo_params of nieuwe veto). Geef alleen JSON: {{'mutation': '...', 'new_veto': '...' }}"}, {"role": "user", "content": summary}]}
        ...
        if r.status_code == 200:
            ...
            print(f"[{datetime.now().strftime('%H:%M:%S')}] 🤖 Meta-suggestie: {suggestion}")
            evo_params["mutation_rate"] = max(0.05, min(0.25, evo_params["mutation_rate"] * 1.02))
    except:
        pass
    return state
...
def save_mutation_proposal(code_snippet: str, description: str):
    global cycle_counter
    filename = f"{MUTATIONS_DIR}/proposal_cycle_{cycle_counter:06d}.py"
    with open(filename, "w", encoding="utf-8") as f:
        f.write(f'# === LUMINA SELF-MUTATION PROPOSAL {cycle_counter} ===\n')
        f.write(f'# Beschrijving: {description}\n')
        f.write(f'# Datum: {datetime.now()}\n\n')
        f.write(code_snippet)
    print(f"[{datetime.now().strftime('%H:%M:%S')}] 📝 MUTATION PROPOSAL SAVED → {filename}")
    logger.info(f"Mutation proposal saved: {filename}")
    return filename
def self_code_mutation_node(state: TradingState) -> TradingState:
    global cycle_counter
    if cycle_counter % 75 != 0 or not XAI_KEY:
        return state
    logger.info("🌌 SELF-CODE MUTATION ENGINE gestart (Grok-powered)")
    print(f"[{datetime.now().strftime('%H:%M:%S')}] 🌌 SELF-CODE MUTATION ENGINE: Grok analyseert en stelt code voor...")
    try:
        log_tail = pd.read_csv(SIGNAL_LOG).tail(300).to_string()
        summary = f"Sharpe: {state.get('sharpe',0):.2f} | Expectancy: {state.get('expectancy',0):.2f} | Winrate: {state.get('winrate',0):.1f}% | Cycles: {cycle_counter} | Last regime: {state.get('regime','UNKNOWN')}"
        payload = {
            "model": "grok-beta",
            "messages": [
                {"role": "system", "content": "Je bent LUMINA's Self-Code Mutation Engine. Geef ALLEEN een geldige Python-functie als string. Output exact JSON: {\"description\": \"korte uitleg\", \"code\": \"def nieuwe_node(state: TradingState) -> TradingState: ... volledige code ...\", \"insert_after\": \"nexus\" or \"critic\" or \"supervisor\"}\nGeen extra tekst, geen markdown."},
                {"role": "user", "content": f"Analyseer de performance en stel 1 nieuwe oracle/veto voor die ons dichter bij Sharpe >2.5 brengt.\nSamenvatting:\n{summary}\nLaatste 300 signals:\n{log_tail[-2000:]}"}
            ]
        }
        ...
        if r.status_code == 200:
            ...
            code_snippet = suggestion["code"]
            description = suggestion.get("description", "Auto-mutatie")
            save_mutation_proposal(code_snippet, description)
            current_file = __file__
            next_version = "lumina_v9.1_mutated.py"
            with open(current_file, "r", encoding="utf-8") as f:
                content = f.read()
            marker = "# === MUTATION INSERT POINT: NEW ORACLES ==="
            if marker in content:
                insert_pos = content.find(marker) + len(marker)
                new_content = content[:insert_pos] + "\n\n" + code_snippet + "\n" + content[insert_pos:]
            else:
                new_content = content + "\n\n# === AUTO INSERTED MUTATION ===\n" + code_snippet
            with open(next_version, "w", encoding="utf-8") as f:
                f.write(new_content)
            print(f"[{datetime.now().strftime('%H:%M:%S')}] 🔥 NEW VERSION GENERATED → {next_version}")
            print("   → Stop huidige bot en start lumina_v9.1_mutated.py voor directe evolutie!")
    except Exception as e:
        print(f"[{datetime.now().strftime('%H:%M:%S')}] ⚠️ Mutation failed (veilig): {e}")
    return state
...
# === MUTATION INSERT POINT: NEW ORACLES ===
...
workflow.add_node("meta", meta_consciousness_node)
workflow.add_node("self_mutation", self_code_mutation_node)
...
workflow.add_edge("performance", "meta")
workflow.add_edge("meta", "self_mutation")
workflow.add_edge("self_mutation", "kelly")
...
if __name__ == "__main__":
    print("🚀 lumina_v9.1 LIVE – SELF-CODE MUTATION ENGINE ACTIEF (Grok haalt code op en genereert nieuwe versie!) (Ctrl+C om stoppen)\n")
    try:
        initial_state = {"obs": [0]*17, "regime": "", "microstructure": {}, "imagined_future_edge": 0.0, "nexus_score": 50.0, "dream_memory_edge": 0.0, "world_knowledge": "Markt gesloten - geen actuele info", "is_market_open": False, "previous_market_open": False, "actor_signal": "", "critic_reasoning": "", "critic_veto": [], "final_signal": "", "last_price": 0, "volume": 0, "position_qty": 0, "atr": 12.0, "data_source": "", "reward": 0.0, "lstm_states": None, "x_sentiment": "", "sharpe": 0.0, "expectancy": 0.0, "winrate": 0.0, "kelly_sizing": 0.0, "messages": []}
        while True:
