...
print("✅ Multi-Agent Swarm v23 – 24/7 Adaptive + Eerste PPO Retrain + MarketStatus (v3.2 compliant)")
...
def is_market_open():
    now = datetime.now()
    # CME Globex hours (approx) - zondag 23:00 CET tot vrijdag 22:00 CET
    weekday = now.weekday()
    hour = now.hour
    if weekday >= 5 and hour < 23:  # weekend closed until Sunday 23:00
        return False
    return True

def fetch_quote():
    # PRIMARY + max 3 attempts
...
def calculate_atr(df):
    ...
    return np.clip(atr, 8.0, 35.0)  # dynamic cap
...
def critic_node(state: TradingState) -> TradingState:
    ...
    if state["is_market_open"]:  # async-style: only call when open
        ...
        advice = "Closed market - sentiment cached"
    veto = []
...
def supervisor_node(state: TradingState) -> TradingState:
    ...
    # EERSTE PPO RETRAIN STUB - echte learning start
    if len(pd.read_csv(SIGNAL_LOG)) % 30 == 0 and state["is_market_open"]:
        print("   🧠 LEARNING ACTIVE: PPO online retrain stub (simulated P&L update)")
        # Hier komt later echte update
...
if __name__ == "__main__":
    print("🚀 v23 Swarm LIVE – 24/7 Adaptive + Eerste Learning Step (Ctrl+C om stoppen)\n")
    try:
        while True:
