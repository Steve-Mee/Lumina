...
print("🌌 LUMINA v16.3 – PURE LIVE PORTFOLIO VALUE (CORRECT /accounts ENDPOINT)")
...
# ====================== PURE LIVE API (CORRECT ENDPOINT) ======================
def fetch_quote():
    try:
        ...
        if r.status_code == 200:
            ...
            print(f"[{datetime.now().strftime('%H:%M:%S')}] ⚠️ QUOTE API ERROR {r.status_code} – raw: {r.text}")
            return 0, 0
...
def get_real_portfolio_value():
    try:
        url = f"https://app.crosstrade.io/v1/api/accounts/{CROSSTRADE_ACCOUNT}"
        r = requests.get(url, headers={"Authorization": f"Bearer {CROSSTRADE_TOKEN}"}, timeout=6)
        print(f"[{datetime.now().strftime('%H:%M:%S')}] DEBUG PORTFOLIO API STATUS: {r.status_code}")
        print(f"[{datetime.now().strftime('%H:%M:%S')}] DEBUG RAW RESPONSE: {r.text[:500]}...")  # truncated voor log
        if r.status_code == 200:
            data = r.json()
            equity = float(data.get("cash_value", data.get("totalCashBalance", data.get("equity", 0))))
            unrealized = float(data.get("unrealizedProfitLoss", 0))
            print(f"[{datetime.now().strftime('%H:%M:%S')}] LIVE PORTFOLIO VALUE: ${equity:.2f} | Unrealized PnL: {unrealized:+.2f}")
            return equity
        else:
            print(f"[{datetime.now().strftime('%H:%M:%S')}] ⚠️ PORTFOLIO API FAILED – raw: {r.text}")
            return 0
    except Exception as e:
...
if __name__ == "__main__":
    print("🚀 LUMINA v16.3 – CORRECT PORTFOLIO ENDPOINT + FULL DEBUG GESTART")
    ...
    try:
        while True:
            ...
            time.sleep(1 if result["is_market_open"] else 30)
