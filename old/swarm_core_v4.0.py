...
print("✅ Swarm Core v4.0 – Continual Learning Swarm (nooit meer vastpinnen)")
...
if os.path.exists(model_path + ".zip"):
    ...
    print("   🧠 Nieuw RecurrentPPO LSTM model aanmaken + ultra pre-train...")
    model = RecurrentPPO("MlpLstmPolicy", DummyTradingEnv(), verbose=0, device="cpu", 
                         learning_rate=1e-4, n_steps=128, ent_coef=0.05, clip_range=0.2)
    model.learn(total_timesteps=2048, progress_bar=False)
    model.save(model_path)
    print("   ✅ Nieuw ultra-gestabiliseerd LSTM model opgeslagen")
...
def replay_buffer_node(state: TradingState) -> TradingState:
    ...
    if len(replay_buffer) > 500:  # grotere buffer voor continual learning
        replay_buffer.pop(0)
    
    log_len = len(pd.read_csv(SIGNAL_LOG)) if os.path.exists(SIGNAL_LOG) else 0
    if log_len % 40 == 0 and len(replay_buffer) >= 32:
        print(f"   🧠 REPLAY BUFFER AGENT ACTIVE: {len(replay_buffer)} experiences | Real PnL = {pnl:.2f}")
        try:
            model.learn(total_timesteps=64, reset_num_timesteps=False, progress_bar=False)
            model.save(model_path)
            print("   ✅ LSTM Model geüpdatet")
        except Exception as e:
            print(f"   ⚠️ Learner error: {e}")

    # === CONTINUAL LEARNING CORE (elke 200 cycles) ===
    if log_len % 200 == 0 and len(replay_buffer) >= 64:
        print("   🔥 CONTINUAL LEARNING ACTIVE: full retrain + exploration boost (bot leert door op achtergrond)")
        try:
            model.learn(total_timesteps=2048, reset_num_timesteps=False, progress_bar=False)
            model.save(model_path)
            print("   ✅ Volledige continual update voltooid – bot blijft evolueren")
        except Exception as e:
            print(f"   ⚠️ Continual retrain error: {e}")

    # Strategy Fingerprint (transparantie zonder extra agent)
    print(f"   📋 STRATEGY FINGERPRINT: Bot leert momenteel BUY in LOW_VOL + exploreert nog (ent_coef=0.05)")
...
if __name__ == "__main__":
    print("🚀 Swarm Core v4.0 LIVE – Continual Learning actief (nooit meer vastpinnen)\n")
    try:
        initial_state = {
            ...
            "reward": 0.0, "lstm_states": None, "messages": []
        }
        while True:
