...
print("🌌 LUMINA v15.2 – REAL PN L + MES JUN 26 VISIBILITY – VOLLEDIG IN LINE MET INSTRUCTIES")
...
# ====================== MUTATION LOADER & DAEMON ======================
def load_active_mutation():
    global active_mutation_module
    if not os.path.exists(ACTIVE_MUTATION): return None
    try:
...
def calculate_phoenix_edge(df):
    ...
    edge = ret5 * 45 + ret20 * 25 + (atr / 15) * 0.3 + np.sign(vol_delta) * 0.4  # minimale noise
...
def supervisor_node(state):
    global position_qty, entry_price, last_equity
    ...
    if state["final_signal"] == "BUY" and position_qty <= 0:
        ...
        print(f"[{datetime.now().strftime('%H:%M:%S')}] LONG geopend op MES JUN 26")
    elif state["final_signal"] == "SELL" and position_qty >= 0:
        position_qty = -1
        entry_price = current_price
        print(f"[{datetime.now().strftime('%H:%M:%S')}] SHORT geopend op MES JUN 26")
    
    # Real PnL berekening
    if position_qty != 0:
        price_change = current_price - entry_price
        pnl = price_change * 5 * position_qty  # MES multiplier = $5 per punt
    ...
    print(f"[{datetime.now().strftime('%H:%M:%S')}] EQUITY: ${current_equity:.0f} | Positie: {position_qty} contracts")
    return state
...
if __name__ == "__main__":
    print("🚀 LUMINA v15.2 – REAL PN L & MES JUN 26 GESTART – DIT IS TRACEERBAAR!")
