...
# =============================================================================
# v41 NATUURLIJKE TTS + CONVERSATIONAL UPGRADE
# =============================================================================
VOICE_ENABLED = os.getenv("VOICE_ENABLED", "True").lower() == "true"
TTS_LANGUAGE = "nl"   # "nl" voor Nederlands, "en" voor Engels
tts_engine = pyttsx3.init() if VOICE_ENABLED else None

if tts_engine:
    tts_engine.setProperty('rate', 172)      # iets natuurlijker tempo
    tts_engine.setProperty('volume', 0.95)
    
    # Probeer Nederlandse stem te kiezen (als beschikbaar op je systeem)
    try:
        voices = tts_engine.getProperty('voices')
        if voices:
            # Probeer itereren over voices
            for voice in voices:  # type: ignore
                ...
                print(f"[{datetime.now().strftime('%H:%M:%S')}] ⚠️ Geen Nederlandse stem gevonden – fallback naar default")
    except Exception as e:
        print(f"[{datetime.now().strftime('%H:%M:%S')}] ⚠️ Fout bij voice detectie: {e} – fallback naar default")
    
    # FUTURE-PROOF NOTE (voor v42+):
    # Als pyttsx3 niet natuurlijk genoeg is, kunnen we hier later ElevenLabs of Azure TTS integreren
    # zonder de rest van de code te breken (alleen speak() functie aanpassen).
...
# =============================================================================
performance_log = []   # lijst van dicts met trade + regime + strategy
...
# TTS_LANGUAGE is al gedefinieerd in v41 TTS UPGRADE section hierboven
...
def get_high_impact_news():
    ...
    try:
        ...
        if r.status_code == 200:
            ...
            # Simpele sentiment-bepaling (kan later met LLM verbeterd worden)
            sentiment = "neutral"
...
def pre_dream_daemon():
    ...
    while True:
        ...
        try:
            ...
            # v40 regime wordt al gebruikt in world_model
            
            # v43 News Sentiment
            news_data = get_high_impact_news()
            news_impact = NEWS_IMPACT_MULTIPLIERS.get(news_data["overall_sentiment"] + "_" + news_data["impact"], 1.0)
            current_dream["news_impact"] = news_impact
...
Geef ALLEEN JSON met: reflection (max 400 chars), key_lesson, suggested_bible_update"""},
        ...
        if r.status_code == 200:
            ...
            # v42: GEEN automatische feedback-vraag meer (24/7 run)
            # Feedback blijft mogelijk via commando op elk moment
            print(f"[{datetime.now().strftime('%H:%M:%S')}] 📝 Trade gesloten – feedback kan later gegeven worden via stem")
...
def speak(text: str):
    ...
    if not VOICE_ENABLED or not tts_engine:
        return
    try:
        # Maak zinnen natuurlijker (korte pauzes)
...
def voice_listener_thread():
    """Natuurlijke conversational voice input met wake-word + feedback erkenning (ook op historische trades)"""
    if not VOICE_INPUT_ENABLED or not voice_recognizer:
        return
    
    print(f"[{datetime.now().strftime('%H:%M:%S')}] 🎤 Voice Input actief – zeg 'Lumina' + commando of feedback (ook later op trades)")
    
    while True:
        try:
            with sr.Microphone() as source:
                voice_recognizer.adjust_for_ambient_noise(source, duration=0.8)
                audio = voice_recognizer.listen(source, timeout=10, phrase_time_limit=8)
            
            text = voice_recognizer.recognize_google(audio, language="nl-NL")  # type: ignore[attr-defined]
            text_lower = text.lower().strip()
            
            print(f"🎤 JIJ: {text}")
            
            # Wake-word detectie (natuurlijk gesprek)
            if VOICE_WAKE_WORD in text_lower:
                command = text_lower.split(VOICE_WAKE_WORD, 1)[1].strip()
                
                # Bestaande commando's
                if any(x in command for x in ["status", "hoe gaat het", "wat is de stand"]):
                    speak(f"Huidige equity is {account_equity:,.0f} dollar. Open PnL is {open_pnl:,.0f}. We zitten in {TRADE_MODE.upper()} modus.")
                
                elif any(x in command for x in ["ga long", "koop", "long"]):
                    current_dream["signal"] = "BUY"
                    current_dream["confluence_score"] = 0.95
                    speak("Oké, ik forceer een long positie. Wil je dat ik dat meteen uitvoer?")
                    ...
                    current_dream["confluence_score"] = 0.95
                    speak("Oké, ik forceer een short positie. Bevestig je dat?")
                    ...
                    speak(f"Mijn huidige dream is {current_dream.get('chosen_strategy', 'onbekend')} met signaal {current_dream.get('signal')} en confidence {current_dream.get('confluence_score',0):.2f}.")
                
                # v42 FEEDBACK (optioneel en later mogelijk op historische trades)
                elif "feedback" in command:
                    # Probeer tijd of "laatste trade" te herkennen
                    if any(x in command for x in ["laatste", "vorige", "laatst"]):
                        trade_data = trade_log[-1] if trade_log else {"signal": current_dream.get("signal")}
                        feedback_msg = f"Feedback op laatste trade: {command}"
                    else:
                        trade_data = {"signal": current_dream.get("signal")}
                        feedback_msg = command
                    
                    reason = command.split("omdat", 1)[1].strip() if "omdat" in command else command
                    process_user_feedback(reason, trade_data)
                    speak("Bedankt voor de feedback. Ik pas mijn bible aan.")
                
                # Snelle feedback-alternatieven (zonder 'feedback' keyword)
                elif any(x in command for x in ["goed", "goed trade", "goede trade", "was goed"]):
                    process_user_feedback("Dit was een goede trade", {"signal": current_dream.get("signal")})
                    speak("Bedankt voor de positieve feedback. Ik pas mijn strategie aan.")
                
                elif any(x in command for x in ["slecht", "slechte trade", "was slecht", "niet goed"]):
                    reason = command.split("omdat", 1)[1].strip() if "omdat" in command else "geen specifieke reden gegeven"
                    process_user_feedback(f"Dit was een slechte trade omdat {reason}", {"signal": current_dream.get("signal")})
                    speak("Bedankt voor de feedback. Ik zal dit verbeteren.")
                
                elif any(x in command for x in ["verbeter", "pas aan", "update"]):
                    process_user_feedback(command)
                    speak("Begrepen. Ik update mijn bible direct.")
                
                else:
                    speak("Ik heb je gehoord, maar ik begrijp het commando niet helemaal. Probeer: status, ga long, ga short, hold, dream of feedback geven.")
            
            # Zonder wake-word: vriendelijke fallback
            elif len(text_lower) > 3:
                speak("Ik luister nog steeds. Zeg 'Lumina' gevolgd door je commando.")
...
def start_dashboard():
    ...
    app.layout = dbc.Container([
        ...
        html.H3("Strategy Heatmap – Winrate per Regime"),
    ...
    def update_dashboard(_):
        ...
        # Heatmap
        heatmap_fig = generate_strategy_heatmap() or go.Figure()
        
        return fig_chart, fig_equity, status, table_header + table_body, heatmap_fig
...
def auto_journal_daemon():
    """Draait dagelijks een professionele PDF + weekly review"""
    while True:
        time.sleep(86400)  # elke 24 uur (of pas aan naar 3600 voor testen)
...
def update_performance_log(trade_data: dict):
    ...
    # Houd alleen laatste 500 trades
...
def generate_professional_pdf_journal():
    ...
    try:
        ...
        r = requests.post("https://api.x.ai/v1/chat/completions", headers={"Authorization": f"Bearer {XAI_KEY}"}, json=weekly_payload, timeout=20)
        if r.status_code == 200:
...
def process_user_feedback(feedback_text: str, trade_data: dict | None = None):
    """Verwerkt feedback van de gebruiker en past de bible aan.
    
    FUTURE-PROOF NOTE (voor v43+):
    - Feedback wordt opgeslagen in vector-DB voor langetermijnleren.
    - Later kunnen we dit uitbreiden naar reinforcement learning of automatische fine-tuning.
    """
    print(f"[{datetime.now().strftime('%H:%M:%S')}] 👤 Feedback ontvangen: {feedback_text[:80]}...")
    ...
    # Stuur feedback naar Grok voor intelligente bible-update
    payload = {
        "model": "grok-4.20-0309-reasoning",
        "messages": [
            {"role": "system", "content": "Je bent een trading-coach. Verwerk de user feedback en stel concrete, bruikbare updates voor de evolvable_layer voor. Geef ALLEEN JSON."},
            {"role": "user", "content": f"""User feedback: {feedback_text}
Laatste trade: {trade_data}
Huidige evolvable_layer: {json.dumps(bible['evolvable_layer'])}
Stel verbeteringen voor."""}
    ...
    try:
        ...
        if r.status_code == 200:
            resp = r.json()["choices"][0]["message"]["content"]
            update = json.loads(resp)
            if update.get("suggested_bible_updates"):
                bible["evolvable_layer"].update(update["suggested_bible_updates"])
                with open(BIBLE_FILE, 'w', encoding='utf-8') as f:
                    json.dump(bible, f, ensure_ascii=False, indent=2)
                print(f"[{datetime.now().strftime('%H:%M:%S')}] 🔥 Bible geüpdatet op basis van je feedback")
    except Exception as e:
...
def supervisor_loop():
    ...
    while True:
        ...
        # === v40 ADAPTIVE RISK & POSITION SIZING ===
        if signal in ["BUY", "SELL"] and current_dream.get("confluence_score", 0) > MIN_CONFLUENCE:
            regime = current_dream.get("regime", "NEUTRAL")
            stop_price = current_dream.get("stop", price * 0.99 if signal == "BUY" else price * 1.01)
            qty = calculate_adaptive_risk_and_qty(price, regime, stop_price)
            ...
            if TRADE_MODE == "paper":
                ...
                # sim of real → echte order
                if place_order(signal, qty):
                    print(f"[{now.strftime('%H:%M:%S')}] ✅ {TRADE_MODE.upper()} {signal} {qty}x @ {price:.2f} (regime-adapted)")
        ...
        # === TRADE CLOSE DETECTION (stops + targets) ===
        if sim_position_qty != 0:
            stop = current_dream.get("stop", 0)
            target = current_dream.get("target", 0)
            ...
            if hit_stop or hit_target:
                ...
                if equity_curve[-1] > sim_peak:
                    sim_peak = equity_curve[-1]
                
                # Log trade
                trade_log.append({
                    "ts": now.isoformat(),
                    "signal": current_dream.get("signal"),
                    "entry": sim_entry_price,
                    "exit": price,
                    "qty": sim_position_qty,
                    "pnl": pnl_dollars,
                    "confluence": current_dream.get("confluence_score", 0)
                })
                
                # Reflection
                reflect_on_trade(pnl_dollars, sim_entry_price, price, abs(sim_position_qty))
                
                # v38 Performance logging
                regime = current_dream.get("regime", "NEUTRAL")
                update_performance_log({
                    ...
                    "confluence": current_dream.get("confluence_score", 0),
                    "pnl": pnl_dollars,
                    "drawdown": (sim_peak - equity_curve[-1]) / sim_peak if sim_peak else 0
                })
