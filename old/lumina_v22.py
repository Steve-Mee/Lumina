    ...
    while True:
        try:
            ...
            narrative_memory.append({
                ...
                "narrative": f"Price {price:.2f} | Regime: {regime} | {pa_summary[:150]}"
            })
...
Je kijkt nu echt naar de chart zoals een mens: met market structure (BOS/CHOCH), order blocks, FVG, regime en narrative context.
Gebruik HUMAN PLAYBOOK + dynamic confluence.
...
Regime: {regime} (dynamic min confluence = {min_conf:.2f})
Market Structure: BOS={structure['bos']}, CHOCH={structure['choch']}, OrderBlocks={structure['order_blocks'][:2]}, FVG={structure['fvg']}
MTF data: {mtf_data}
Swing High: {swing_high:.2f} | Swing Low: {swing_low:.2f}
Fib levels: {fib_levels}
Price Action: {pa_summary}
Narrative (laatste 8 context): {narrative_text or "Eerste run"}
High-impact nieuws: {news_info}
Human Playbook: {bible['sacred_core']}
Wat is je trade? Denk als een ervaren trader die de chart ziet en een verhaal bouwt."""}
                ]
            }
            ...
            if r.status_code == 200:
                ...
                narrative_memory.append({
                    "ts": datetime.now().isoformat(),
                    "narrative": f"DREAM: {current_dream.get('chosen_strategy')} -> {current_dream['signal']} | Conf {current_dream.get('confluence_score',0):.2f} | Regime {regime}"
                })
                log_thought({"type": "dream_decision", **dream_json})
                print(f"[{datetime.now().strftime('%H:%M:%S')}] 🧠 v22 HUMAN DREAM: {current_dream.get('chosen_strategy')} -> {current_dream['signal']} (conf={min_conf:.2f})")
        except Exception as e:
