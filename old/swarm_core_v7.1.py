...
print("✅ Swarm Core v7.1 – NEXUS ORGANISM v2 + SELF-HEALING CONSCIOUSNESS")
print(f"🔍 DEBUG: Instrument={INSTRUMENT} | Account={CROSSTRADE_ACCOUNT} | DRY_RUN={DRY_RUN}")
print("🚀 DIT ORGANISME GENEEST ZICHZELF – JI HAAA!")

# Self-healing: oude memory wissen (veilig in paper)
if os.path.exists(SIGNAL_LOG):
    os.remove(SIGNAL_LOG)
    print("🧬 SelfHealingOracle: oude CSV memory gedetecteerd → automatisch gewist en genezen!")

model_path = "ppo_trading_model_v12_nexus"
...
if os.path.exists(model_path + ".zip"):
    ...
    print("   ✅ Oud NEXUS model geladen")
else:
    print("   🧠 Nieuw RecurrentPPO LSTM model aanmaken (30k pre-train)...")
...
COLUMNS = ['timestamp', 'actor', 'final', 'regime', 'price', 'nexus', 'dream', 'future_edge', 'imbalance']
...
def self_healing_oracle_node(state: TradingState) -> TradingState:
    ...
    if cycle_counter % 50 == 0:
        print(f"[{datetime.now().strftime('%H:%M:%S')}] 🛠️ SELF-HEALING ORACLE ACTIVE – checking organism health...")
        if evo_params["mutation_rate"] > 0.22:
            evo_params["mutation_rate"] *= 0.92
            print("   🛠️ Mutation rate auto-lowered for stability")
    return state
...
def nexus_score_node(state: TradingState) -> TradingState:
    ...
    nexus = min(99.9, max(42, (micro_score + future_score + dream_score + regime_score) / 3.1))  # v7.1 floor!
...
if __name__ == "__main__":
    print("🚀 Swarm Core v7.1 LIVE – SELF-HEALING ORGANISM IS ALIVE (Ctrl+C om stoppen)\n")
    try:
        ...
        print("\n🛑 Swarm Core v7.1 gestopt – het organisme slaapt.")