...
print(f"🌌 LUMINA v21.2 – HISTORICAL BARS + FINNHUB NEWS + AUTOMATIC GAP RECOVERY")
...
""",
        "evolvable_layer": {
            ...
            "last_reflection": "2026-03-26: v21.2 Automatic Gap Recovery + Full Bar Continuity",
            "lessons_learned": []
        }
    }
        ...
        if new_entries:
            ...
            with open(LIVE_JSONL, "a", encoding="utf-8") as f:
                for entry in new_entries:
                    f.write(json.dumps({**entry, "current_dream": current_dream}) + "\n")
            print(f"✅ GAP RECOVERY: {len(new_entries)} nieuwe 5-min bars toegevoegd")
            return len(new_entries)
    return 0

def fetch_historical_bars(days_back=3, limit=1500):
    print(f"📥 Ophalen {limit} historische 5-min bars (laatste {days_back} dagen)...")
    try:
        payload = {
        ...
        if r.status_code == 200:
            ...
            print(f"✅ {len(bars)} bars opgehaald → {added} nieuwe toegevoegd")
            return True
        else:
    ...
    while True:
        ...
        try:
            ...
            if max_gap > 360:
                ...
                fetch_historical_bars(days_back=3, limit=1500)
            else:
                fetch_historical_bars(days_back=3, limit=1500)
        except Exception as e:
...
def pre_dream_daemon():
    ...
    if len(live_data) < 200:
        fetch_historical_bars(days_back=5, limit=800)
...
if __name__ == "__main__":
    print("🚀 LUMINA v21.2 – HISTORISCHE BARS + FINNHUB NEWS + AUTOMATIC GAP RECOVERY GESTART")
    ...
    threading.Thread(target=pre_dream_daemon, daemon=True).start()   # ← nu ook hier
    try:
        while True:
            time.sleep(60)
    except KeyboardInterrupt:
        save_state()
        print("\n🛑 LUMINA v21.2 gestopt – state opgeslagen.")
