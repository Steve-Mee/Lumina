...
def voice_listener_thread():
    """Natuurlijke conversational voice input met wake-word"""
    if not VOICE_INPUT_ENABLED or not voice_recognizer:
        return
    
    print(f"[{datetime.now().strftime('%H:%M:%S')}] 🎤 Natural Voice Input actief – zeg gewoon 'Lumina' + commando")
        ...
        try:
            ...
            if VOICE_WAKE_WORD in text_lower:
                ...
                if any(x in command for x in ["status", "hoe gaat het", "wat is de stand"]):
                    ...
                    speak("Ik heb je gehoord, maar ik begrijp het commando niet helemaal. Probeer: status, ga long, ga short of hold.")
