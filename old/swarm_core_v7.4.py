...
print("✅ Swarm Core v7.4 – NEXUS ORGANISM + DUIDELIJKE LOGS + SLIMME CSV-UPGRADE")
print(f"🔍 DEBUG: Instrument={INSTRUMENT} | Account={CROSSTRADE_ACCOUNT} | DRY_RUN={DRY_RUN}")

# Slimme CSV-upgrade: oude log backuppen als kolommen niet kloppen
if os.path.exists(SIGNAL_LOG):
    try:
        ...
        if list(existing.columns) != ['timestamp', 'actor', 'final', 'regime', 'price', 'nexus', 'dream', 'future_edge', 'imbalance', 'world_knowledge']:
            print("🛠️ Oude log gevonden met andere kolommen → backup gemaakt")
            shutil.copy(SIGNAL_LOG, BACKUP_LOG)
            os.remove(SIGNAL_LOG)
    except:
        os.remove(SIGNAL_LOG)

model_path = "ppo_trading_model_v15_nexus"
...
def world_knowledge_oracle_node(state: TradingState) -> TradingState:
    ...
    if cycle_counter % 8 == 0 and XAI_KEY:
        ...
        try:
            payload = {"model": "grok-4.20-0309-reasoning", "messages": [
                {"role": "system", "content": "Geef een korte samenvatting (max 15 woorden) van het huidige sentiment en belangrijke nieuws rond MES / S&P 500 futures."},
            ...
            print(f"[{datetime.now().strftime('%H:%M:%S')}] World Knowledge: geen nieuwe info (gebruik vorige)")
    return {**state, "world_knowledge": world_knowledge}

def imagination_oracle_node(state: TradingState) -> TradingState:
    print(f"[{datetime.now().strftime('%H:%M:%S')}] Imagination Oracle: 32 toekomstscenario's berekenen...")
    df = pd.read_csv(CSV_FILE).tail(100) if os.path.exists(CSV_FILE) else pd.DataFrame()
    ...
    print(f"[{datetime.now().strftime('%H:%M:%S')}] Imagination Oracle: future_edge = {future_edge:.3f}%")
...
def self_healing_oracle_node(state: TradingState) -> TradingState:
    ...
    if cycle_counter % 50 == 0:
        ...
        if evo_params["mutation_rate"] > 0.22:
            ...
            print("   Self-Healing: mutation_rate verlaagd voor stabiliteit")
    return state
...
def evolution_oracle_node(state: TradingState) -> TradingState:
    ...
    if cycle_counter % 80 == 0 and cycle_counter > 100:
        ...
        print(f"   Evolution: nieuwe drift_base = {evo_params['drift_base']:.5f}")
    return state
...
def dream_memory_node(state: TradingState) -> TradingState:
    ...
    print(f"[{datetime.now().strftime('%H:%M:%S')}] Dream Memory: {len(dream_memory)} succesvolle dromen | gemiddelde edge = {dream_edge:.3f}")
...
def nexus_score_node(state: TradingState) -> TradingState:
    ...
    world_score = 25 if any(x in state.get("world_knowledge", "").lower() for x in ["bullish", "bearish", "news"]) else 12
...
def supervisor_node(state: TradingState) -> TradingState:
    print(f"[{datetime.now().strftime('%H:%M:%S')}] Supervisor: Nexus Score = {state.get('nexus_score', 0):.1f} | World: {state.get('world_knowledge', 'Stil')}")
    print(f"[{datetime.now().strftime('%H:%M:%S')}] Actor signaal: {state['actor_signal']} → Final: {state['final_signal']}")
    ...
    if state["is_market_open"] and state["final_signal"] != "HOLD":
        ...
        print(f"[{datetime.now().strftime('%H:%M:%S')}] Order: {side} {quantity} contracts (paper)")
        if not DRY_RUN:
            try:
...
def replay_buffer_node(state: TradingState) -> TradingState:
    ...
    if log_len % 40 == 0:
        ...
        model.save(model_path)
    if log_len % 200 == 0:
        print(f"[{datetime.now().strftime('%H:%M:%S')}] Replay Buffer: continual learning actief (4096 steps)")
...
if __name__ == "__main__":
    print("🚀 Swarm Core v7.4 LIVE – Duidelijke logs actief (Ctrl+C om stoppen)\n")
    try:
