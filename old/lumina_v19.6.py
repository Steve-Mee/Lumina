...
print("🌌 LUMINA v19.6 – WEBSOCKET STREAMING + ECHTE OHLC BARS (exact NinjaTrader)")
...
""",
        "evolvable_layer": {
            ...
            "last_reflection": "2026-03-24: WebSocket fixed + echte OHLC bars"
        }
    }
...
live_data = []  # lijst van {"timestamp": datetime, "last": float, "volume": int}
...
async def websocket_listener():
    if not CROSSTRADE_TOKEN:
        raise ValueError("❌ CROSSTRADE_TOKEN niet gevonden in .env!")
    ...
    try:
        async with websockets.connect(
            uri,
            additional_headers=headers,   # <-- correct sinds websockets 14+
            ping_interval=20,
            ping_timeout=20
        ) as ws:
            await ws.send(json.dumps({"action": "subscribe", "instruments": [INSTRUMENT]}))
            print(f"[{datetime.now().strftime('%H:%M:%S')}] ✅ WebSocket verbonden – live updates ~1 sec")
                    ...
                    if data.get("type") == "marketData":
                        for quote in data.get("quotes", []):
                            if quote.get("instrument") == INSTRUMENT:
                                ...
                                if len(live_data) > 10000:
                                    live_data.pop(0)
                except Exception as e:
                    logger.error(f"WS parse error: {e}")

    except Exception as e:
        print(f"[{datetime.now().strftime('%H:%M:%S')}] ⚠️ WebSocket connectie mislukt ({e}) – val terug op REST polling")
        # fallback naar REST polling
        while True:
            try:
                price, vol = fetch_quote()
                entry = {"timestamp": datetime.now(), "last": price, "volume": vol}
                live_data.append(entry)
                if len(live_data) > 10000:
                    live_data.pop(0)
            except:
...
def get_mtf_snapshots():
    ...
    for tf_name, seconds in TIMEFRAMES.items():
        ...
        print(f"[{datetime.now().strftime('%H:%M:%S')}] MTF → {tf_name} exact {seconds}s | O:{ohlc['open']:.2f} H:{ohlc['high']:.2f} L:{ohlc['low']:.2f} C:{ohlc['close']:.2f} V:{ohlc['volume']}")
    ...
    while True:
        try:
            ...
            payload = {
                ...
                "messages": [
                    {"role": "system", "content": f"Je bent LUMINA's brein. Je krijgt nu VOLLEDIGE OHLC bars via streaming (exact zoals NinjaTrader). Sacred Core is HEILIG.\n\nSacred Core:\n{bible['sacred_core']}\n\nEvolvable layer:\n{json.dumps(bible['evolvable_layer'], ensure_ascii=False)}\n\nGeef ALLEEN JSON."},
...
if __name__ == "__main__":
    print("🚀 LUMINA v19.6 – WEBSOCKET STREAMING GESTART")
