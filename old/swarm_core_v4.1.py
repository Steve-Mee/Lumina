...
print("✅ Swarm Core v4.1 – ECHTE X-Sentiment + Dynamische Fingerprint + Paper-Trade Ready")
...
def get_real_x_sentiment():
    try:
        payload = {"model": "grok-4.20-0309-reasoning", "messages": [
            {"role": "system", "content": "Geef alleen één woord: BULLISH, BEARISH of NEUTRAL. Analyseer live X-sentiment voor MES futures (maart 2026)."},
...
def replay_buffer_node(state: TradingState) -> TradingState:
    ...
    # Dynamische fingerprint (gebaseerd op laatste signals)
    recent_signals = pd.read_csv(SIGNAL_LOG).tail(20)["actor"].value_counts() if os.path.exists(SIGNAL_LOG) else {}
    buy_ratio = recent_signals.get("BUY", 0) / max(1, len(recent_signals))
    fingerprint = f"Bot leert momenteel BUY in LOW_VOL ({buy_ratio:.0%} BUY-ratio) + exploreert nog"
    print(f"   📋 STRATEGY FINGERPRINT: {fingerprint}")
...
if __name__ == "__main__":
    print("🚀 Swarm Core v4.1 LIVE – ECHTE X-Sentiment + Paper-Trade Ready (Ctrl+C om stoppen)\n")
    try:
