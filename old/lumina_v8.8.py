# === lumina_v8.8.py – LUMINA v4.0 + LIVE PERFORMANCE TRACKING + BUG FIXES ===
import os
import time
import pandas as pd
import numpy as np
...
print("✅ lumina_v8.8.py – LUMINA v4 + LIVE PERFORMANCE TRACKING")
...
def microstructure_oracle_node(state: TradingState) -> TradingState:
    ...
    if len(df) < 10:
        ...
        imbalance = np.clip(imbalance, -5, 5)  # fix explosie
...
def supervisor_node(state: TradingState) -> TradingState:
    ...
    # === LIVE PERFORMANCE TRACKING (zicht op resultaat) ===
    position = state.get("position_qty", 0)
    current_price = state["last_price"]
    estimated_pnl = position * (current_price - 6500) * 5 if position != 0 else 0  # geschat PnL
...
if __name__ == "__main__":
    print("🚀 lumina_v8.8 LIVE – LUMINA v4 met LIVE PERFORMANCE TRACKING (Ctrl+C om stoppen)\n")
    try:
