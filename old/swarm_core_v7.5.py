...
print("✅ Swarm Core v7.5 – LUMINA v1 + MULTI-SOURCE WORLD EAR")
...
model_path = "ppo_trading_model_v16_lumina"
...
def world_knowledge_oracle_node(state: TradingState) -> TradingState:
    ...
    if cycle_counter % 8 == 0:
        print(f"[{datetime.now().strftime('%H:%M:%S')}] World Knowledge Oracle: ophalen...")
        try:
...
def imagination_oracle_node(state: TradingState) -> TradingState:
    print(f"[{datetime.now().strftime('%H:%M:%S')}] Imagination Oracle: 32 scenario's berekenen...")
    df = pd.read_csv(CSV_FILE).tail(100) if os.path.exists(CSV_FILE) else pd.DataFrame()
    if len(df) < 20 or state["atr"] == 0:
        ...
        # v7.5: realistische cap
        raw_edge = np.mean([(np.mean([current_price * (1 + np.random.normal(drift, atr * 0.28)) for _ in range(5)]) - current_price) / current_price for _ in range(32)]) * 100
        future_edge = np.clip(raw_edge, -8.0, 8.0)
...
def supervisor_node(state: TradingState) -> TradingState:
    print(f"[{datetime.now().strftime('%H:%M:%S')}] Supervisor: Nexus Score = {state.get('nexus_score', 0):.1f} | World Knowledge = {state.get('world_knowledge', 'Stil')}")
...
if __name__ == "__main__":
    print("🚀 Swarm Core v7.5 LIVE – LUMINA is wakker (Ctrl+C om stoppen)\n")
    try:
