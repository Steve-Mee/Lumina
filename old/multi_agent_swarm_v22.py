...
print("✅ Multi-Agent Swarm v22 – PRIMARY_INSTRUMENT + GROK API PROOF PRINT (v3.2 compliant)")
print(f"🔍 DEBUG: Primary Instrument={INSTRUMENT} | Account={CROSSTRADE_ACCOUNT} | DRY_RUN={DRY_RUN}")
...
def fetch_quote():
    # PRIMARY_INSTRUMENT first + max 3 attempts (tijdwinst)
    formats = [INSTRUMENT, "MES1!", "MES JUN 26"]
    for fmt in formats:
        try:
            ...
            if r.status_code == 200:
                ...
                print(f"   ⚠️ CrossTrade {r.status_code} met '{fmt}'")
        except Exception as e:
            print(f"   ⚠️ CrossTrade exception: {e}")
    ...
    for tick in ["MES=F", "ES=F"]:
        try:
            ...
            if not data.empty:
                ...
                if volume < 50000:
                    volume = 150000
                print(f"   ✅ yfinance SUCCESS met {tick} | Price: {price:.2f} | Vol: {volume}")
                return price, volume, f"yfinance ({tick})"
        except:
            pass

    print("   ❌ Alles gefaald → mock")
...
def regime_oracle_node(state: TradingState) -> TradingState:
    ...
    if not df.empty:
        df = df.drop_duplicates(subset=['last'], keep='last')
    ...
    for attempt in range(3):
        try:
            ...
            r = requests.post("https://api.x.ai/v1/chat/completions",
                              headers={"Authorization": f"Bearer {XAI_KEY}"}, json=payload, timeout=10)
            if r.status_code == 200:
                advice = r.json()["choices"][0]["message"]["content"]
                print(f"   🔥 GROK API SUCCESS - Real xAI Devil’s Advocate reasoning received at {datetime.now().strftime('%H:%M:%S')} - Model: grok-4.20-0309-reasoning")
                break
            else:
                print(f"   ⚠️ Grok API {r.status_code}")
        except Exception as e:
            print(f"   ⚠️ Grok API exception (attempt {attempt+1}): {str(e)[:80]}")
    veto = []
...
def supervisor_node(state: TradingState) -> TradingState:
    print(f"\n📊 Regime Oracle: {state['regime']} (ATR {state['atr']:.2f}) | Data Source: {state['data_source']}")
...
if __name__ == "__main__":
    print("🚀 v22 Swarm LIVE – GROK API PROOF + PRIMARY_INSTRUMENT (Ctrl+C om stoppen)\n")
    try:
        while True:
            initial_state = {"obs": [0,0,0,0,0], "regime": "", "actor_signal": "", "critic_reasoning": "", "critic_veto": [], "final_signal": "", "last_price": 0, "volume": 0, "position_qty": 0, "atr": 12.0, "data_source": "", "messages": []}
