...
print("🌌 LUMINA v17.0 – PHOENIX UPGRADE: RSI + MACD + REGIME + TP/SL SIMULATION")
...
def get_market_status():
    try:
        ...
        print(f"[{datetime.now().strftime('%H:%M:%S')}] MARKET STATUS: {'OPEN' if status else 'CLOSED'} (MES JUN 26 futures)")
        return status
    except: return False

def calculate_phoenix_edge(df):
    if len(df) < 10: return np.random.normal(0, 0.1)
    df = df.copy()
    # RADICAAL VERBETERD: RSI + MACD + REGIME
    delta = df['last'].diff()
    ...
    macd_value = macd.iloc[-1] - macd_signal.iloc[-1]
    regime = "HIGH_VOL" if df['last'].diff().abs().rolling(14).mean().iloc[-1] > 15 else "LOW_VOL"
    regime_factor = 1.2 if regime == "HIGH_VOL" else 0.8
    rsi_factor = (50 - rsi_value) / 50 * 1.2 if rsi_value > 50 else (rsi_value - 50) / 50 * -1.2
    edge = macd_value * 35 + rsi_factor * 30 + regime_factor * 0.5
...
def supervisor_node(state):
    ...
    if SIMULATE_TRADES:
        ...
        if sim_position_qty != 0:
            unrealized_pct = (current_price - sim_entry_price) / sim_entry_price * 100 * sim_position_qty
            if unrealized_pct > 0.5 or unrealized_pct < -0.3:
                pnl = (current_price - sim_entry_price) * 5 * sim_position_qty
                print(f"[{datetime.now().strftime('%H:%M:%S')}] SIMULATED: TP/SL TRIGGERED | PnL: {pnl:+.0f}")
                sim_position_qty = 0
        ...
        if sim_position_qty != 0:
            ...
            if len(pnl_history) > 20:
                sharpe = np.mean(pnl_history) / (np.std(pnl_history) + 1e-8) * np.sqrt(252)
                winrate = sum(1 for p in pnl_history if p > 0) / len(pnl_history) * 100
                print(f"[{datetime.now().strftime('%H:%M:%S')}] PERFORMANCE ORACLE: Sharpe = {sharpe:.2f} | Winrate = {winrate:.1f}%")
    else:
...
if __name__ == "__main__":
    print("🚀 LUMINA v17.0 – RSI + MACD + TP/SL SIMULATION GESTART")
