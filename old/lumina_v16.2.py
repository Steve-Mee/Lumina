...
print("🌌 LUMINA v16.2 – PURE LIVE PORTFOLIO VALUE + FULL API DEBUG")
...
# ====================== PURE LIVE API (GEEN FALLBACK 25000 IN DISPLAY) ======================
def fetch_quote():
    try:
...
def get_real_portfolio_value():
    try:
        ...
        print(f"[{datetime.now().strftime('%H:%M:%S')}] DEBUG BALANCE API STATUS: {r.status_code}")
        print(f"[{datetime.now().strftime('%H:%M:%S')}] DEBUG RAW RESPONSE: {r.text}")
        if r.status_code == 200:
            equity = float(r.json().get("equity", 0))
            print(f"[{datetime.now().strftime('%H:%M:%S')}] LIVE PORTFOLIO VALUE: ${equity:.2f}")
            return equity
        else:
            print(f"[{datetime.now().strftime('%H:%M:%S')}] ⚠️ BALANCE API FAILED – raw response: {r.text}")
            return 0
    except Exception as e:
        print(f"[{datetime.now().strftime('%H:%M:%S')}] ⚠️ BALANCE API EXCEPTION: {e}")
        return 0
...
if __name__ == "__main__":
    print("🚀 LUMINA v16.2 – PURE LIVE MODE + FULL DEBUG GESTART")
