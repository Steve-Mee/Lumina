# === lumina_v8.6.py – LUMINA v4.0 + ZICHTBARE FIRST PRINCIPLES ONTLEDING ===
import os
import time
import pandas as pd
import numpy as np
...
print("✅ lumina_v8.6.py – LUMINA v4 + ZICHTBARE FIRST PRINCIPLES ONTLEDING")
...
def first_principles_oracle_node(state: TradingState) -> TradingState:
    ...
    momentum_score = (df['last'].pct_change(5).iloc[-1] * 100) if len(df) > 5 else 0
    ...
    if len(predicted_edges) > 30:
        ...
        impact = abs(corr) * 100
        print(f"   🌍 Universe Effectiveness (First Principles): corr={corr:.3f} | impact={impact:.1f}%")
...
def nexus_score_node(state: TradingState) -> TradingState:
    ...
    future_score = abs(state.get("imagined_future_edge", 0)) * 22
...
if __name__ == "__main__":
    print("🚀 lumina_v8.6 LIVE – LUMINA v4 met ZICHTBARE First Principles ontleding (Ctrl+C om stoppen)\n")
    try:
