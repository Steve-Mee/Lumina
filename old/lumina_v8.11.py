# === lumina_v8.11.py – LUMINA v4.0 + EXACTE 4S CYCLES + PAPER TRADE FIX ===
import os
import time
import pandas as pd
import numpy as np
...
print("✅ lumina_v8.11.py – LUMINA v4 + EXACTE 4S CYCLES + PAPER TRADE FIX")
...
def supervisor_node(state: TradingState) -> TradingState:
    ...
    # ALLEEN echte BUY/SELL uitvoeren (geen false PAPER TRADE meer!)
...
if __name__ == "__main__":
    print("🚀 lumina_v8.11 LIVE – LUMINA v4 met EXACTE 4S cycles + PAPER TRADE fix (Ctrl+C om stoppen)\n")
    try:
        ...
        while True:
            ...
            print(f"[{datetime.now().strftime('%H:%M:%S')}] ⏱️ Cycle Time: {elapsed:.2f}s → sleep {sleep_time:.2f}s")
