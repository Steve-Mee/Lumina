...
def start_dashboard():
    ...
    app.layout = dbc.Container([
        ...
        dcc.ConfirmDialog(
            id="shutdown-confirm",
            message="Weet je zeker dat je LUMINA volledig wilt afsluiten?",
        ),
        dcc.Store(id="close-browser-signal", data=0),
        html.Div(id="close-browser-action", style={"display": "none"}),
    ...
    def update_dashboard(_):
        ...
        fig_chart = go.Figure()  # Placeholder chart; can be replaced with full image rendering.
        if chart_base64:
            # Keep dashboard responsive by rendering a lightweight placeholder.
    ...
    app.clientside_callback(
        ...
        """,
        Output("close-browser-action", "children"),
        Input("close-browser-signal", "data"),
        prevent_initial_call=True,
    )

    @app.callback(
        Output("shutdown-feedback", "children"),
        Output("close-browser-signal", "data"),
        Input("shutdown-confirm", "submit_n_clicks"),
        Input("shutdown-confirm", "cancel_n_clicks"),
        prevent_initial_call=True,
    )
    def shutdown_from_dashboard(submit_n_clicks, cancel_n_clicks):
        submit_count = submit_n_clicks or 0
        cancel_count = cancel_n_clicks or 0

        if cancel_count >= submit_count and cancel_count > 0:
            return "Afsluiten geannuleerd.", no_update

        if submit_count > cancel_count:
            print(f"[{datetime.now().strftime('%H:%M:%S')}] 🛑 Shutdown button confirmed from dashboard")
            threading.Thread(target=delayed_emergency_stop, args=(1.2,), daemon=True).start()
            return "App wordt afgesloten...", time.time()
...
def delayed_emergency_stop(delay_seconds: float = 1.2):
    """Delay shutdown shortly so dashboard can close/blank first."""
...
def bootstrap_runtime() -> None:
    ...
    start_runtime_services(
        ...
        pre_dream_daemon_fn=pre_dream_daemon,
