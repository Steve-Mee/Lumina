...
print(f"🌌 LUMINA v21.5 – FORCE 1-MINUTE BARS + ROBUUSTE API PARSING")
...
""",
        "evolvable_layer": {
            ...
            "last_reflection": "2026-03-26: v21.5 Force 1-Minute Bars + Robuuste API Parsing",
            "lessons_learned": []
        }
    }
...
# =============================================================================
# HISTORISCHE BARS + ROBUUSTE PARSING (v21.5 fix)
# =============================================================================
    ...
    with live_data_lock:
        ...
        for bar in bars:
            if isinstance(bar, dict):
                ...
                if ts_str and close is not None:
...
def fetch_historical_bars(days_back=3, limit=3000):
    ...
    try:
        ...
        if r.status_code != 200:
            print(f"❌ API fout: {r.status_code} → {r.text[:300]}")
            return False

        data = r.json()
        print(f"   Response type: {type(data)} | Sample: {str(data)[:300]}...")

        # Robuuste extractie van de bars-lijst
        if isinstance(data, list):
            bars = data
        elif isinstance(data, dict):
            bars = (data.get("bars") or data.get("data") or data.get("result") or data.get("ohlc") or [])
        else:
            bars = []

        if not isinstance(bars, list):
            print("❌ Geen lijst gevonden in response – probeer andere structuur")
            bars = []

        added = merge_historical_bars(bars)
        print(f"✅ SUCCES → {len(bars)} bars verwerkt, {added} nieuwe toegevoegd!")
        return True
    except Exception as e:
...
if __name__ == "__main__":
    print("🚀 LUMINA v21.5 – FORCE INITIAL 1-MINUTE BARS GESTART")
    ...
    try:
        ...
        save_state()
        print("\n🛑 LUMINA v21.5 gestopt – state opgeslagen.")
