...
# Automatische log-creatie (fix voor jouw melding)
...
print("✅ Multi-Agent Swarm v13.2 – Grok-4.20-multi-agent-0309 Connect Test")

# === DIRECTE GROK CONNECTIE TEST (nu met correct model) ===
print("\n🔄 Grok API connectie test...")
if XAI_KEY:
    try:
        test_payload = {
            "model": "grok-4.20-multi-agent-0309",
            "messages": [{"role": "user", "content": "Test: zeg kort 'GROK-4.20-MULTI-AGENT CONNECTIE SUCCES' als je dit leest."}]
        }
        r = requests.post("https://api.x.ai/v1/chat/completions",
                          headers={"Authorization": f"Bearer {XAI_KEY}", "Content-Type": "application/json"},
                          json=test_payload, timeout=10)
        if r.status_code == 200:
            print("✅ GROK-4.20-MULTI-AGENT CONNECTIE SUCCES! Je key werkt perfect.")
...
print("\n🚀 v13.2 Hybride Swarm actief (Ctrl+C om te stoppen)\n")
    ...
    try:
        while True:
            ...
            if counter % 50 == 0 and XAI_KEY:
                print("   🔄 Grok-4.20-multi-agent-0309 activeert...")
                print("   (Grok analyse zou hier komen – connectie is nu correct)")
