...
print("✅ lumina_v9.3.py – LUMINA v5.2 + HOT-RELOAD MUTATIONS + ZERO-DOWNTIME + BAD MUTATION SAFEGUARDS")
...
model_path = "ppo_trading_model_v22_lumina"
...
def performance_oracle_node(state: TradingState) -> TradingState:
    ...
    if sharpe < -1.0:
        print("⚠️ NEGATIEVE SHARPE → extra voorzichtigheid")
...
def load_active_mutation():
    ...
    try:
        ...
        print(f"[{datetime.now().strftime('%H:%M:%S')}] 🔄 HOT-RELOAD: nieuwe mutatie succesvol geladen")
        active_mutation_module = module
        return module
    except Exception as e:
        print(f"[{datetime.now().strftime('%H:%M:%S')}] ⚠️ BAD MUTATION DETECTED → rollback naar vorige versie")
        logger.error(f"Mutation load failed: {traceback.format_exc()}")
        if os.path.exists(ACTIVE_MUTATION + ".backup"):
            shutil.copy(ACTIVE_MUTATION + ".backup", ACTIVE_MUTATION)
        return None
...
def mutation_hot_reload_node(state: TradingState) -> TradingState:
    global last_mutation_hash, active_mutation_module
    if not os.path.exists(ACTIVE_MUTATION):
        return state
    current_hash = hash(open(ACTIVE_MUTATION, 'rb').read())
    if current_hash != last_mutation_hash:
        new_module = load_active_mutation()
        if new_module:
            last_mutation_hash = current_hash
            print(f"[{datetime.now().strftime('%H:%M:%S')}] Mutation geaccepteerd – hot-reload succesvol")
        else:
            print(f"[{datetime.now().strftime('%H:%M:%S')}] Mutation geweigerd – code bevat syntax/runtime error")
    return state

# Voorbeeld hoe je een nieuwe node zou aanroepen als hij bestaat
def call_mutation_oracle(state: TradingState) -> TradingState:
    if active_mutation_module and hasattr(active_mutation_module, 'nieuwe_node'):
        try:
            return active_mutation_module.nieuwe_node(state)
        except:
            print("Mutation oracle failed – skipping")
    return state
...
if __name__ == "__main__":
    print("🚀 lumina_v9.3 LIVE – ZERO-DOWNTIME HOT-RELOAD MUTATIONS (Ctrl+C om stoppen)\n")
    try:
