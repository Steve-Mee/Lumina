# === lumina_v8.5.py – LUMINA v4.0 + FIRST PRINCIPLES ORACLE + ADAPTIVE UNIVERSE IMPACT ===
import os
import time
import pandas as pd
import numpy as np
...
print("✅ lumina_v8.5.py – LUMINA v4 + FIRST PRINCIPLES ORACLE")
...
if model is None:
    print("   🔥 Zware pre-training gestart (30.000 steps) – LUMINA is aan het dromen...")
    ...
    for i in range(0, total_steps, chunk_size):
        ...
        print(f"   🌌 LUMINA droomt... [{i:5d}/{total_steps}] → nu {current} steps bezig")
        model.learn(total_timesteps=chunk_size, progress_bar=False, reset_num_timesteps=False)
    model.save(model_path)
    print("   ✅ Pre-training VOLTOOID – LUMINA is wakker!")
...
# === NIEUWE FIRST PRINCIPLES ORACLE ===
def first_principles_oracle_node(state: TradingState) -> TradingState:
    global predicted_edges, actual_returns
    logger.info("First Principles Oracle: markt ontleden tot basis (aanbod × vraag × momentum)")
    ...
    if len(predicted_edges) > 30:
        ...
        impact = abs(corr) * 100
        logger.info(f"Universe Effectiveness (First Principles): corr={corr:.3f} | impact={impact:.1f}%")
        print(f"[{datetime.now().strftime('%H:%M:%S')}] 🌍 Universe Effectiveness (First Principles): corr={corr:.3f} | impact={impact:.1f}%")
...
# (alle andere nodes uit v8.4 blijven exact hetzelfde – alleen toegevoegd + nexus weging iets hoger)
...
def get_current_position():
    try:
        r = requests.get(f"https://app.crosstrade.io/v1/api/accounts/{CROSSTRADE_ACCOUNT}/positions", headers={"Authorization": f"Bearer {CROSSTRADE_TOKEN}"}, timeout=6)
...
def get_recent_pnl():
    try:
        r = requests.get(f"https://app.crosstrade.io/v1/api/accounts/{CROSSTRADE_ACCOUNT}/executions", headers={"Authorization": f"Bearer {CROSSTRADE_TOKEN}"}, timeout=6)
...
def calculate_technical_features(df):
    if len(df) < 50: return {"rsi": 50, "macd": 0, "momentum": 0}
    df = df.copy()
    delta = df['last'].diff()
...
def world_knowledge_oracle_node(state: TradingState) -> TradingState:
    ...
    if cycle_counter % 8 == 0:
        ...
        try:
            payload = {"model": "grok-4.20-0309-reasoning", "messages": [{"role": "system", "content": "Geef een korte samenvatting (max 12 woorden) van het huidige sentiment en nieuws rond MES / S&P 500 futures."}, {"role": "user", "content": "Wat gebeurt er nu echt in de markt voor MES JUN 26?"}]}
...
def parallel_universe_simulator_node(state: TradingState) -> TradingState:
    ...
    for _ in range(100):
        ...
        edges.append(edge)
    future_edge = np.clip(np.mean(edges), -9.0, 9.0)
    logger.info(f"Parallel Universe Simulator: future_edge = {future_edge:.3f}% (100 werelden)")
    print(f"[{datetime.now().strftime('%H:%M:%S')}] Parallel Universe Simulator: future_edge = {future_edge:.3f}% (100 werelden)")
...
def nexus_score_node(state: TradingState) -> TradingState:
    ...
    future_score = abs(state.get("imagined_future_edge", 0)) * 22   # nog hoger door First Principles
...
def critic_node(state: TradingState) -> TradingState:
    ...
    veto = []
    if "HIGH_VOLATILITY" in state["regime"] and state["actor_signal"] != "HOLD": veto.append("High vol risk")
    if state["x_sentiment"] == "BEARISH" and state["actor_signal"] == "BUY": veto.append("Bearish sentiment veto")
    if state["x_sentiment"] == "BULLISH" and state["actor_signal"] == "SELL": veto.append("Bullish sentiment veto")
    micro = state.get("microstructure", {"imbalance": 0.0})
    if micro["imbalance"] < -1.3 and state["actor_signal"] == "BUY": veto.append("Negative imbalance veto")
    if micro["imbalance"] > 1.3 and state["actor_signal"] == "SELL": veto.append("Positive imbalance veto")
    if state.get("imagined_future_edge", 0) < -1.0 and state["actor_signal"] != "HOLD": veto.append("Negative future_edge veto")
    if "bearish" in state.get("world_knowledge", "").lower() and state["actor_signal"] == "BUY": veto.append("World says bearish")
    if cycle_counter > 120 and state.get("nexus_score", 50) < 35: veto.append("Low consciousness veto")
...
def supervisor_node(state: TradingState) -> TradingState:
    ...
    if len(equity_curve) > 1:
        ...
        if prev_price != 0:
            ...
            if len(predicted_edges) > 20:
                corr = np.corrcoef(predicted_edges[-50:], actual_returns[-50:])[0, 1]
                impact = abs(corr) * 100
                logger.info(f"Universe Effectiveness: correlation = {corr:.3f} | impact = {impact:.1f}%")
                print(f"[{datetime.now().strftime('%H:%M:%S')}] 🌍 Universe Effectiveness: correlation = {corr:.3f} | impact = {impact:.1f}%")
    ...
    if state.get("previous_market_open", True) and not state["is_market_open"]:
        ...
        print(f"[{datetime.now().strftime('%H:%M:%S')}] 🚨 MARKT SLUITING → ALLE POSITIES WORDEN GECLOSET (geen overnight)")
...
workflow.add_edge("parallel_universe", "first_principles")   # NIEUWE RADICALE STAP
...
if __name__ == "__main__":
    print("🚀 lumina_v8.5 LIVE – LUMINA v4 met First Principles Oracle (Ctrl+C om stoppen)\n")
    try:
