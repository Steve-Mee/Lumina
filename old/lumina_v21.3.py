...
print(f"🌌 LUMINA v21.3 – 1-MINUTE HISTORICAL BARS + FULL VISUALIZER OVERVIEW + GAP RECOVERY")
...
""",
        "evolvable_layer": {
            ...
            "last_reflection": "2026-03-26: v21.3 1-Minute Historical Bars + Full Visualizer Overview",
            "lessons_learned": []
        }
    }
...
# =============================================================================
# STATE + THOUGHT LOGGER (zelfde als altijd)
# =============================================================================
def save_state():
    state = {
...
# =============================================================================
# WEBSOCKET + LIVE_JSONL (ongewijzigd)
# =============================================================================
...
# =============================================================================
# HISTORISCHE BARS (nu 1-MINUTE voor betere visualizer + consistentie)
# =============================================================================
        ...
        if new_entries:
            ...
            with open(LIVE_JSONL, "a", encoding="utf-8") as f:
                for entry in new_entries:
                    f.write(json.dumps({**entry, "current_dream": current_dream}) + "\n")
            print(f"✅ GAP RECOVERY: {len(new_entries)} nieuwe 1-min bars toegevoegd")
            return len(new_entries)
    return 0

def fetch_historical_bars(days_back=2, limit=2000):
    print(f"📥 Ophalen {limit} historische 1-MINUTE bars (laatste {days_back} dagen)...")
    try:
        payload = {
            "instrument": INSTRUMENT,
            "periodType": "minute",
            "period": 1,           # ← VERANDERD NAAR 1-MINUTE
            "daysBack": days_back,
            "limit": limit
        }
        r = requests.post(
        ...
        if r.status_code == 200:
            ...
            print(f"✅ {len(bars)} 1-min bars opgehaald → {added} nieuwe toegevoegd")
            return True
        else:
            print(f"❌ Historical bars mislukt: status {r.status_code}")
            return False
    except Exception as e:
...
def gap_recovery_daemon():
    while True:
        ...
        try:
            ...
            if max_gap > 120:   # > 2 minuten gat (aangepast voor 1-min bars)
                print(f"[{datetime.now().strftime('%H:%M:%S')}] ⚠️ GAP DETECTED ({max_gap/60:.1f} min) → recovery gestart")
                fetch_historical_bars(days_back=2, limit=2000)
            else:
                fetch_historical_bars(days_back=2, limit=2000)
        except Exception as e:
            logger.error(f"Gap recovery error: {e}")

# =============================================================================
# FINNHUB NEWS + DREAM 2.0 + SUPERVISOR + DNA + BACKTESTER (ongewijzigd)
# =============================================================================
...
def pre_dream_daemon():
    ...
    if len(live_data) < 200:
        fetch_historical_bars(days_back=2, limit=2000)   # ← meer data bij start
...
# =============================================================================
# START – ALLE 4 THREADS HIER (duidelijk)
# =============================================================================
if __name__ == "__main__":
    print("🚀 LUMINA v21.3 – 1-MINUTE BARS + VOLLEDIG VISUALIZER OVERVIEW GESTART")
    ...
    try:
        ...
        save_state()
        print("\n🛑 LUMINA v21.3 gestopt – state opgeslagen.")
