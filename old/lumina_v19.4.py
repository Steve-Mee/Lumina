...
print("🌌 LUMINA v19.4 – EXACT SECONDEN MTF + 100% ROBUUSTE DROMEN")
...
""",
        "evolvable_layer": {
            ...
            "last_reflection": "2026-03-24: Robuuste JSON + exacte seconden MTF"
        }
    }
    ...
    while True:
        try:
            ...
            payload = {
                ...
                "messages": [
                    {"role": "system", "content": f"Je bent LUMINA's brein. Sacred Core is HEILIG. Geef ALTIJD de exacte keys: signal, confidence, stop, target, reason, why_no_trade. Als geen setup voldoet, gebruik signal='NO_TRADE' en leg uit in why_no_trade.\n\nSacred Core:\n{bible['sacred_core']}\n\nEvolvable layer:\n{json.dumps(bible['evolvable_layer'], ensure_ascii=False)}\n\nGeef ALLEEN JSON."},
...
# ====================== SIMULATOR & SUPERVISOR (nu 100% robuust) ======================
sim_position_qty = 0
sim_entry_price = 0.0
sim_unrealized = 0.0
sim_peak = 50000.0
...
def supervisor_loop():
    ...
    while True:
        ...
        # ROBUUSTE CHECK (geen KeyError meer)
    ...
    while True:
        try:
            ...
            payload = {
                "model": "grok-4.20-0309-reasoning",
                "messages": [
                    {"role": "system", "content": "Je bent LUMINA's Bible Evolutie Engine. Sacred Core is HEILIG – NOOIT wijzigen. Verbeter alleen evolvable_layer. Geef ALLEEN JSON met volledige nieuwe evolvable_layer."},
            ...
            if r.status_code == 200:
                ...
                with open(BIBLE_FILE, 'w', encoding='utf-8') as f:
                    json.dump(bible, f, ensure_ascii=False, indent=2)
                print(f"[{datetime.now().strftime('%H:%M:%S')}] 🔥 BIBLE EVOLVED – MTF & filters geoptimaliseerd")
        except:
            pass
        time.sleep(900)
...
if __name__ == "__main__":
    print("🚀 LUMINA v19.4 – 100% ROBUUSTE DROMEN GESTART")
