...
print("🌌 LUMINA v15.1 – PERFORMANCE TRACKING + MUTATION INTEGRATIE – VOLLEDIG IN LINE MET INSTRUCTIES")
print(f"Instrument: {INSTRUMENT} | DRY_RUN={DRY_RUN} | Levend organisme met meting")

if os.path.exists(VERSION_FILE):
    with open(VERSION_FILE, 'r') as f: current_version = f.read().strip()
else:
    current_version = "v15.1"
...
# ====================== MUTATION LOADER (nu écht gebruikt) ======================
def load_active_mutation():
    ...
    try:
        ...
        print(f"[{datetime.now().strftime('%H:%M:%S')}] 🔄 MUTATION LOADED & ACTIEF")
        return module
        ...
        try:
            ...
            payload = {
                ...
                "messages": [
                    ...
                    {"role": "user", "content": "Genereer 1 nieuwe oracle die Sharpe >2.5 brengt op basis van huidige performance."}
                ]
            }
...
def calculate_phoenix_edge(df):
    ...
    edge = ret5 * 45 + ret20 * 25 + (atr / 15) * np.random.normal(0, 0.6) + np.sign(vol_delta) * 0.4
...
def supervisor_node(state):
    ...
    current_equity = last_equity + random.uniform(-50, 150)  # simuleer PnL
    equity_curve.append(current_equity)
    last_equity = current_equity
    pd.DataFrame(equity_curve, columns=["equity"]).to_csv(EQUITY_FILE, index=False)
    
    # Log signal
    row = {"timestamp": datetime.now(), "edge": state.get("imagined_future_edge", 0), "signal": state["final_signal"], "nexus": state.get("nexus_score", 50)}
...
if __name__ == "__main__":
    print("🚀 LUMINA v15.1 – PERFORMANCE TRACKING ACTIEF – DIT IS DE NIEUWE BASIS")
