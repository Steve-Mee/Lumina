...
MAX_RISK_PER_TRADE = 0.02  # 2% max risico per trade (kan later slimmer worden)
MAX_DRAWDOWN = -0.15

print("🌌 LUMINA v18.0 – IRONCLAD CORE – RADICALE EENVOUD + KAPITAALBEHOUD")
...
# ====================== DNA REWRITE ENGINE (alleen edge, veilig) ======================
...
# === Verbeterde edge (symmetrischer + z-score component) ===
def calculate_phoenix_edge(df):
    ...
    # Z-score van prijs t.o.v. recente range
    ...
    edge = (
        1.8 * mom_short +
        0.9 * mom_long +
        1.4 * z_price * -0.75 +          # mean-reversion component
        0.6 * vol_factor
    )
    
    # harde caps
    edge = np.clip(edge, -1.9, 1.9)
    return float(edge)
...
def supervisor_node(state):
    global sim_position_qty, sim_entry_price, sim_last_pnl, max_equity
    ...
    # Eerst checken of we moeten sluiten op reversal of SL/TP
    if sim_position_qty != 0:
        price_diff = current_price - sim_entry_price
        pnl_points = price_diff * sim_position_qty
        pnl_dollars = pnl_points * 5  # MES multiplier
        
        # Simpele SL/TP: 25 punten (~$125 risico)
        ...
        drawdown = (current_equity - sim_peak) / sim_peak
        print(f"[{datetime.now().strftime('%H:%M:%S')}] Equity: ${current_equity:,.0f} | DD: {drawdown*100:.2f}% | Unrealized: {sim_unrealized:+.0f}")
        
        # PnL delta voor sharpe
...
if __name__ == "__main__":
    print("🚀 LUMINA v18.0 – IRONCLAD CORE GESTART – KAPITAALBEHOUD EERST")
