...
def pre_dream_daemon():
    ...
    while True:
        ...
        try:
            ...
            # === v34 VECTOR MEMORY RETRIEVAL ===
            query = f"Prijs {price:.2f} | Regime {regime} | {pa_summary[:100]}"
            past_experiences = retrieve_relevant_experiences(query, n_results=4)
            print(f"[{datetime.now().strftime('%H:%M:%S')}] 📚 Vector memory: {len(past_experiences.splitlines())} relevante ervaringen opgehaald")

            # Vision call met consensus + vector memory
            vision_content = [
                {"type": "text", "text": f"""Multi-Agent Consensus: {consensus['signal']} (conf {consensus['confidence']:.2f})
Agent votes: {consensus['agent_votes']}
Relevante eerdere ervaringen (vector memory):
{past_experiences}
Gebruik dit als basis voor je besluit.
Geef ALLEEN JSON met: signal, confidence, stop, target, reason, why_no_trade, confluence_score, chosen_strategy, fib_levels_drawn, narrative_reasoning"""},
            ...
            payload = {
                "model": VISION_MODEL,
                "messages": [
                    {"role": "system", "content": "Je bent visueel getraind. Gebruik consensus en vector-memory als basis en geef een uitvoerbaar trade-besluit."},
            ...
            if r.status_code == 200:
                ...
                current_dream["confluence_score"] = max(min_conf, float(consensus.get("confidence", 0.5)))
                ...
                # Sla deze ervaring op in vector-DB voor toekomstig gebruik
                store_experience_to_vector_db(
                    context=f"Prijs {price:.2f} | {pa_summary} | Signal {current_dream['signal']} | Conf {current_dream['confluence_score']:.2f}",
                    metadata={
                        "date": datetime.now().isoformat(),
                        "regime": regime,
                        "signal": current_dream['signal'],
                        "outcome": "open" if current_dream['signal'] != "HOLD" else "no_trade"
                    }
                )

                print(f"[{datetime.now().strftime('%H:%M:%S')}] 👁️ v34 VECTOR MEMORY + MULTI-AGENT DREAM: {current_dream.get('chosen_strategy')} → {current_dream['signal']} (conf={current_dream['confluence_score']:.2f})")
            else:
