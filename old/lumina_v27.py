...
signal, confidence, stop, target, reason, why_no_trade, confluence_score, chosen_strategy, fib_levels_drawn, narrative_reasoning"""},
            ...
            if r.status_code == 200:
                ...
                speak(narrative_reasoning)   # ← echte stem!
                
                if DISCORD_WEBHOOK:
                    try:
                        requests.post(DISCORD_WEBHOOK, json={"content": f"**LUMINA v27** {current_dream.get('chosen_strategy')} → {current_dream['signal']}\n\n{narrative_reasoning}"}, timeout=5)
                    except:
                        pass
                
                log_thought({"type": "dream_decision", **dream_json, "narrative": narrative_reasoning})
                print(f"[{datetime.now().strftime('%H:%M:%S')}] 👁️ v27 FULL HUMAN PARTNER: {current_dream.get('chosen_strategy')} → {current_dream['signal']} (conf={min_conf:.2f})")
            else:
                logger.error(f"VISION_API_ERROR,status={r.status_code}")
        ...
        print(f"[{datetime.now().strftime('%H:%M:%S')}] ⏱️ Volledige v27-cyclus: {total_cycle_ms:.0f} ms")
