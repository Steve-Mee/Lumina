...
print("🌌 LUMINA v19.2 – LEVEND ORGANISME MET EXACTE SECONDEN-BASED MTF")
print(f"Trading MES JUN 26 | DRY_RUN={DRY_RUN} | SIMULATE_TRADES={SIMULATE_TRADES} | MTF exact seconden")

# ====================== SACRED BIBLE (onveranderbaar) ======================
def load_bible():
    if os.path.exists(BIBLE_FILE):
        with open(BIBLE_FILE, 'r', encoding='utf-8') as f:
            return json.load(f)
    bible = {  # Sacred Core blijft exact zoals afgesproken
        "sacred_core": """
...
""",
        "evolvable_layer": {
            "mtf_matrix": {"dominant_tf": "240min", "confluence_scores": {"240min_5min": 0.85, "240min_15min": 0.92, "240min_30min": 0.88, "1440min_60min": 0.95, "60min_5min": 0.78, "30min_15min": 0.81}},
            "filters": ["volume_delta > 1.5x avg", "no news in next 15 min"],
            "probability_model": {"base_winrate": 0.62, "confluence_bonus": 0.18},
            "last_reflection": "2026-03-24: Exact seconden-based MTF geïmplementeerd"
        }
    }
...
# ====================== EXACTE SECONDEN-BASED MTF RESAMPLER ======================
TIMEFRAMES = {
    "5min": 300,      # 5 * 60
    "15min": 900,
    "30min": 1800,
...
def get_mtf_snapshots(df):
    if len(df) < 300:
        return "Insufficient data for exact MTF"
    ...
    for tf_name, seconds in TIMEFRAMES.items():
        # Neem exact de laatste 'seconds' data
        ...
        snapshots[tf_name] = {
            ...
            "trend": trend,
            "bars_formed": len(window)
        }
        print(f"[{datetime.now().strftime('%H:%M:%S')}] MTF → {tf_name} gevormd over exact {seconds} seconden ({len(window)} bars)")
    
    return json.dumps(snapshots, ensure_ascii=False)

# ====================== PRE-DREAMING DAEMON (nu met exacte seconden) ======================
    ...
    while True:
        try:
            ...
            payload = {
                ...
                "messages": [
                    {"role": "system", "content": f"Je bent LUMINA's brein. Sacred Core is HEILIG. Analyseer ALLE 6 exacte timeframes.\n\nSacred Core:\n{bible['sacred_core']}\n\nEvolvable layer:\n{json.dumps(bible['evolvable_layer'], ensure_ascii=False)}\n\nGeef ALLEEN JSON: signal, confidence (0-100), stop, target, reason."},
                    {"role": "user", "content": f"Huidige prijs: {price:.2f}\nExacte MTF snapshots (seconden-based):\n{mtf_data}\nWat is je trade volgens mijn regels?"}
                ]
            }
            ...
            if r.status_code == 200:
                ...
                print(f"[{datetime.now().strftime('%H:%M:%S')}] 🧠 EXACT MTF-DROOM: {dream_json['signal']} | Conf {dream_json['confidence']}%")
...
if __name__ == "__main__":
    print("🚀 LUMINA v19.2 – EXACTE SECONDEN-BASED MTF ORGANISME GESTART")
