...
print("🌌 LUMINA v16.9 – IMPROVED EDGE + CUMULATIVE SIM PnL + SHARPE ORACLE")
...
def calculate_phoenix_edge(df):
    if len(df) < 5: return np.random.normal(0, 0.1)
    df = df.copy()
    # RADICALE VERBETERING: RSI + momentum + volume strength
    delta = df['last'].diff()
    ...
    volume_strength = np.sign(vol_delta) * 0.6
    rsi_factor = (rsi.iloc[-1] - 50) / 50 * 0.8
    edge = ret5 * 35 + ret20 * 20 + (atr / 15) * 0.3 + volume_strength + rsi_factor * 0.5
    ...
    if SIMULATE_TRADES:
        ...
        if sim_position_qty != 0:
            ...
            if len(pnl_history) > 20:
                ...
                print(f"[{datetime.now().strftime('%H:%M:%S')}] PERFORMANCE ORACLE: Sharpe = {sharpe:.2f}")
    else:
...
if __name__ == "__main__":
    print("🚀 LUMINA v16.9 – IMPROVED EDGE + PERFORMANCE ORACLE GESTART")
