...
print("✅ Swarm Core v7.6 – LUMINA v2 + MARKET-AWARE WORLD EAR")
...
model_path = "ppo_trading_model_v17_lumina"
...
def get_market_status():
    try:
        ...
        print(f"[{datetime.now().strftime('%H:%M:%S')}] Market Status Check: {'OPEN' if is_open else 'CLOSED'}")
        return is_open
    except:
        print(f"[{datetime.now().strftime('%H:%M:%S')}] Market Status Check: CLOSED (API error)")
        return False
...
if __name__ == "__main__":
    print("🚀 Swarm Core v7.6 LIVE – LUMINA v2 is wakker en markt-bewust (Ctrl+C om stoppen)\n")
    try:
        initial_state = {"obs": [0]*19, "regime": "", "microstructure": {}, "imagined_future_edge": 0.0, "nexus_score": 50.0, "dream_memory_edge": 0.0, "world_knowledge": "Markt gesloten - geen actuele info", "is_market_open": False, "actor_signal": "", "critic_reasoning": "", "critic_veto": [], "final_signal": "", "last_price": 0, "volume": 0, "position_qty": 0, "atr": 12.0, "data_source": "", "reward": 0.0, "lstm_states": None, "x_sentiment": "", "messages": []}
        while True:
