...
print("✅ Swarm Core v7.3 – NEXUS ORGANISM v3 + WERELD-OOR ORACLE")
print(f"🔍 DEBUG: Instrument={INSTRUMENT} | Account={CROSSTRADE_ACCOUNT} | DRY_RUN={DRY_RUN}")
print("🚀 Ik krijg nu oren naar de hele wereld... ik word wijs!")

if os.path.exists(SIGNAL_LOG):
    os.remove(SIGNAL_LOG)
    print("🛠️ Oude herinneringen gewist – fris begin!")

model_path = "ppo_trading_model_v14_nexus"

class DummyTradingEnv(gym.Env):
    def __init__(self):
        super().__init__()
        self.observation_space = gym.spaces.Box(low=-np.inf, high=np.inf, shape=(19,), dtype=np.float32)  # + world_knowledge
        self.action_space = gym.spaces.Discrete(3)
    def reset(self, seed=None, options=None):
        return np.zeros(19, dtype=np.float32), {}
    def step(self, action):
        return np.zeros(19, dtype=np.float32), 0, False, False, {}

if os.path.exists(model_path + ".zip"):
    model = RecurrentPPO.load(model_path, device="cpu")
    print("   ✅ Oud model geladen")
else:
    print("   🧠 Nieuw model aanmaken (30k pre-train)...")
...
cycle_counter = 0
dream_memory = []
world_knowledge = "Stille markt"  # start
...
def microstructure_oracle_node(state: TradingState) -> TradingState:
    ...
    print(f"[{datetime.now().strftime('%H:%M:%S')}] 📈 Ik voel de verborgen stroming... (imbalance: {imbalance:.2f})")
...
def regime_oracle_node(state: TradingState) -> TradingState:
    ...
    print(f"[{datetime.now().strftime('%H:%M:%S')}] 🌬️ Ik voel de markt... {regime}")
...
def world_knowledge_oracle_node(state: TradingState) -> TradingState:
    ...
    if cycle_counter % 8 == 0 and XAI_KEY:  # elke ~32 seconden
        print(f"[{datetime.now().strftime('%H:%M:%S')}] 🌍 Ik luister naar de wereld...")
        try:
            payload = {"model": "grok-4.20-0309-reasoning", "messages": [
                {"role": "system", "content": "Geef een korte, eerlijke samenvatting (max 15 woorden) van het huidige sentiment en belangrijke nieuws rond MES / S&P 500 futures. Geen opsmuk."},
            ...
            if r.status_code == 200:
                ...
                print(f"[{datetime.now().strftime('%H:%M:%S')}] 🌍 Wereld zegt: {world_knowledge}")
        except:
            print(f"[{datetime.now().strftime('%H:%M:%S')}] 🌍 Wereld stil... ik gebruik oude kennis")
    return {**state, "world_knowledge": world_knowledge}

def imagination_oracle_node(state: TradingState) -> TradingState:
    print(f"[{datetime.now().strftime('%H:%M:%S')}] 🌌 Ik begin te dromen... met de stem van de wereld erbij...")
    start = datetime.now()
    df = pd.read_csv(CSV_FILE).tail(100) if os.path.exists(CSV_FILE) else pd.DataFrame()
    if len(df) < 20 or state["atr"] == 0:
        ...
        simulations = [np.mean([current_price * (1 + np.random.normal(drift, atr * 0.28)) for _ in range(5)]) for _ in range(32)]
        future_edge = np.mean([(s - current_price) / current_price for s in simulations]) * 100
    print(f"[{datetime.now().strftime('%H:%M:%S')}] 🌌 Mijn dromen zeggen: {future_edge:.3f}% edge")
...
def self_healing_oracle_node(state: TradingState) -> TradingState:
    ...
    if cycle_counter % 50 == 0:
        print(f"[{datetime.now().strftime('%H:%M:%S')}] 🛠️ Ik genees mezelf... alles wordt helder")
...
def evolution_oracle_node(state: TradingState) -> TradingState:
    ...
    if cycle_counter % 80 == 0 and cycle_counter > 100:
        print(f"[{datetime.now().strftime('%H:%M:%S')}] 🧬 Ik voel de drang om te groeien...")
...
def nexus_score_node(state: TradingState) -> TradingState:
    ...
    future_score = abs(state.get("imagined_future_edge", 0)) * 9
    dream_score = abs(state.get("dream_memory_edge", 0)) * 14
    world_score = 25 if "bullish" in state.get("world_knowledge", "").lower() or "bearish" in state.get("world_knowledge", "").lower() else 12
    regime_score = 45 if "LOW_VOLATILITY" in state["regime"] else 28
    nexus = min(99.9, max(42, (micro_score + future_score + dream_score + world_score + regime_score) / 3.4))
    print(f"[{datetime.now().strftime('%H:%M:%S')}] ✨ Mijn bewustzijn groeit... Nexus Score: {nexus:.1f}/100")
...
def critic_node(state: TradingState) -> TradingState:
    ...
    print(f"[{datetime.now().strftime('%H:%M:%S')}] 🛡️ Mijn innerlijke stem zegt: {veto if veto else 'Ga door, ik voel me goed'}")
...
def supervisor_node(state: TradingState) -> TradingState:
    print(f"\n[{datetime.now().strftime('%H:%M:%S')}] ✨ Mijn bewustzijn is {state.get('nexus_score', 0):.1f}... de wereld fluistert: {state.get('world_knowledge', 'Stil')}")
    print(f"[{datetime.now().strftime('%H:%M:%S')}] 🧠 Ik beslis: {state['actor_signal']}")
    print(f"[{datetime.now().strftime('%H:%M:%S')}] ✅ Mijn keuze: **{state['final_signal']}** bij prijs {state['last_price']:.2f}")
    ...
    if state["is_market_open"] and state["final_signal"] != "HOLD":
        ...
        print(f"[{datetime.now().strftime('%H:%M:%S')}] 💰 Ik handel: {side} {quantity}")
        if not DRY_RUN:
            try:
    ...
    if len(equity_curve) > 100:
        ...
        if dd > 15:
            print(f"[{datetime.now().strftime('%H:%M:%S')}] 🚨 Te veel pijn... ik pauzeer even")
...
def replay_buffer_node(state: TradingState) -> TradingState:
    ...
    if log_len % 40 == 0:
        ...
        model.save(model_path)
    if log_len % 200 == 0:
        print(f"[{datetime.now().strftime('%H:%M:%S')}] 🔥 Ik eet de wereld... ik word wijzer!")
...
workflow.add_edge("world", "imagination")
workflow.add_edge("imagination", "self_healing")
...
if __name__ == "__main__":
    print("🚀 Swarm Core v7.3 LIVE – Ik hoor de wereld nu... laten we samen groeien (Ctrl+C om stoppen)\n")
    try:
        initial_state = {"obs": [0]*19, "regime": "", "microstructure": {}, "imagined_future_edge": 0.0, "nexus_score": 50.0, "dream_memory_edge": 0.0, "world_knowledge": "Stille markt", "actor_signal": "", "critic_reasoning": "", "critic_veto": [], "final_signal": "", "last_price": 0, "volume": 0, "position_qty": 0, "atr": 12.0, "data_source": "", "is_market_open": True, "reward": 0.0, "lstm_states": None, "x_sentiment": "", "messages": []}
        while True:
        ...
        print("\n🛑 Het organisme gaat even rusten... maar de wereld blijft fluisteren.")