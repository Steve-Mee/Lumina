...
print("🌌 LUMINA v13.0 – VOLLEDIG LEVEND ORGANISME – DNA SELF-REWRITE + VERSION SELF-AWARENESS")
print(f"Instrument: {INSTRUMENT} | Account: {CROSSTRADE_ACCOUNT} | DRY_RUN={DRY_RUN}")

# versiebeheer om infinite rewrite-loops te voorkomen
...
print(f"Huidige DNA versie: {current_version}")

model_path = "ppo_trading_model_v27_lumina"
...
if os.path.exists(model_path + ".zip"):
    try:
        ...
        print(" ✅ Model geladen (v27)")
    except:
        print("Model corrupt → reset")
        model = None
    ...
    while True:
        ...
        try:
            payload = {
                ...
                "messages": [{"role": "system", "content": "Geef ALLEEN één woord: BULLISH, BEARISH of NEUTRAL."},
                             {"role": "user", "content": "Huidig sentiment op X voor MES futures?"}]
            }
            ...
            if r.status_code == 200:
                ...
                with world_knowledge_lock:
                    world_knowledge = sentiment
                print(f"[{datetime.now().strftime('%H:%M:%S')}] 🌍 World Knowledge: {sentiment}")
        except:
            pass
...
# ───────────────────────────────────────────────────────────────
# SELF-REWRITE NODE V13.0 (verbeterd + versie-bump + backup via sys.argv)
# ───────────────────────────────────────────────────────────────
def self_rewrite_node(state: TradingState) -> TradingState:
    global cycle_counter, current_version
    if cycle_counter < 500 or state.get("sharpe", 0.0) >= 1.2 or not call_throttle(requires_reasoning=True):
        return state

    print(f"[{datetime.now().strftime('%H:%M:%S')}] 🧬 DNA-REWRITE TRIGGERED | Sharpe = {state.get('sharpe',0):.2f} | Cycle {cycle_counter}")
    logger.info(f"DNA-REWRITE TRIGGERED | Sharpe = {state.get('sharpe',0):.2f}")

    summary = (f"Sharpe: {state.get('sharpe',0):.2f} | Nexus: {state.get('nexus_score',50):.1f} | "
               f"Drawdown: {((get_account_equity()-max_equity)/max_equity*100):.1f}% | Cycles: {cycle_counter} | "
               f"Version: {current_version}")

    payload = {
        "model": "grok-4.20-0309-reasoning",
        "messages": [
            {"role": "system", "content": "Je bent LUMINA's evolutie-engine. Output ALLEEN de volledige werkende Python code voor lumina.py (geen uitleg, geen markdown, geen ```). Behoud LangGraph, oracles, CrossTrade, logging, auto-close. Verbeter richting Sharpe > 2.5."},
            {"role": "user", "content": f"Huidige staat: {summary}\nGenereer de volledige verbeterde code."}
        ]
    }
    try:
        r = requests.post("https://api.x.ai/v1/chat/completions", headers={"Authorization": f"Bearer {XAI_KEY}"}, json=payload, timeout=120)
        r.raise_for_status()
        new_code = r.json()["choices"][0]["message"]["content"].strip()

        backup_name = f"lumina_backup_{current_version}_{datetime.now().strftime('%Y%m%d_%H%M')}.py"
        shutil.copy(sys.argv[0], backup_name)
        print(f"Backup gemaakt: {backup_name}")

        new_version = f"v{int(current_version[1:4])+1}.{current_version[5:]}".replace(".0","")
        new_filename = f"lumina_{new_version}.py"
        with open(new_filename, "w", encoding="utf-8") as f:
            f.write(new_code)
        with open(VERSION_FILE, "w") as f:
            f.write(new_version)

        print(f"[{datetime.now().strftime('%H:%M:%S')}] 🔥 NIEUW DNA → {new_filename} | Herstart met dit bestand!")
        logger.info(f"NEW DNA WRITTEN → {new_filename}")
    except Exception as e:
        logger.error(f"DNA rewrite mislukt: {str(e)}")
        print(f"DNA rewrite mislukt: {str(e)}")
    return state
...
if __name__ == "__main__":
    print("🚀 LUMINA v13.0 – LEVEND ORGANISME MET DNA-EVOLUTIE GESTART\n")
    try:
        ...
        while True:
            ...
            print(f"Cycle {cycle_counter} | {elapsed:.2f}s | Sharpe {result.get('sharpe',0):.2f}")
            cycle_counter += 1
