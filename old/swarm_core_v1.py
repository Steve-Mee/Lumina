...
INSTRUMENT = os.getenv("INSTRUMENT", "MES JUN 26")  # docs-recommended format
...
print("✅ Swarm Core v1 – Echte Multi-Agent Swarm (v3.2 compliant reset)")
...
def supervisor_node(state: TradingState) -> TradingState:
    ...
    if state["is_market_open"] and state["final_signal"] != "HOLD" and state["position_qty"] == 0:
        ...
        if not DRY_RUN:
            ...
            # Echte order placement stub (volgens CrossTrade docs)
            try:
...
if __name__ == "__main__":
    print("🚀 Swarm Core v1 LIVE – Echte Multi-Agent Swarm (Ctrl+C om stoppen)\n")
    try:
        while True:
