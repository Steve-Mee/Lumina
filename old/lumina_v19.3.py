...
print("🌌 LUMINA v19.3 – EXACT SECONDEN MTF + TRANSPARANTE DROMEN")
...
def load_bible():
    if os.path.exists(BIBLE_FILE):
        with open(BIBLE_FILE, 'r', encoding='utf-8') as f:
            return json.load(f)
    bible = {  # Sacred Core exact zoals afgesproken
        "sacred_core": """
...
""",
        "evolvable_layer": {
            ...
            "last_reflection": "2026-03-24: Exact seconden MTF + transparante dromen"
        }
    }
    ...
    while True:
        try:
            ...
            payload = {
                ...
                "messages": [
                    {"role": "system", "content": f"Je bent LUMINA's brein. Sacred Core is HEILIG. Analyseer ALLE 6 exacte timeframes. Als geen setup voldoet aan Sacred Core, zeg NO_TRADE en leg uit waarom in 'why_no_trade'. Anders geef BUY/SELL met hoge confidence wanneer confluence sterk is.\n\nSacred Core:\n{bible['sacred_core']}\n\nEvolvable layer:\n{json.dumps(bible['evolvable_layer'], ensure_ascii=False)}\n\nGeef ALLEEN JSON."},
            ...
            if r.status_code == 200:
                ...
                print(f"[{datetime.now().strftime('%H:%M:%S')}] 🧠 EXACT MTF-DROOM: {dream_json.get('signal','NONE')} | Conf {dream_json.get('confidence',0)}% | {dream_json.get('reason','')[:100]}")
                if "why_no_trade" in dream_json:
                    print(f"   → Waarom geen trade: {dream_json['why_no_trade']}")
...
if __name__ == "__main__":
    print("🚀 LUMINA v19.3 – TRANSPARANTE DROMEN + EXACTE MTF GESTART")
    threading.Thread(target=supervisor_loop, daemon=True).start()  # supervisor_loop uit v19.2
    try:
        while True:
            time.sleep(60)
