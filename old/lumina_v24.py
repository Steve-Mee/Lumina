...
def pre_dream_daemon():
    ...
    while True:
        ...
        try:
            ...
            # Eerste chart (met bot-fibs) voor de vision-model
            chart_base64 = generate_multi_tf_chart()   # zonder AI-fibs nog
            ...
            # Vision prompt - nu met verzoek om zelf fibs te tekenen
            vision_content = [
                {"type": "text", "text": f"""Je bent een ervaren MES daytrader met 15+ jaar ervaring.
Je kijkt NU naar de echte chart image + alle data.
Teken zelf je Fibonacci levels (geef ze terug in JSON als fib_levels_drawn).
Gebruik market structure, fibs, candle patterns, regime en narrative.
Geef ALLEEN JSON met: signal, confidence, stop, target, reason, why_no_trade, confluence_score, chosen_strategy, fib_levels_drawn"""},
            ...
            payload = {
                "model": VISION_MODEL,
                "messages": [
                    {"role": "system", "content": "Je bent visueel getraind. Teken je eigen fibs op de chart en neem een trade-besluit."},
            ...
            if r.status_code == 200:
                ...
                # v24: AI-fibs opslaan en visueel tekenen
                ...
                if AI_DRAWN_FIBS:
                    ...
                    # Genereer tweede chart met AI-fibs (voor gebruiker/feedback)
                ...
                narrative_memory.append({
                    ...
                    "narrative": f"DREAM: {current_dream.get('chosen_strategy')} → {current_dream['signal']} | Conf {min_conf:.2f} | Regime {regime} | AI-fibs: {list(AI_DRAWN_FIBS.keys())}"
                })

                log_thought({"type": "dream_decision", **dream_json})
                print(f"[{datetime.now().strftime('%H:%M:%S')}] 👁️ v24 VISION + FIB DRAWER: {current_dream.get('chosen_strategy')} → {current_dream['signal']} (conf={min_conf:.2f})")
            else:
        ...
        print(f"[{datetime.now().strftime('%H:%M:%S')}] ⏱️ Volledige v24-cyclus: {total_cycle_ms:.0f} ms")
