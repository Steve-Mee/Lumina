...
print("✅ Swarm Core v3.7 – 5-Agent Swarm (RecurrentPPO LSTM + echte CrossTrade PnL)")
...
if os.path.exists(model_path + ".zip"):
    ...
    print("   🧠 Nieuw RecurrentPPO LSTM model aanmaken + pre-train...")
    model = RecurrentPPO("MlpLstmPolicy", DummyTradingEnv(), verbose=0, device="cpu", learning_rate=1e-4, n_steps=128)
...
def get_recent_pnl():  # NIEUWE echte PnL fetch
    try:
        ...
        if r.status_code == 200:
            ...
            if isinstance(executions, list) and len(executions) > 0:
                last_trade = executions[-1]
                pnl = float(last_trade.get("realizedPnL", 0))
                print(f"   📈 Real PnL fetched: {pnl:.2f}")
                return pnl
    except Exception as e:
        print(f"   ⚠️ Executions API (Pro?) error: {e} → simulated fallback")
    return np.random.normal(0.8, 1.5)  # fallback voor DEMO
...
def replay_buffer_node(state: TradingState) -> TradingState:
    ...
    if log_len % 40 == 0 and len(replay_buffer) >= 32:
        print(f"   🧠 REPLAY BUFFER AGENT ACTIVE: {len(replay_buffer)} experiences | Real PnL = {pnl:.2f} | LSTM update...")
        try:
            model.learn(total_timesteps=64, reset_num_timesteps=False, progress_bar=False)
            model.save(model_path)
            print("   ✅ LSTM Model geüpdatet via echte PnL")
        except Exception as e:
...
if __name__ == "__main__":
    print("🚀 Swarm Core v3.7 LIVE – ECHTE CrossTrade PnL + LSTM (Ctrl+C om stoppen)\n")
    try:
        while True:
            initial_state = {
