...
# Laad het getrainde PPO model (CPU forceren = fix warning + sneller)
...
def get_obs(df):
    """Maak observatie voor de RL-agent (exact zoals tijdens training)"""
    if len(df) < 20:
        return np.zeros(5, dtype=np.float32)
    row = df.iloc[-1]
    price = row['last']
    volume = row['volume']
    atr = abs(price - df['last'].iloc[-2]) * 100 / price if len(df) > 1 else 0
    trend = 1 if price > df['last'].iloc[-20:-10].mean() else -1
    position = 0  # later dynamisch
    return np.array([price/1000, volume/100000, atr, trend, position], dtype=np.float32)

def calculate_regime(df):
    """Herbruik regime detectie"""
    if len(df) < 20:
        return "OPBOUWEN"
...
if __name__ == "__main__":
    print("🚀 Live Trader v7 – Echte PPO AI Brain in actie!")
    ...
    try:
        while True:
            ...
            if quote:
                ...
                # Laad volledige data + maak observatie
                ...
                action, _ = model.predict(obs, deterministic=True)  # AI beslissing!
                ...
                print(f"   🧠 PPO AI Signal: **{signal}** (deterministic)")
