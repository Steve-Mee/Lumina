...
print("✅ Swarm Core v3.6 – 5-Agent Swarm (RecurrentPPO LSTM + ReplayBuffer)")
print("💡 Tip: sb3-contrib is geïnstalleerd → LSTM memory active")
...
def replay_buffer_node(state: TradingState) -> TradingState:
    ...
    if log_len % 40 == 0 and len(replay_buffer) >= 32:
        print(f"   🧠 REPLAY BUFFER AGENT ACTIVE: {len(replay_buffer)} experiences | LSTM update...")
        try:
...
if __name__ == "__main__":
    print("🚀 Swarm Core v3.6 LIVE – RecurrentPPO LSTM + ReplayBuffer (Ctrl+C om stoppen)\n")
    try:
        while True:
            initial_state = {
