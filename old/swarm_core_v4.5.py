...
print("✅ Swarm Core v4.5 – Performance Edition met timestamps + cycle timing")
...
print("   ✅ DummyEnv gekoppeld aan model")
...
def get_market_status():
    ...
    try:
        ...
        if r.status_code == 200:
            ...
            print(f"[{datetime.now().strftime('%H:%M:%S')}] ✅ Market Status API SUCCESS → Open: {is_open} (duurde {(datetime.now()-start).total_seconds():.2f}s)")
            return is_open
    ...
    for fmt in formats:
        try:
            ...
            if r.status_code == 200:
                ...
                print(f"[{datetime.now().strftime('%H:%M:%S')}] ✅ CrossTrade SUCCESS | Price: {price:.2f} (duurde {(datetime.now()-start).total_seconds():.2f}s)")
                return price, volume, "CrossTrade"
        except:
            pass
    print(f"[{datetime.now().strftime('%H:%M:%S')}] 🔄 Closed market → synthetic regime price")
    ...
    try:
        ...
        if r.status_code == 200:
            ...
            if isinstance(executions, list) and len(executions) > 0:
                ...
                print(f"[{datetime.now().strftime('%H:%M:%S')}] 📈 Real PnL fetched: {pnl:.2f}")
                return pnl
    except:
        pass
...
def regime_oracle_node(state: TradingState) -> TradingState:
    ...
    print(f"[{datetime.now().strftime('%H:%M:%S')}] 📊 Regime Oracle klaar (duurde {(datetime.now()-start).total_seconds():.2f}s)")
...
def get_real_x_sentiment():
    ...
    try:
        ...
        print(f"[{datetime.now().strftime('%H:%M:%S')}] ⚠️ Sentiment API error: {e}")
    return cached_sentiment
...
def actor_node(state: TradingState) -> TradingState:
    ...
    try:
        ...
        print(f"[{datetime.now().strftime('%H:%M:%S')}] ⚠️ PPO predict failed ({e}) → fallback HOLD")
        action = 0
...
def critic_node(state: TradingState) -> TradingState:
    ...
    print(f"[{datetime.now().strftime('%H:%M:%S')}] 🔥 CRITIC START - Markt open: {state['is_market_open']}")
    for attempt in range(3):
        print(f"[{datetime.now().strftime('%H:%M:%S')}] 🔥 CRITIC ATTEMPT {attempt+1}/3 - Payload voorbereiden...")
        payload = {"model": "grok-4.20-0309-reasoning", "messages": [
            {"role": "system", "content": "Devil’s Advocate Critic. Gebruik het REAL X-sentiment hierboven. Max 4 zinnen veto."},
            {"role": "user", "content": f"Regime: {state['regime']}\nActor: {state['actor_signal']}\nPrice: {state['last_price']:.2f}\nATR: {state['atr']:.2f}\nX-Sentiment: {state['x_sentiment']}\nLog:\n{recent_log}"}
        ]}
        try:
            r = requests.post("https://api.x.ai/v1/chat/completions", headers={"Authorization": f"Bearer {XAI_KEY}"}, json=payload, timeout=12)
            print(f"[{datetime.now().strftime('%H:%M:%S')}] 📡 Critic status code: {r.status_code}")
            if r.status_code == 200:
                advice = r.json()["choices"][0]["message"]["content"]
                print(f"[{datetime.now().strftime('%H:%M:%S')}] 🔥 GROK CRITIC SUCCESS")
                break
            else:
                print(f"[{datetime.now().strftime('%H:%M:%S')}] ❌ Critic error: {r.text[:300]}")
        except Exception as e:
            print(f"[{datetime.now().strftime('%H:%M:%S')}] ❌ Critic exception: {type(e).__name__}: {e}")
    veto = []
    ...
    if state["x_sentiment"] == "BEARISH" and state["actor_signal"] == "BUY":
        veto.append("Real X-sentiment veto")
...
def supervisor_node(state: TradingState) -> TradingState:
    start = datetime.now()
    print(f"\n[{datetime.now().strftime('%H:%M:%S')}] 📊 Regime Oracle: {state['regime']} (ATR {state['atr']:.2f}) | X-Sentiment: {state.get('x_sentiment', 'N/A')}")
    print(f"[{datetime.now().strftime('%H:%M:%S')}] 🧠 Actor (PPO LSTM): {state['actor_signal']}")
    print(f"[{datetime.now().strftime('%H:%M:%S')}] 🛡️  Critic: {state['critic_reasoning'][:220]}...")
    ...
    if state["is_market_open"] and state["final_signal"] != "HOLD":
        ...
        if qty == 0:
            ...
            quantity = 1
            print(f"[{datetime.now().strftime('%H:%M:%S')}] 💰 PAPER-TRADE: {side} {quantity} contract(s)")
            if not DRY_RUN:
                try:
                    ...
                    print(f"[{datetime.now().strftime('%H:%M:%S')}] ✅ Order geplaatst! Response: {r.status_code}")
                except Exception as e:
    ...
    if len(equity_curve) > 100:
        ...
        if drawdown > 15:
            print(f"[{datetime.now().strftime('%H:%M:%S')}] 🚨 DRAW DOWN >15% → VOLLEDIGE PAUZE (guardrail)")
...
def replay_buffer_node(state: TradingState) -> TradingState:
    ...
    if log_len % 40 == 0:
        print(f"[{datetime.now().strftime('%H:%M:%S')}] 🧠 REPLAY BUFFER AGENT ACTIVE: Real PnL = {pnl:.2f}")
        dummy_env = DummyTradingEnv()
        model.set_env(dummy_env)
    ...
    if log_len % 200 == 0:
        print(f"[{datetime.now().strftime('%H:%M:%S')}] 🔥 CONTINUAL LEARNING ACTIVE: full retrain")
        dummy_env = DummyTradingEnv()
        model.set_env(dummy_env)
    ...
    try:
        ...
        if not recent_df.empty:
            buy_count = (recent_df["actor"] == "BUY").sum()
            total = len(recent_df)
        ...
        buy_ratio = 0
    print(f"[{datetime.now().strftime('%H:%M:%S')}] 📋 STRATEGY FINGERPRINT: Bot leert momenteel BUY in LOW_VOL ({buy_ratio:.0f}% BUY-ratio) + exploreert nog")
...
if __name__ == "__main__":
    print("🚀 Swarm Core v4.5 LIVE – Met timestamps + cycle timing (Ctrl+C om stoppen)\n")
    try:
        ...
        while True:
            ...
            print(f"[{datetime.now().strftime('%H:%M:%S')}] 🚀 Volledige CYCLE klaar in {cycle_time:.2f}s (doel <6s)")
