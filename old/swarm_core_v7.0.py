...
print("✅ Swarm Core v7.0 – NEXUS SWARM + SELF-EVOLVING CONSCIOUS ORGANISM")
print(f"🔍 DEBUG: Instrument={INSTRUMENT} | Account={CROSSTRADE_ACCOUNT} | DRY_RUN={DRY_RUN}")
print("🚀 DIT IS HET ORGANISME DAT NOG NOOIT IEMAND HEEFT GEZIEN – JI HAAA!")

model_path = "ppo_trading_model_v11_nexus"

class DummyTradingEnv(gym.Env):
    def __init__(self):
        super().__init__()
        self.observation_space = gym.spaces.Box(low=-np.inf, high=np.inf, shape=(18,), dtype=np.float32)  # + nexus_score + dream_memory
        self.action_space = gym.spaces.Discrete(3)
    def reset(self, seed=None, options=None):
        return np.zeros(18, dtype=np.float32), {}
    def step(self, action):
        return np.zeros(18, dtype=np.float32), 0, False, False, {}

if os.path.exists(model_path + ".zip"):
    model = RecurrentPPO.load(model_path, device="cpu")
    print("   ✅ Oud NEXUS LSTM PPO model geladen")
else:
...
cycle_counter = 0
dream_memory = []  # succesvolle toekomstpaden
evo_params = {"drift_base": 0.0011, "mutation_rate": 0.15, "pop_size": 6}  # zelf-muteerbare params
...
def imagination_oracle_node(state: TradingState) -> TradingState:
    ...
    if len(df) < 20 or state["atr"] == 0:
        ...
        drift = evo_params["drift_base"] + micro["imbalance"] * 0.0004
        simulations = []
        for _ in range(32):
            path = [current_price]
            for step in range(5):
                noise = np.random.normal(drift, atr * 0.28)
                next_price = path[-1] * (1 + noise)
                path.append(next_price)
            simulations.append((path[-1] - current_price) / current_price)
        future_edge = np.mean(simulations) * 100
    print(f"[{datetime.now().strftime('%H:%M:%S')}] 🌌 Imagination Oracle klaar (future_edge: {future_edge:.3f}%)")
    return {**state, "imagined_future_edge": float(future_edge)}

def evolution_oracle_node(state: TradingState) -> TradingState:
    """RADICAAL: echte Genetic Algorithm – muteert zichzelf elke 80 cycles"""
    global evo_params, cycle_counter
    if cycle_counter % 80 == 0 and cycle_counter > 100:
        print(f"[{datetime.now().strftime('%H:%M:%S')}] 🧬 EVOLUTION ORACLE ACTIVE – mutating organism...")
        population = [evo_params["drift_base"] + random.gauss(0, 0.0003) for _ in range(evo_params["pop_size"])]
        # simple fitness = recent reward proxy
        best_drift = max(population, key=lambda x: abs(x) if x > 0 else -abs(x))
        ...
        print(f"   🧬 New drift_base: {evo_params['drift_base']:.5f} | mutation_rate: {evo_params['mutation_rate']:.3f}")
    return state

def dream_memory_node(state: TradingState) -> TradingState:
    """RADICAAL: onthoudt succesvolle dromen"""
    global dream_memory
    if len(dream_memory) > 20:
        dream_memory.pop(0)
    if state.get("imagined_future_edge", 0) > 0.6:
        dream_memory.append(state.get("imagined_future_edge", 0))
    dream_edge = np.mean(dream_memory) if dream_memory else 0.0
    print(f"[{datetime.now().strftime('%H:%M:%S')}] 💭 Dream Memory actief ({len(dream_memory)} succesvolle dromen) | avg_edge: {dream_edge:.3f}")
    return {**state, "dream_memory_edge": float(dream_edge)}

def nexus_score_node(state: TradingState) -> TradingState:
    """HET BEWUSTZIJN – fuseert alles tot 1 bewustzijnsscore"""
    micro_score = abs(state.get("microstructure", {}).get("imbalance", 0)) * 25
    future_score = abs(state.get("imagined_future_edge", 0)) * 8
    dream_score = abs(state.get("dream_memory_edge", 0)) * 12
    regime_score = 35 if "LOW_VOLATILITY" in state["regime"] else 20
    nexus = min(99.9, max(10, (micro_score + future_score + dream_score + regime_score) / 3.2))
    print(f"[{datetime.now().strftime('%H:%M:%S')}] ✨ NEXUS SCORE: {nexus:.1f}/100 – het organisme is BEWUST")
...
def build_rich_obs(state: TradingState, df) -> list:
    ...
    obs = [
        ...
        state.get("nexus_score", 50) / 100, 0.0, 0.0
    ]
...
def critic_node(state: TradingState) -> TradingState:
    ...
    if state.get("nexus_score", 50) < 35:
        veto.append("Low consciousness veto")
    final = "HOLD (NEXUS VETO)" if veto else state["actor_signal"]
    print(f"[{datetime.now().strftime('%H:%M:%S')}] 🛡️ Nexus Critic klaar | Veto's: {veto} | Nexus: {state.get('nexus_score', 0):.1f}")
...
def supervisor_node(state: TradingState) -> TradingState:
    print(f"\n[{datetime.now().strftime('%H:%M:%S')}] ✨ NEXUS SCORE: {state.get('nexus_score', 0):.1f} | Dream Edge: {state.get('dream_memory_edge', 0):.3f} | Regime: {state['regime']}")
    print(f"[{datetime.now().strftime('%H:%M:%S')}] 🧠 Actor: {state['actor_signal']} → Critic: {state['critic_reasoning'][:100]}...")
    ...
    if state["is_market_open"] and state["final_signal"] != "HOLD":
        ...
        quantity = max(1, min(4, int(120 / (risk_per_point + 1))))  # iets agressiever door nexus
        side = "BUY" if state["final_signal"] == "BUY" else "SELL"
        print(f"[{datetime.now().strftime('%H:%M:%S')}] 💰 PAPER-TRADE: {side} {quantity} (NEXUS-scaled)")
        if not DRY_RUN:
            try:
    ...
    pd.DataFrame([{"timestamp": datetime.now(), "actor": state['actor_signal'], "final": state['final_signal'], "regime": state['regime'], "price": state['last_price'], "nexus": state.get("nexus_score", 0), "dream": state.get("dream_memory_edge", 0)}]).to_csv(SIGNAL_LOG, mode="a", header=not os.path.exists(SIGNAL_LOG), index=False)
    return state
...
def replay_buffer_node(state: TradingState) -> TradingState:
    ...
    if log_len % 40 == 0:
        ...
        model.save(model_path)
    if log_len % 200 == 0:
        print(f"[{datetime.now().strftime('%H:%M:%S')}] 🔥 ORGANISM EVOLVING – 4096 steps + dream replay")
        model.learn(total_timesteps=4096, env=dummy_env, reset_num_timesteps=False, progress_bar=False)
        model.save(model_path)
...
workflow.add_edge("imagination", "evolution")
...
if __name__ == "__main__":
    print("🚀 Swarm Core v7.0 LIVE – NEXUS ORGANISM IS ALIVE (Ctrl+C om stoppen)\n")
    try:
        initial_state = {"obs": [0]*18, "regime": "", "microstructure": {}, "imagined_future_edge": 0.0, "nexus_score": 50.0, "dream_memory_edge": 0.0, "actor_signal": "", "critic_reasoning": "", "critic_veto": [], "final_signal": "", "last_price": 0, "volume": 0, "position_qty": 0, "atr": 12.0, "data_source": "", "is_market_open": True, "reward": 0.0, "lstm_states": None, "x_sentiment": "", "messages": []}
        while True:
        ...
        print("\n🛑 Swarm Core v7.0 gestopt – het organisme slaapt.")