...
print("🌌 LUMINA v19.7 – WEBSOCKET STREAMING + ECHTE OHLC BARS (jouw fix)")
...
""",
        "evolvable_layer": {
            ...
            "last_reflection": "2026-03-26: Jouw WebSocket-fix + echte OHLC bars"
        }
    }
...
# =============================================================================
# WEBSOCKET (jouw fix met additional_headers)
# =============================================================================
async def websocket_listener():
    ...
    try:
        async with websockets.connect(
            uri,
            additional_headers=headers,   # jouw fix
            ping_interval=20,
            ping_timeout=20
        ) as ws:
        ...
        print(f"[{datetime.now().strftime('%H:%M:%S')}] ⚠️ WebSocket mislukt ({e}) – val terug op REST polling")
        while True:
            price, vol = fetch_quote()
            live_data.append({"timestamp": datetime.now(), "last": price, "volume": vol})
            if len(live_data) > 10000:
                live_data.pop(0)
            time.sleep(1)
...
if __name__ == "__main__":
    print("🚀 LUMINA v19.7 – JOUW WEBSOCKET-FIX GESTART")
