# === lumina_v8.9.py – LUMINA v4.0 + ASYNCHRONOUS WORLD KNOWLEDGE + LIVE PERFORMANCE ===
import os
import time
import pandas as pd
import numpy as np
...
print("✅ lumina_v8.9.py – LUMINA v4 + ASYNCHRONOUS WORLD KNOWLEDGE")
...
# === BACKGROUND THREAD VOOR WORLD KNOWLEDGE (geen blokkade meer!) ===
def background_world_knowledge_update():
    global world_knowledge, last_sentiment_time
    while True:
        time.sleep(30)  # update elke 30 seconden, geen blokkade
        if not XAI_KEY:
            continue
        try:
            ...
            if r.status_code == 200:
                new_knowledge = r.json()["choices"][0]["message"]["content"].strip()
                with world_knowledge_lock:
                    world_knowledge = new_knowledge
                print(f"[{datetime.now().strftime('%H:%M:%S')}] 🌍 World Knowledge background update: {new_knowledge}")
                logger.info(f"World Knowledge background update: {new_knowledge}")
        except:
            pass
...
def supervisor_node(state: TradingState) -> TradingState:
    ...
    # === LIVE PERFORMANCE TRACKING (echte positie + PnL) ===
    position = get_current_position()
    current_price = state["last_price"]
    estimated_pnl = position * (current_price - 6500) * 5 if position != 0 else 0
    if len(equity_curve) > 100:
        recent_max = max(equity_curve[-100:])
        drawdown = (recent_max - current_price) / recent_max * 100 if recent_max > 0 else 0
    else:
        drawdown = 0
    print(f"[{datetime.now().strftime('%H:%M:%S')}] 📈 Live Performance: Positie={position} | Geschat PnL={estimated_pnl:.1f} | Drawdown={drawdown:.1f}%")
    ...
    # ALLEEN echte orders uitvoeren (geen false prints meer!)
...
def replay_buffer_node(state: TradingState) -> TradingState:
    ...
    if log_len % 40 == 0:
        model.learn(total_timesteps=128, reset_num_timesteps=False, progress_bar=False)
        model.save(model_path)
    if log_len % 200 == 0:
...
if __name__ == "__main__":
    print("🚀 lumina_v8.9 LIVE – LUMINA v4 met BACKGROUND World Knowledge (geen blokkade meer!) (Ctrl+C om stoppen)\n")
    try:
