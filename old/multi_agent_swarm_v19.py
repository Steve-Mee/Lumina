...
# ==================== CONFIG ====================
INSTRUMENT = os.getenv("INSTRUMENT", "MES1!")  # primary = MES1! (aanbevolen)
...
print("✅ Multi-Agent Swarm v19 – MES1! + yfinance 24/7 fallback + full debug (v3.2 compliant)")
...
def fetch_quote():
    # 1. Probeer CrossTrade met meerdere formats
    formats = [INSTRUMENT, "MES1!", "MES JUN 26", "MESM6", "MES 06-26", "MES JUN26", "MES%20JUN%2026"]
    for fmt in formats:
    ...
    # 2. Fallback: yfinance (werkt 24/7, zelfs nu op zondag)
    print("   🔄 CrossTrade unavailable (markt gesloten) → yfinance fallback")
    try:
        ...
        if not data.empty:
            ...
            print(f"   ✅ yfinance SUCCESS | Price: {price:.2f} | Volume: {volume}")
            return price, volume
    except Exception as e:
...
if __name__ == "__main__":
    print("🚀 v19 Swarm LIVE – MES1! + 24/7 yfinance fallback (Ctrl+C om stoppen)\n")
    try:
        while True:
