...
print("✅ Multi-Agent Swarm v21 – ATR Capped + Volume Normalized + CSV Cleaning (v3.2 compliant)")
...
def fetch_quote():
    # CrossTrade primary
    formats = [INSTRUMENT, "MES1!", "MES JUN 26", "MESM6", "MESH26.CME", "MESM26.CME"]
    for fmt in formats:
        try:
            ...
            if r.status_code == 200:
                ...
                print(f"   ⚠️ CrossTrade {r.status_code} met '{fmt}' → {r.text[:150]}")
        except Exception as e:
            print(f"   ⚠️ CrossTrade exception met '{fmt}': {e}")

    # yfinance 24/7 fallback
    yf_tickers = ["MES=F", "ES=F"]
    for tick in yf_tickers:
        try:
            ...
            if not data.empty:
                ...
                if volume < 50000:
                    volume = 150000  # normalized
                print(f"   ✅ yfinance SUCCESS met {tick} | Price: {price:.2f} | Vol: {volume}")
                return price, volume, f"yfinance ({tick})"
        except Exception as e:
            print(f"   ⚠️ yfinance {tick} failed: {str(e)[:80]}")

    # noodfallback
    print("   ❌ Alles gefaald → random mock")
...
def calculate_atr(df):
    if len(df) < 10:
        return 12.0
    # Gefixte ATR met clip + smoothing
    changes = df['last'].diff().abs()
    atr = changes.rolling(14).mean().iloc[-1]
    atr = np.clip(atr, 8.0, 35.0)  # realistisch voor MES
    return atr
...
if __name__ == "__main__":
    print("🚀 v21 Swarm LIVE – ATR capped + volume normalized (Ctrl+C om stoppen)\n")
    try:
        while True:
