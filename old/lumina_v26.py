...
def generate_multi_tf_chart(ai_fibs: dict | None = None) -> str | None:
    """Genereert chart met fibs, BOS/CHOCH, OB, FVG en optioneel AI-getekende fibs"""
    ...
    logger.info(f"CHART_GEN_COMPLETE,duration_ms={duration_ms:.0f},base64_kb={len(base64_img)//1000},ai_fibs_drawn={len(ai_fibs) if ai_fibs else 0}")
    print(f"[{datetime.now().strftime('%H:%M:%S')}] 🖼️ v24 Chart met AI-fibs gegenereerd in {duration_ms:.0f} ms")
...
def pre_dream_daemon():
    global current_dream, dynamic_min_confluence, AI_DRAWN_FIBS
    while True:
        ...
        try:
            ...
            # v25: Vision prompt met expliciete vraag om narrative
            vision_content = [
                {"type": "text", "text": f"""Je bent een ervaren MES daytrader met 15+ jaar ervaring.
Je kijkt NU naar de echte chart image + alle data.
Geef een VOLLEDIGE hardop redenering in het veld 'narrative_reasoning' (alsof je in Discord spreekt).
Teken zelf je Fibonacci levels (fib_levels_drawn).
Geef ALLEEN JSON met: 
signal, confidence, stop, target, reason, why_no_trade, confluence_score, chosen_strategy, fib_levels_drawn, narrative_reasoning"""},
            ...
            payload = {
                "model": VISION_MODEL,
                "messages": [
                    {"role": "system", "content": "Je bent visueel getraind. Praat als een echte trader in Discord: leg stap voor stap uit wat je ziet en waarom je trade."},
            ...
            if r.status_code == 200:
                ...
                try:
                    ...
                    dream_json = json.loads(json_match.group(0)) if json_match else {"signal": "HOLD", "reason": "Vision parse error"}
                    logger.info("VISION_JSON_PARSE_SUCCESS,method=regex_fallback")
                
                current_dream.update(dream_json)
                current_dream["confluence_score"] = min_conf
                
                # v24 AI-fibs opslaan
                raw_fibs = dream_json.get("fib_levels_drawn", {})
                AI_DRAWN_FIBS = raw_fibs if isinstance(raw_fibs, dict) else {}
                if AI_DRAWN_FIBS:
                    logger.info(f"AI_FIBS_DRAWN,levels={list(AI_DRAWN_FIBS.keys())}")
                    ai_chart_base64 = generate_multi_tf_chart(AI_DRAWN_FIBS)
                    print(f"[{datetime.now().strftime('%H:%M:%S')}] 🎨 AI heeft {len(AI_DRAWN_FIBS)} fib-levels getekend!")
                
                # v25: Narrative Voice
                narrative_reasoning = dream_json.get("narrative_reasoning", "Geen redenering ontvangen")
                narrative_memory.append({
                    "ts": datetime.now().isoformat(),
                    "narrative": f"DREAM: {current_dream.get('chosen_strategy')} → {current_dream['signal']} | {narrative_reasoning[:200]}"
                })
                
                # Mooi geformatteerde Discord-style output
                print("\n" + "="*80)
                print(f"🗣️ TRADER SPEAKS @ {datetime.now().strftime('%H:%M:%S')}")
                print(narrative_reasoning)
                print("="*80 + "\n")
                
                # Optioneel naar Discord sturen
                if DISCORD_WEBHOOK:
                    try:
                        discord_payload = {
                            "content": f"**LUMINA v25** {current_dream.get('chosen_strategy')} → {current_dream['signal']}\n\n{narrative_reasoning}"
                        }
                ...
                log_thought({"type": "dream_decision", **dream_json, "narrative": narrative_reasoning})
                print(f"[{datetime.now().strftime('%H:%M:%S')}] 👁️ v25 VISION + NARRATIVE: {current_dream.get('chosen_strategy')} → {current_dream['signal']} (conf={min_conf:.2f})")
            else:
        ...
        logger.info(f"VISION_CYCLE_COMPLETE,total_ms={total_cycle_ms:.0f}")
        print(f"[{datetime.now().strftime('%H:%M:%S')}] ⏱️ Volledige v25-cyclus: {total_cycle_ms:.0f} ms")
...
def reflect_on_trade(pnl_dollars: float, entry_price: float, exit_price: float, position_qty: int):
    ...
    try:
        ...
        chart_base64 = generate_multi_tf_chart(AI_DRAWN_FIBS)
...
Geef ALLEEN JSON met: reflection (max 400 chars), key_lesson, suggested_bible_update"""},
        ...
        if r.status_code == 200:
            ...
            # Naar Discord (optioneel)
            if DISCORD_WEBHOOK:
                try:
                    discord_msg = {
                        "content": f"**📝 LUMINA REFLECTION**\nResultaat: {'WIN' if pnl_dollars > 0 else 'LOSS'} ${pnl_dollars:.0f}\n{ref_json.get('reflection','')}"
                    }
                    requests.post(DISCORD_WEBHOOK, json=discord_msg, timeout=5)
                except:
                    pass
    except Exception as e:
    ...
    while True:
        ...
        if sim_position_qty != 0:
            ...
            if hit_stop or hit_target or opposite or not is_market_open():
                ...
                # === v26 SELF-IMPROVING REFLECTIE ===
                print(f"[{now.strftime('%H:%M:%S')}] ✅ SIM CLOSE @ {price:.2f} | PnL ${pnl_dollars:.0f}")
                reflect_on_trade(pnl_dollars, sim_entry_price, price, sim_position_qty)
