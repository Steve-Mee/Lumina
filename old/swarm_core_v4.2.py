...
print("✅ Swarm Core v4.2 – Gefixt: Critic API + Dynamische Fingerprint")
    ...
    if state["is_market_open"]:
        for attempt in range(3):
            try:
                ...
                print(f"   ⚠️ Critic API error (attempt {attempt+1}): {e}")
    veto = []
...
def replay_buffer_node(state: TradingState) -> TradingState:
    ...
    # Gefixeerde dynamische fingerprint
    try:
...
if __name__ == "__main__":
    print("🚀 Swarm Core v4.2 LIVE – Gefixt Critic + Correcte Fingerprint (Ctrl+C om stoppen)\n")
    try:
