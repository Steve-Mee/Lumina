...
print("✅ Swarm Core v5.1 – RULE-BASED DEVIL’S ADVOCATE + FULL ERROR LOGGING")
print(f"🔍 DEBUG: Instrument={INSTRUMENT} | Account={CROSSTRADE_ACCOUNT} | DRY_RUN={DRY_RUN}")
if not XAI_KEY:
    print("⚠️ WAARSCHUWING: XAI_API_KEY ontbreekt in .env → Critic gebruikt 100% rule-based fallback")

model_path = "ppo_trading_model_v9_lstm"
...
if os.path.exists(model_path + ".zip"):
    ...
    print("   ✅ Oud LSTM PPO model geladen (v9)")
else:
    print("   🧠 Nieuw RecurrentPPO LSTM model aanmaken (15k pre-train steps)...")
    model = RecurrentPPO("MlpLstmPolicy", DummyTradingEnv(), verbose=0, device="cpu", learning_rate=1e-4, n_steps=128, ent_coef=0.08)
    model.learn(total_timesteps=15000, progress_bar=False)  # v5.1 boost
...
def get_real_x_sentiment():
    ...
    if time.time() - last_sentiment_time < 90:  # v5.1 cache verlengd
    ...
    try:
        ...
        print(f"[{datetime.now().strftime('%H:%M:%S')}] 📡 Sentiment API status: {r.status_code}")
        if r.status_code == 200:
            ...
            print(f"⚠️ Sentiment API error body: {r.text[:300]}")
    except Exception as e:
        print(f"⚠️ EXACT Sentiment error: {type(e).__name__}: {e}")
    return cached_sentiment
...
def actor_node(state: TradingState) -> TradingState:
    ...
    # v5.1: forced exploration eerste 50 cycles
    if cycle_counter < 50:
        action = np.random.randint(0, 3)
        new_lstm_states = None
        print(f"[{datetime.now().strftime('%H:%M:%S')}] 🧠 Actor EXPLORATION MODE (random action)")
    else:
        try:
...
def critic_node(state: TradingState) -> TradingState:
    ...
    if micro["imbalance"] < -1.2 and state["actor_signal"] == "BUY":
        ...
        advice = "Negative micro imbalance → BUY veto"
    if micro["imbalance"] > 1.2 and state["actor_signal"] == "SELL":
        veto.append("Positive imbalance veto")
        advice = "Positive micro imbalance → SELL veto"
    if abs(state["atr"]) > 28 and state["actor_signal"] != "HOLD":
        veto.append("Extreme ATR veto")
        advice = "Extreme ATR → veto non-HOLD"

    # LLM call alleen elke 8 cycles (throttle + enhancement)
    if (cycle_counter % 8 == 0) and XAI_KEY and state["is_market_open"]:
        print(f"[{datetime.now().strftime('%H:%M:%S')}] 🔥 CRITIC LLM CALL (cycle {cycle_counter})")
        try:
            payload = {"model": "grok-4.20-0309-reasoning", "messages": [
                {"role": "system", "content": "Devil’s Advocate Critic. Max 2 zinnen. Geef altijd confidence 0-100."},
            ...
            print(f"[{datetime.now().strftime('%H:%M:%S')}] Critic LLM status: {r.status_code}")
            if r.status_code == 200:
                llm_advice = r.json()["choices"][0]["message"]["content"]
                advice = f"Rule-based + LLM: {llm_advice[:180]}"
                if "veto" in llm_advice.lower():
                    veto.append("LLM veto")
            else:
                print(f"⚠️ Critic LLM error body: {r.text[:400]}")
        except Exception as e:
            print(f"⚠️ EXACT Critic LLM error: {type(e).__name__}: {e}")

    final = "HOLD (Critic VETO)" if veto else state["actor_signal"]
    print(f"[{datetime.now().strftime('%H:%M:%S')}] 🛡️ Critic klaar → {advice} | Veto's: {veto}")
...
def supervisor_node(state: TradingState) -> TradingState:
    ...
    print(f"[{datetime.now().strftime('%H:%M:%S')}] 🧠 Actor: {state['actor_signal']} → Critic: {state['critic_reasoning'][:140]}...")
...
if __name__ == "__main__":
    print("🚀 Swarm Core v5.1 LIVE – RULE-BASED DEVIL’S ADVOCATE (Ctrl+C om stoppen)\n")
    try:
